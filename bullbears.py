...
# Tabs  (UPDATED: added Tab 8 = Kijun Cross)
...
# -------------------------------------------------------------------
# (Tab 1 through Tab 7 remain exactly as provided in Batches 1 and 2)
# -------------------------------------------------------------------
...
with tab8:
    ...
    st.caption(
        "Shows symbols from the **daily chart** where price has recently crossed **Ichimoku Kijun**, "
        "split into: (1) **Going Up** and (2) **Going Down**, and each divided by **slope > 0** vs **slope < 0**."
    )

    kijun_recent_bars = st.slider(
        "Recent Kijun cross window (daily bars)",
        1, 12, 3, 1,
        key="kijun_cross_recent_bars_daily"
    )
    run_kijun_scan = st.button("Scan Daily Kijun Crosses", key="btn_kijun_cross_daily_scan")

    def _latest_recent_kijun_cross_daily(ohlc_df: pd.DataFrame, base: int = 26, recent_bars: int = 3):
        """
        Detect the most recent price/Kijun cross within the last `recent_bars` DAILY bars.
        ...
        ohlc = ohlc_df[['High', 'Low', 'Close']].copy().dropna()
        ...
        dfx = pd.DataFrame({
            ...
            "Kijun": _coerce_1d_series(kijun).astype(float)
        }).dropna()
        ...
        up_cross = (diff > 0) & (prev <= 0)    # price crossed Kijun going up
        dn_cross = (diff < 0) & (prev >= 0)    # price crossed Kijun going down
        ...
        # Most recent = smallest bars_ago; tie-break latest timestamp
        events = sorted(events, key=lambda d: (d["bars_ago"], d["time"]), reverse=False)
        return events[0]

    def _daily_slope_sign(close_like, lookback: int):
        """
        Classify slope sign using regression slope on daily close.
          > 0 => Positive
          < 0 => Negative
          else => Zero/Unknown
        Returns (label, slope_value)
        """
        s = _coerce_1d_series(close_like).astype(float).dropna()
        if s.empty or len(s) < 2:
            return "Zero/Unknown", float("nan")
        ...
        if np.isfinite(m):
            if float(m) > 0:
                return "Slope > 0", float(m)
            elif float(m) < 0:
                return "Slope < 0", float(m)
            else:
                return "Zero/Unknown", float(m)
    ...
    def _sort_kijun_hits(df_in: pd.DataFrame) -> pd.DataFrame:
        ...
        return df_in.sort_values(
            ["Bars Ago", "Timestamp", "Symbol"],
            ascending=[True, False, True]
        ).reset_index(drop=True)

    def _prep_kijun_display(df_in: pd.DataFrame) -> pd.DataFrame:
        if df_in.empty:
            return df_in
        out = df_in.copy()
        out["Close"] = out["Close"].map(lambda v: fmt_price_val(v) if np.isfinite(v) else "n/a")
        out["Kijun"] = out["Kijun"].map(lambda v: fmt_price_val(v) if np.isfinite(v) else "n/a")
        out["Distance to Kijun"] = out["Distance to Kijun"].map(lambda v: f"{v:+.4f}" if np.isfinite(v) else "n/a")
        out["Slope"] = out["Slope"].map(lambda v: fmt_slope(v) if np.isfinite(v) else "n/a")
        return out

    if run_kijun_scan:
        rows_up_pos = []     # price crossed Kijun up, slope > 0
        rows_up_neg = []     # price crossed Kijun up, slope < 0
        rows_dn_pos = []     # price crossed Kijun down, slope > 0
        rows_dn_neg = []     # price crossed Kijun down, slope < 0
        rows_uncat = []      # optional: slope == 0 or unavailable
        ...
        for i, sym in enumerate(universe, start=1):
            try:
                ...
                event = _latest_recent_kijun_cross_daily(
                    df_ohlc_sym,
                    base=ichi_base,
                    recent_bars=kijun_recent_bars
                )

                if event is not None:
                    close_series_daily = (
                        _coerce_1d_series(df_ohlc_sym["Close"]).astype(float).dropna()
                        if df_ohlc_sym is not None and "Close" in df_ohlc_sym.columns
                        else pd.Series(dtype=float)
                    )
                    slope_lbl, slope_val = _daily_slope_sign(close_series_daily, lookback=int(slope_lb_daily))
                    ...
                    row = {
                        ...
                        "Slope Bucket": slope_lbl,
                        "Slope": slope_val
                    }

                    if event["direction"] == "UP":
                        if slope_lbl == "Slope > 0":
                            rows_up_pos.append(row)
                        elif slope_lbl == "Slope < 0":
                            rows_up_neg.append(row)
                        else:
                            rows_uncat.append({**row, "Direction": "UP"})
                    elif event["direction"] == "DOWN":
                        if slope_lbl == "Slope > 0":
                            rows_dn_pos.append(row)
                        elif slope_lbl == "Slope < 0":
                            rows_dn_neg.append(row)
                        else:
                            rows_uncat.append({**row, "Direction": "DOWN"})
            except Exception:
                # Skip symbol to preserve scanner continuity
                pass
        ...
        df_up_pos = _sort_kijun_hits(pd.DataFrame(rows_up_pos))
        df_up_neg = _sort_kijun_hits(pd.DataFrame(rows_up_neg))
        df_dn_pos = _sort_kijun_hits(pd.DataFrame(rows_dn_pos))
        df_dn_neg = _sort_kijun_hits(pd.DataFrame(rows_dn_neg))
        df_uncat  = _sort_kijun_hits(pd.DataFrame(rows_uncat))

        # Metrics
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("Universe Size", len(universe))
        c2.metric("Kijun Up + Slope > 0", int(df_up_pos.shape[0]))
        c3.metric("Kijun Up + Slope < 0", int(df_up_neg.shape[0]))
        c4.metric("Kijun Down + Slope > 0", int(df_dn_pos.shape[0]))
        c5.metric("Kijun Down + Slope < 0", int(df_dn_neg.shape[0]))

        # 1) Price recently crossed Kijun going UP
        st.markdown("---")
        st.subheader("Price recently crossed Kijun Going Up — Slope > 0")
        if df_up_pos.empty:
            st.info(f"No symbols found for **Kijun UP cross** with **slope > 0** in the last {kijun_recent_bars} daily bar(s).")
        else:
            show_df = _prep_kijun_display(df_up_pos)
            st.dataframe(
                show_df[["Symbol", "Timestamp", "Bars Ago", "Slope Bucket", "Slope", "Close", "Kijun", "Distance to Kijun"]],
                use_container_width=True
            )
        ...
        if df_up_neg.empty:
            ...
            show_df = _prep_kijun_display(df_up_neg)
            st.dataframe(
                show_df[["Symbol", "Timestamp", "Bars Ago", "Slope Bucket", "Slope", "Close", "Kijun", "Distance to Kijun"]],
                use_container_width=True
            )
        ...
        st.markdown("---")
        st.subheader("Price recently crossed Kijun Going Down — Slope > 0")
        if df_dn_pos.empty:
            st.info(f"No symbols found for **Kijun DOWN cross** with **slope > 0** in the last {kijun_recent_bars} daily bar(s).")
        else:
            show_df = _prep_kijun_display(df_dn_pos)
            st.dataframe(
                show_df[["Symbol", "Timestamp", "Bars Ago", "Slope Bucket", "Slope", "Close", "Kijun", "Distance to Kijun"]],
                use_container_width=True
            )
        ...
        if df_dn_neg.empty:
            ...
            show_df = _prep_kijun_display(df_dn_neg)
            st.dataframe(
                show_df[["Symbol", "Timestamp", "Bars Ago", "Slope Bucket", "Slope", "Close", "Kijun", "Distance to Kijun"]],
                use_container_width=True
            )

        # Optional (avoids silently dropping slope==0/unavailable)
        if not df_uncat.empty:
            st.markdown("---")
            st.caption("Optional: Kijun cross results where slope = 0 or slope could not be classified.")
            show_df = _prep_kijun_display(df_uncat)
            extra_cols = ["Direction", "Symbol", "Timestamp", "Bars Ago", "Slope Bucket", "Slope", "Close", "Kijun", "Distance to Kijun"]
            present_cols = [c for c in extra_cols if c in show_df.columns]
            st.dataframe(show_df[present_cols], use_container_width=True)

    else:
        st.info(
            "Click **Scan Daily Kijun Crosses** to list symbols where price recently crossed Kijun "
            "going up/down, split into **slope > 0** and **slope < 0**."
        )
