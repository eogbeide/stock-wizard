...
def render_audio_player(audio_bytes: BytesIO, rate: float = 1.5):
    ...
    <script>
      ...
      // Set playback rate after metadata is loaded to avoid being overwritten
      audio.addEventListener('loadedmetadata', () => {{
          audio.playbackRate = {rate};
      }});
      // Also set immediately in case metadata is already available
      audio.playbackRate = {rate};
    </script>
    """
...
if "playback_rate" not in st.session_state:
    st.session_state.playback_rate = 1.5  # default speed 1.5x
    ...
    if st.button("🔊 Read this page aloud"):
        try:
            with st.spinner("Generating audio..."):
                ...
                st.session_state.last_audio = audio_buf  # store for re-rendering at different speeds
        except Exception as e:
