...
# Batch 1/3 — BEGIN through Tabs 1–4 (Tabs 5+ defined; logic added in Batch 2/3)
...
def format_trade_instruction(
    ...
    """
    Shows BUY only when Global + Local slopes agree UP; SELL only when both agree DOWN.
...
# Sessions (PST) — duplicate-key fix: make key unique per mode
...
if mode == "Forex":
    ...
    show_sessions_pst = st.sidebar.checkbox(
        ...
        value=True,
        key=f"sb_show_sessions_pst_{mode}_v2",
    )
...
def fib_reversal_trigger_from_extremes(
    ...
    def _confirmed_up(from_time):
        seg = s.loc[from_time:]
        return bool(
            len(seg) >= int(confirm_bars) + 1
            and np.all(np.diff(seg.iloc[-(int(confirm_bars) + 1):]) > 0)
        )
...
def _fib_npx_zero_signal_series(
    close: pd.Series,
    npx: pd.Series,
    prox: float,
    lookback_bars: int,
    ...
    if cross_up0.any() and touch_lo.any():
        ...
        if touch_before.any():
            ...
            if slope_ok and npx_ok:
                ...
                buy = {"side": "BUY", "time": t_cross, "price": px}
    ...
    if cross_dn0.any() and touch_hi.any():
        ...
        if touch_before.any():
            ...
            if slope_ok and npx_ok:
                px = float(c.loc[t_cross]) if (t_cross in c.index and np.isfinite(c.loc[t_cross])) else np.nan
                sell = {"side": "SELL", "time": t_cross, "price": px}
...
# ---------------------------
# Shared hourly renderer (Stock & Forex) — NEWS REMOVED (THIS REQUEST)
# ---------------------------
def render_hourly_views(sel: str, intraday: pd.DataFrame, p_up: float, p_dn: float, hour_range_label: str, is_forex: bool):
    ...
    return {"trade_instruction": instr_txt, "fib_trigger": trig_disp}
...
# - Replaced with: "Buy Candidates", "NBB -0.8 to -1.0 Lists" (implemented in later batches)
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        if chart in ("Daily", "Both"):
            ...
            if show_bbands and not bb_up_d.dropna().empty and not bb_lo_d.dropna().empty:
                bb_mid_d_show, bb_up_d_show, bb_lo_d_show, _, _ = compute_bbands(df_show, window=bb_win, mult=bb_mult, use_ema=bb_use_ema)
            ...
            ax.set_ylabel("Price")
            ax.text(
                0.50, 0.02, f"R² ({slope_lb_daily} bars): {fmt_r2(r2_d)}",
            ...
            if show_ntd and not ntd_d_show.dropna().empty:
                ...
                overlay_ntd_sr_reversal_stars(
                    axdw, price=df_show, sup=sup_d_show, res=res_d_show,
...
# ---------------------------
# TAB 2: ENHANCED FORECAST (Daily regression + ±2σ added; duplicate key fixed)
# ---------------------------
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        view = st.radio("View:", ["Daily", "Intraday", "Both"], key=f"enh_view_{mode}_v2")
        ...
        if view in ("Daily", "Both"):
            ...
            # THIS REQUEST: regression line + ±2σ on Enhanced Daily chart
            yhat_d, upper_d, lower_d, m_d, r2_d = regression_with_band(df_show, lookback=int(slope_lb_daily), z=2.0)
            ...
            if not yhat_d.dropna().empty:
                ax.plot(yhat_d.index, yhat_d.values, "-", linewidth=2.0, label=f"Regression ({slope_lb_daily} bars)")
            if not upper_d.dropna().empty and not lower_d.dropna().empty:
                ax.plot(upper_d.index, upper_d.values, "--", linewidth=2.0, color="black", alpha=0.85, label="+2σ")
                ax.plot(lower_d.index, lower_d.values, "--", linewidth=2.0, color="black", alpha=0.85, label="-2σ")
...
with tab5:
    ...
    scan_frame = st.radio("Frame:", ["Hourly (intraday)", "Daily"], index=0, key=f"ntd_scan_frame_{mode}_t5")
    run_scan = st.button("Run Scanner", key=f"btn_run_ntd_scan_{mode}_t5")
...
with tab6:
    ...
    sel_lt = st.selectbox("Ticker:", universe, key=f"lt_ticker_{mode}_t6")
    try:
...
with tab7:
    ...
    max_bars = st.slider("Max bars since NPX↑NTD cross", 0, 20, 2, 1, key=f"buy_scan_npx_max_bars_{mode}_t7")
    run_buy_scan = st.button("Run Recent BUY Scan", key=f"btn_run_recent_buy_scan_npx_{mode}_t7")
...
#   - price crossed Ichimoku Kijun within 1–2 bars
#   - regression line slope > 0
# ---------------------------
with tab8:
    st.header("Buy Candidates")
    st.caption(
        "Shows symbols where **price crossed ABOVE Ichimoku Kijun** recently (**1–2 bars ago**) "
        "and **Regression slope > 0** (Daily view range)."
    )

    c1, c2 = st.columns(2)
    max_bars_kij = c1.slider("Max bars since Kijun cross-up", 1, 20, 2, 1, key=f"buycand_max_bars_{mode}_t8")
    include_same_bar = c2.checkbox("Include same-bar crosses (0 bars since)", value=False, key=f"buycand_inc0_{mode}_t8")

    run_buycand = st.button("Run Buy Candidates Scan", key=f"btn_run_buy_candidates_{mode}_t8")
    ...
    if run_buycand:
        ...
        for sym in universe:
            r = last_daily_kijun_cross_up(
                ...
                within_last_n_bars=int(max_bars_kij),
            )
            if r is None:
                continue
            ...
            if include_same_bar:
                ok_bars = (0 <= bars_since <= int(max_bars_kij))
            else:
                ok_bars = (1 <= bars_since <= int(max_bars_kij))
        ...
        if not rows:
            ...
            st.dataframe(out.reset_index(drop=True), use_container_width=True)
# =========================
# BATCH 3 — remainder of tabs (Tab 9 onward)
#   • Tab 9 is NEW (replaces removed Fib NPX tab)
#   • Tabs 10–18 remain (same content) with UNIQUE widget keys
#   • Tab 19 is NEW (NPX 0.0–0.2 in the NTD chart)
# =========================

# ---------------------------
# NEW helper (THIS REQUEST #5): last NBB (daily view)
# ---------------------------
@st.cache_data(ttl=120)
def last_daily_nbb_in_view(symbol: str,
                           daily_view_label: str,
                           bb_window: int,
                           bb_mult: float,
                           use_ema_mid: bool):
    """
    Returns (nbb_last, pctb_last, asof_ts) for the selected daily view range.
    """
    try:
        ...
        _, _, _, pctb, nbb = compute_bbands(
            close_show, window=int(bb_window), mult=float(bb_mult), use_ema=bool(use_ema_mid)
        )
        nbb_s = _coerce_1d_series(nbb).dropna()
        pctb_s = _coerce_1d_series(pctb).dropna()
        if nbb_s.empty:
            return np.nan, np.nan, close_show.index[-1]

        nbb_last = float(nbb_s.iloc[-1]) if np.isfinite(nbb_s.iloc[-1]) else np.nan
        pctb_last = float(pctb_s.iloc[-1]) if (not pctb_s.empty and np.isfinite(pctb_s.iloc[-1])) else np.nan
        return nbb_last, pctb_last, close_show.index[-1]
    except Exception:
        return np.nan, np.nan, None


# ---------------------------
# TAB 9 (NEW): NBB -0.8 to -1.0 Lists (THIS REQUEST #5)
# ---------------------------
with tab9:
    st.header("NBB -0.8 to -1.0 Lists")
    st.caption(
        "Creates two lists where **NBB (normalized Bollinger position)** is between **-1.0 and -0.8** "
        "(near the lower Bollinger band), split by regression slope sign:\n"
        "• (a) **Regression > 0** and **NBB ∈ [-1.0, -0.8]**\n"
        "• (b) **Regression < 0** and **NBB ∈ [-1.0, -0.8]**\n\n"
        "Uses the selected **Daily view range** and current BB settings from the sidebar."
    )

    c1, c2, c3 = st.columns(3)
    nbb_lo = c1.slider("NBB lower bound", -1.00, 1.00, -1.00, 0.01, key=f"nbb_lo_{mode}_t9")
    nbb_hi = c2.slider("NBB upper bound", -1.00, 1.00, -0.80, 0.01, key=f"nbb_hi_{mode}_t9")
    run_nbb = c3.button("Run NBB Scan", key=f"btn_run_nbb_scan_{mode}_t9")
    ...
    if run_nbb:
        ...
        for sym in universe:
            ...
            nbb_last, pctb_last, asof = last_daily_nbb_in_view(
                sym, daily_view_label=daily_view,
                bb_window=int(bb_win), bb_mult=float(bb_mult), use_ema_mid=bool(bb_use_ema)
            )
            ...
            row = {
                ...
                "BB Window": int(bb_win),
                "BB Mult": float(bb_mult),
                "BB Mid": "EMA" if bb_use_ema else "SMA",
            }
        ...
        with left:
            st.subheader("(a) Regression > 0  AND  NBB in range")
            if not rows_up:
                st.info("No matches.")
            else:
                out = pd.DataFrame(rows_up).sort_values(["NBB", "Slope"], ascending=[True, False])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

        with right:
            st.subheader("(b) Regression < 0  AND  NBB in range")
            if not rows_dn:
                st.info("No matches.")
            else:
                out = pd.DataFrame(rows_dn).sort_values(["NBB", "Slope"], ascending=[True, True])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)
...
with tab10:
    ...
    run_slope = st.button("Run Slope Direction Scan", key=f"btn_run_slope_dir_{mode}_t10")
...
with tab11:
    ...
    run_trend_lists = st.button("Run Trendline Direction Lists", key=f"btn_run_trendline_lists_{mode}_t11")
...
with tab12:
    ...
    run_hot = st.button("Run NTD Hot List", key=f"btn_run_ntd_hot_{mode}_t12")
...
# ---------------------------
# TAB 13: NTD NPX 0.0–0.2 Scanner (existing)
# ---------------------------
with tab13:
    ...
    c1, c2 = st.columns(2)
    npx_up_bars = c1.slider("NPX heading-up confirmation (consecutive bars)", 1, 5, 1, 1, key=f"npx_02_up_bars_{mode}_t13")
    run_npx02 = c2.button("Run NTD NPX 0.0-0.2 Scan", key=f"btn_run_npx02_{mode}_t13")
...
with tab14:
    ...
    c1, c2 = st.columns(2)
    hz_sr = c1.slider("Support-touch lookback window (bars)", 1, 60, int(max(3, rev_horizon)), 1, key=f"ud_sr_hz_{mode}_t14")
    run_ud = c2.button("Run Uptrend vs Downtrend Scan", key=f"btn_run_ud_{mode}_t14")
...
with tab15:
    ...
    c1, c2 = st.columns(2)
    kijun_within = c1.slider("Cross must be within last N bars", 0, 60, 5, 1, key=f"kijun_within_n_{mode}_t15")
    run_kijun = c2.button("Run Ichimoku Kijun Scan", key=f"btn_run_kijun_scan_{mode}_t15")
...
with tab16:
    ...
    c1, c2, c3 = st.columns(3)
    r2_thr = c1.slider("R² threshold", 0.00, 1.00, 0.45, 0.01, key=f"r2_thr_scan_{mode}_t16")
    hour_period = c2.selectbox("Hourly intraday period", ["1d", "2d", "4d"], index=0, key=f"r2_hour_period_{mode}_t16")
    run_r2 = c3.button("Run R² Scan", key=f"btn_run_r2_scan_{mode}_t16")
...
with tab17:
    ...
    st.caption(
        ...
        "• **Daily:** regression_with_band on daily close\n"
        "• **Hourly (intraday):** regression_with_band on intraday close\n\n"
        "Scanner tab only."
    )

    c1, c2, c3 = st.columns(3)
    r2_thr_lo = c1.slider("R² threshold", 0.00, 1.00, 0.45, 0.01, key=f"r2_thr_scan_lo_{mode}_t17")
    hour_period_lo = c2.selectbox("Hourly intraday period", ["1d", "2d", "4d"], index=0, key=f"r2_hour_period_lo_{mode}_t17")
    run_r2_lo = c3.button("Run R² Low Scan", key=f"btn_run_r2_scan_lo_{mode}_t17")
...
with tab18:
    ...
    run_band_scan = st.button("Run R² Sign ±2σ Proximity Scan (Daily)", key=f"btn_run_r2_sign_band_scan_{mode}_t18")
...
# ---------------------------
# TAB 19 (NEW — THIS REQUEST #6): NPX 0.0–0.2 in the NTD chart
# ---------------------------
with tab19:
    st.header("NPX 0.0–0.2 (NTD Chart)")
    st.caption(
        "Scanner + mini-viewer:\n"
        "• Lists symbols where **NPX (Norm Price)** is between **0.0** and **0.2** (Daily view range)\n"
        "• Optionally displays the **NTD chart** (NTD + NPX overlay) for a selected symbol\n\n"
        "This does not change any existing charts; it is an added tab only."
    )

    c1, c2, c3 = st.columns(3)
    npx_min = c1.slider("NPX min", -1.00, 1.00, 0.00, 0.01, key=f"npx02_min_{mode}_t19")
    npx_max = c2.slider("NPX max", -1.00, 1.00, 0.20, 0.01, key=f"npx02_max_{mode}_t19")
    run_scan_19 = c3.button("Run NPX 0.0–0.2 Scan", key=f"btn_run_npx02_ntd_{mode}_t19")

    if "tab19_matches" not in st.session_state:
        st.session_state.tab19_matches = []

    if run_scan_19:
        rows = []
        lo_v = float(min(npx_min, npx_max))
        hi_v = float(max(npx_min, npx_max))

        for sym in universe:
            # Daily view series (so results match the visual daily view)
            ...
            ntd_s = _coerce_1d_series(ntd).dropna()
            if npx_s.empty:
                continue

            npx_last = float(npx_s.iloc[-1]) if np.isfinite(npx_s.iloc[-1]) else np.nan
            ntd_last = float(ntd_s.iloc[-1]) if (not ntd_s.empty and np.isfinite(ntd_s.iloc[-1])) else np.nan
            if not np.isfinite(npx_last):
                continue

            if lo_v <= float(npx_last) <= hi_v:
                m, r2, ts = daily_global_slope(sym, daily_view_label=daily_view)
                rows.append({
                    "Symbol": sym,
        ...
        st.session_state.tab19_matches = rows

    rows = st.session_state.get("tab19_matches", [])
    if not rows:
        st.info("Run the scan to populate matches.")
    else:
        out = pd.DataFrame(rows).sort_values(["NPX (last)", "Slope"], ascending=[True, False])
        st.dataframe(out.reset_index(drop=True), use_container_width=True)

        syms = out["Symbol"].tolist()
        sel_sym = st.selectbox("Show NTD chart for symbol:", syms, key=f"tab19_sel_{mode}_t19")

        # Plot NTD chart (Daily view subset)
        close_full = _coerce_1d_series(fetch_hist(sel_sym)).dropna()
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view)).dropna()
        ...
        ax.set_title(f"{sel_sym} — NTD Chart (Daily view: {daily_view})")

        if shade_ntd and show_ntd:
            shade_ntd_regions(ax, ntd)

        if show_ntd:
            ax.plot(ntd.index, ntd.values, "-", linewidth=1.8, label=f"NTD (win={ntd_window})")
        ax.plot(npx.index, npx.values, "-", linewidth=1.2, alpha=0.9, label="NPX (Norm Price)")

        ax.axhline(0.0, linestyle="--", linewidth=1.0, color="black", label="0.00")
        ax.axhline(0.5, linestyle="-", linewidth=1.0, color="black", alpha=0.5)
        ax.axhline(-0.5, linestyle="-", linewidth=1.0, color="black", alpha=0.5)
        ax.set_ylim(-1.1, 1.1)
        ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.18), ncol=4, framealpha=0.65, fontsize=9, fancybox=True)
        style_axes(ax)
