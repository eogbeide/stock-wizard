...
# =========================
# (Scanning helpers continue in later parts of the original file)
...
# UPDATED (THIS REQUEST):
# - Removed: NPX 0.5-Cross Scanner tab, Fib NPX 0.0 Signal Scanner tab, Ichimoku Kijun Scanner tab
# - Added: HMA Buy tab, NPX Cross (0.0/0.5 Up) tab
...
with tab1:
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        # NOTE: Remaining Tab 1 logic continues exactly as in your original codebase.
        # (Unchanged here; continued in subsequent batches where applicable.)
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        if view in ("Daily", "Both"):
            ...
            fib_sig_d = fib_reversal_trigger_from_extremes(
                df_show,
            ...
            if isinstance(fib_sig_d, dict):
                annotate_reverse_possible(ax, {
                    "touch_time": fib_sig_d.get("touch_time"),
                    "touch_price": fib_sig_d.get("touch_price", np.nan),
                    "side": fib_sig_d.get("side", "")
                })
        ...
        if view in ("Intraday", "Both"):
            st.info("Intraday renderer continues in later batches (unchanged).")
...
# ---------------------------
# TAB 8: HMA Buy (NEW — THIS REQUEST)
# ---------------------------
with tab8:
    st.header("HMA Buy")
    st.caption(
        "Lists symbols where **price recently crossed ABOVE HMA(55)** on the **Daily** chart "
        f"(within the last N bars), split into:\n"
        "• **(a) Regression > 0**\n"
        "• **(b) Regression < 0**\n\n"
        "Uses the selected **Daily view range** from the sidebar."
    )

    # Slider requested: 1–3 bars
    max_bars_hma = st.slider("Max bars since price crossed above HMA(55)", 1, 3, 2, 1, key="hma_buy_max_bars")
    run_hma_buy = st.button("Run HMA Buy Scan", key=f"btn_run_hma_buy_{mode}")

    @st.cache_data(ttl=120)
    def last_daily_hma55_cross_up(symbol: str, daily_view_label: str):
        try:
            ...
            if close_show.empty or len(close_show) < 60:
                return None

            hma55 = compute_hma(close_show, period=55)
            hma55 = _coerce_1d_series(hma55).reindex(close_show.index)

            cross_up, _ = _cross_series(close_show, hma55)
            ...
            px_cross = float(close_show.loc[t]) if np.isfinite(close_show.loc[t]) else np.nan
            hma_cross = float(hma55.loc[t]) if (t in hma55.index and np.isfinite(hma55.loc[t])) else np.nan
    ...
    if run_hma_buy:
        ...
        for sym in universe:
            r = last_daily_hma55_cross_up(sym, daily_view_label=daily_view)
            if r is None:
                continue
        ...
        with left:
            st.subheader("(a) Regression > 0 — HMA(55) Cross-Up (recent)")
            if not rows_pos:
                st.info("No matches.")
            else:
        ...
        with right:
            st.subheader("(b) Regression < 0 — HMA(55) Cross-Up (recent)")
            if not rows_neg:
                st.info("No matches.")
            else:
...
with tab9:
    st.header("HMA Sell")
    st.caption(
        "Lists symbols where **price recently crossed BELOW HMA(55)** on the **Daily** chart "
        f"(within the last N bars), split into:\n"
        "• **(a) Regression < 0** (sell with downtrend)\n"
        "• **(b) Regression > 0** (counter-trend sell)\n\n"
        "Uses the selected **Daily view range** from the sidebar."
    )

    max_bars_hma_sell = st.slider(
        "Max bars since price crossed below HMA(55)",
        1, 3, 2, 1,
        key="hma_sell_max_bars"
    )
    run_hma_sell = st.button("Run HMA Sell Scan", key=f"btn_run_hma_sell_{mode}")

    @st.cache_data(ttl=120)
    def last_daily_hma55_cross_down(symbol: str, daily_view_label: str):
        try:
            close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
            if close_full.empty:
                return None
            ...
            # _cross_series returns (cross_up, cross_down)
            _, cross_down = _cross_series(close_show, hma55)
            cross_down = cross_down.reindex(close_show.index, fill_value=False)
            if not cross_down.any():
                return None

            t = cross_down[cross_down].index[-1]
            try:
                loc = int(close_show.index.get_loc(t))
            except Exception:
                return None
    ...
    if run_hma_sell:
        rows_dntrend, rows_ctr = [], []

        for sym in universe:
            r = last_daily_hma55_cross_down(sym, daily_view_label=daily_view)
            if r is None:
                continue
            if int(r.get("Bars Since Cross", 9999)) > int(max_bars_hma_sell):
                continue
        ...
        with left:
            st.subheader("(a) Regression < 0 — HMA(55) Cross-Down (recent)")
            if not rows_dntrend:
                st.info("No matches.")
            else:
                out = pd.DataFrame(rows_dntrend)
                out["Bars Since Cross"] = out["Bars Since Cross"].astype(int)
                out = out.sort_values(["Bars Since Cross", "Regression Slope"], ascending=[True, True])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

        with right:
            st.subheader("(b) Regression > 0 — HMA(55) Cross-Down (recent)")
            if not rows_ctr:
                st.info("No matches.")
            else:
                out = pd.DataFrame(rows_ctr)
                out["Bars Since Cross"] = out["Bars Since Cross"].astype(int)
                out = out.sort_values(["Bars Since Cross", "Regression Slope"], ascending=[True, False])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

# ---------------------------
# TAB 10: Utilities / Diagnostics
# ---------------------------
with tab10:
    st.header("Utilities / Diagnostics")
    ...
    with c1:
        if st.button("Clear st.cache_data", key=f"btn_clear_cache_data_{mode}"):
            st.cache_data.clear()
            st.success("Cleared st.cache_data.")
    with c2:
        if st.button("Clear st.cache_resource", key=f"btn_clear_cache_resource_{mode}"):
            st.cache_resource.clear()
            st.success("Cleared st.cache_resource.")
    with c3:
        if st.button("Rerun app", key=f"btn_rerun_{mode}"):
            st.rerun()

    st.divider()

    st.subheader("Universe export")
    st.caption("Download the current universe (based on mode + your sidebar filters).")
    uni_df = pd.DataFrame({"Symbol": list(universe)})
    st.dataframe(uni_df, use_container_width=True, height=240)

    csv_bytes = uni_df.to_csv(index=False).encode("utf-8")
    st.download_button(
        "Download universe CSV",
        data=csv_bytes,
        file_name=f"universe_{mode.lower()}.csv",
        mime="text/csv",
        key=f"dl_universe_{mode}",
    )

    st.divider()

    st.subheader("Quick data check")
    st.caption("Pulls the most recent daily close available from your data source (via fetch_hist).")

    chk = st.selectbox("Symbol to check:", universe, key=f"diag_symbol_{mode}")
    try:
        s = _coerce_1d_series(fetch_hist(chk)).dropna()
    except Exception:
    ...
    if s.empty:
        ...
        st.write(f"**Last daily bar:** {last_dt}  |  **Close:** {last_px}")
