...
# ---------------------------
# NEW (THIS REQUEST): R² > 45% scanner helpers (Daily + Hourly)
#   - Uses the SAME regression_with_band R² that is displayed on the charts:
#       • Daily: regression_with_band(close, lookback=slope_lb_daily)
#       • Hourly: regression_with_band(intraday_close, lookback=slope_lb_hourly)
# ---------------------------
@st.cache_data(ttl=120)
def last_daily_reg_r2(symbol: str, slope_lb: int):
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if close_full.empty or len(close_full) < 3:
            return np.nan, np.nan, None
        _, _, _, m, r2 = regression_with_band(close_full, lookback=int(slope_lb))
        return float(r2) if np.isfinite(r2) else np.nan, float(m) if np.isfinite(m) else np.nan, close_full.index[-1]
    except Exception:
        return np.nan, np.nan, None

@st.cache_data(ttl=120)
def last_hourly_reg_r2(symbol: str, period: str, slope_lb: int):
    try:
        df = fetch_intraday(symbol, period=period)
        if df is None or df.empty or "Close" not in df.columns:
            return np.nan, np.nan, None
        ...
        ts = None
        if isinstance(real_times, pd.DatetimeIndex) and len(real_times) > 0:
            ts = real_times[-1]
...
# UPDATED (THIS REQUEST): added R² > 45% Scanner tab (Daily + Hourly) without changing existing UI views
...
# ---------------------------
# TAB 16: R² > 45% Scanner (NEW — THIS REQUEST)
# ---------------------------
with tab16:
    st.header("R² > 45% Scanner")
    st.caption(
        "Shows symbols where the **regression R²** (the same R² displayed on the charts) is **> 45%**:\n"
        "• **Daily:** regression_with_band(close, lookback=Daily slope lookback)\n"
        "• **Hourly:** regression_with_band(intraday close, lookback=Hourly slope lookback) using **1d intraday**"
    )

    R2_THRESHOLD = 0.45
    run_r2 = st.button("Run R² > 45% Scan", key=f"btn_run_r2_scan_{mode}")
    ...
    if run_r2:
        ...
        for sym in universe:
            r2d, md, tsd = last_daily_reg_r2(sym, slope_lb=int(slope_lb_daily))
            if np.isfinite(r2d) and float(r2d) > float(R2_THRESHOLD):
                daily_rows.append({
                    "Symbol": sym,
                    "R2": float(r2d),
                    "R2 (%)": float(r2d) * 100.0,
                    "Slope": float(md) if np.isfinite(md) else np.nan,
                    "AsOf": tsd
                })

            r2h, mh, tsh = last_hourly_reg_r2(sym, period="1d", slope_lb=int(slope_lb_hourly))
            if np.isfinite(r2h) and float(r2h) > float(R2_THRESHOLD):
                hourly_rows.append({
                    "Symbol": sym,
                    "R2": float(r2h),
                    "R2 (%)": float(r2h) * 100.0,
                    "Slope": float(mh) if np.isfinite(mh) else np.nan,
                    "AsOf": tsh
                })
        ...
        with right:
            st.subheader("Hourly (1d intraday) — R² > 45%")
            if not hourly_rows:
                st.info("No matches.")
            else:
        ...
        st.info("Click **Run R² > 45% Scan** to scan the current universe.")
