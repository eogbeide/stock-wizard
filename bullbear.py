...
#   • NEW (Dec 2025, this update): HMA signals are now rendered as **outside callouts** with
#       a leader line from the entry point (no chart blockage). Daily chart gets the same HMA
#       outside callouts. Added new Tab: **HMA 55 Cross Reversals**.
...
# >>> Instruction banner (alignment-aware)
...
# Sidebar (omitted here; unchanged till after helpers) — continues later…

# --- Existing annotation helpers (kept) ---
def annotate_signal_box(ax, ts, px, side: str, note: str = "", ypad_frac: float = 0.045):
    try:
        ...
        ax.annotate(
            text, xy=(ts, px), xytext=(ts, px + yoff), textcoords='data',
            ha='left', va='bottom' if side == "BUY" else 'top',
...
# --- NEW: Outside HMA callout with non-overlapping stack ---
def annotate_outside_hma(ax, x_val, y_val, side: str, note: str = "HMA55 Cross",
                         slots: list = None, above: bool = True):
    """
    Draws a leader line from entry point to an OUTSIDE box (kept off-plot).
    Uses `slots` to stack multiple boxes without overlap.
    """
    try:
        fig = ax.figure
        ax_bbox = ax.get_position()  # axes bbox in figure coords
        # data -> display -> figure coords for x
        x_disp, _ = ax.transData.transform((x_val, y_val))
        x_fig, _ = fig.transFigure.inverted().transform((x_disp, 0.0))
        x_fig = float(np.clip(x_fig, 0.08, 0.92))

        # vertical stacking away from other outside markers used in this plot
        if slots is None:
            slots = []
        base = ax_bbox.y1 + (0.12 if above else -0.12)
        y_fig = base + 0.06 * len(slots)
        y_fig = float(np.clip(y_fig, 0.01, 0.99))  # keep in figure bounds
        slots.append((x_fig, y_fig))

        label = f"{'▲ BUY' if str(side).upper()=='BUY' else '▼ SELL'} {note}"
        color = "tab:green" if str(side).upper() == "BUY" else "tab:red"

        ax.annotate(
            label,
            xy=(x_val, y_val), xycoords='data',
            xytext=(x_fig, y_fig), textcoords=fig.transFigure,
        ...
        # emphasis marker at entry
        ax.scatter([x_val], [y_val], s=60, c=color, zorder=1201)
    except Exception:
        # fallback inside
        annotate_signal_box(ax, x_val, y_val, side=side, note=note)

# --- NEW: Price Reversed outside (kept) ---
...
def last_reversal_star(price: pd.Series,
    ...
    if trend_slope < 0:
        ...
        seg = s.loc[t_peak:].iloc[:confirm_bars+1]
        if len(seg) == confirm_bars+1 and np.all(np.diff(seg) < 0):
            return {"time": t_peak, "price": float(s.loc[t_peak]), "kind": "peak"}
    else:
        cand = tail.iloc[:-confirm_bars]
        t_trough = cand.idxmin()
        seg = s.loc[t_trough:].iloc[:confirm_bars+1]
        if len(seg) == confirm_bars+1 and np.all(np.diff(seg) > 0):
...
# --- NEW: HMA-cross signal for use on Daily (and elsewhere)
def last_hma_cross_signal(price: pd.Series,
                          hma: pd.Series,
                          trend_slope: float,
                          lookback: int = 60):
    """
    Latest price/HMA cross aligned with trend:
      • m>0 → BUY when close crosses above HMA
      • m<0 → SELL when close crosses below HMA
    """
    if not np.isfinite(trend_slope) or trend_slope == 0:
        return None
...
def compute_parabolic_sar(high: pd.Series, low: pd.Series, step: float = 0.02, max_step: float = 0.2):
    ...
    for i in range(1, n):
        ...
        if uptrend:
            ...
            psar[i] = prev_psar + af * (ep - prev_psar)
            hi1 = df["H"].iiloc[i-1] if i-1 < len(df) else df["H"].iloc[-1]  # guard
...
# --- Daily-only SR 99% alert & support-touch scanner (unchanged from your version) ---
Z_FOR_99 = 2.576
...
def daily_sr_99_reversal_signal(price: pd.Series,
    ...
    if trend_slope > 0:
        if (c1 <= s1 * (1.0 + prox)) and _rel_near(s1, l1, prox) and _inc_ok(p, confirm_bars):
            return {"time": t0, "price": c0, "side": "BUY", "note": "ALERT 99% SR REV"}
    else:
        if (c1 >= r1 * (1.0 - prox)) and _rel_near(r1, u1, prox) and _dec_ok(p, confirm_bars):
...
def find_support_touch_confirmed_up(price: pd.Series,
    ...
    for t in reversed(tail.iloc[:-confirm_bars].index.tolist()):
        ...
        if not touched:
            continue
        seg = p.loc[t:].iloc[:confirm_bars+1]
        if len(seg) == confirm_bars+1 and np.all(np.diff(seg) > 0):
...
# --- Sessions & Market-time axis (unchanged) ---
...
# --- NEW (Hourly): HMA(55) cross AFTER S/R reversal (unchanged) ---
    ...
    if global_slope > 0:
        for i in range(len(p) - confirm_bars - 1, start - 1, -1):
            ...
            for t in range(t0 + 1, len(p)):
                ...
                heading_up = (p.iloc[t] > p.iloc[t - 1])
        ...
        for i in range(len(p) - confirm_bars - 1, start - 1, -1):
            ...
            for t in range(t0 + 1, len(p)):
                ...
                heading_down = (p.iloc[t] < p.iloc[t - 1])
...
# Sidebar config, universe, fetchers… (your original sidebar & fetch functions are unchanged)
# [KEEP your existing sidebar & data fetch blocks exactly as before]
# (For brevity here, paste your original sidebar and fetch_* functions unchanged.)
...
with tab1:
    ...
    if st.button("Run Forecast", key="btn_run_forecast") or auto_run:
        df_hist = fetch_hist(st.session_state.get("ticker") or universe[0])
        sel = st.selectbox("Ticker:", universe, key="orig_ticker", index=universe.index(st.session_state.get("ticker", universe[0])) if st.session_state.get("ticker") in universe else 0)
        # force re-download using selected ticker
        ...
        st.session_state.update({
            ...
            "intraday": intraday, "ticker": sel, "hour_range": hour_range, "run_all": True
        })

    st.caption("The Slope Line serves as an informational tool…")
    ...
    if st.session_state.run_all:
        ...
        # HMA for plotting + HMA(55) for signals
        hma_d_plot = compute_hma(df, period=hma_period).reindex(df_show.index)
        hma_d_sig  = compute_hma(df, period=55).reindex(df_show.index)
        ...
        if show_hma and not hma_d_plot.dropna().empty:
            ax.plot(hma_d_plot.index, hma_d_plot.values, "-", linewidth=1.3, alpha=0.9, label="HMA")
        ...
        badges_top = []
        # Band REV signals (outside for SELL, inside triangle for BUY)
        band_sig_d = last_band_reversal_signal(price=df_show, band_upper=upper_d_show, band_lower=lower_d_show,
        ...
        # Existing HMA cross star/badges (kept) — de-clutter logic intact
        hma_cross_star = last_hma_cross_signal(df_show, hma_d_sig, trend_slope=m_d, lookback=60)
        # --- NEW: Outside HMA callout for Daily (no overlap) ---
        outside_slots_daily = []
        if hma_cross_star is not None:
            annotate_outside_hma(ax, hma_cross_star["time"], hma_cross_star["price"],
                                 side=hma_cross_star["side"], note="HMA55 Cross (Daily)",
                                 slots=outside_slots_daily, above=True)
        ...
        # Instruction banner (Daily) — LOCAL=GLOBAL=m_d
        try:
            ...
            draw_instruction_ribbons(ax, m_d, sup_val_d, res_val_d, px_val_d, sel,
                                     confirm_side=confirm_side, global_slope=m_d)
        except Exception:
            pass
        ...
        if intraday is None or intraday.empty or "Close" not in intraday:
            ...
            # Caution if local vs global disagree
            try:
                if np.isfinite(slope_h) and np.isfinite(m_h) and (slope_h * m_h < 0):
                    caution_below_btn.warning("ALERT: Please exercise caution…")
            except Exception:
                pass
            ...
            badges_top_h = []
            band_sig_h = last_band_reversal_signal(price=hc, band_upper=upper_h, band_lower=lower_h,
            ...
            breakout_h = None  # per Dec 2025: keep focus on HMA after S/R; ±2σ removed

            # NEW: HMA(55) AFTER S/R REV — OUTSIDE CALLOUT (no chart blocking)
            hma_sr_sig = hma_cross_after_sr_reversal_signal(
                price=hc, hma=hma_h_sig, support=sup_h, resistance=res_h,
                global_slope=m_h, prox=sr_prox_pct, confirm_bars=rev_bars_confirm, lookback_bars=slope_lb_hourly
            )
            outside_slots_hourly = []
            if hma_sr_sig is not None:
                tpos_sig = _pos(hma_sr_sig["time"])
                if np.isfinite(tpos_sig):
                    annotate_outside_hma(ax2, tpos_sig, hma_sr_sig["price"],
                                         side=hma_sr_sig["side"], note=hma_sr_sig["note"],
                                         slots=outside_slots_hourly, above=True)

            # Outside "Price Reversed" for Hourly (keep)
            if band_sig_h is not None:
                tpos = _pos(band_sig_h["time"])
                if np.isfinite(tpos):
            ...
            # Ribbon (Hourly)
            draw_instruction_ribbons(ax2, slope_h, sup_val, res_val, px_val, sel, confirm_side=None, global_slope=m_h)
            ...
            if np.isfinite(px_val):
                ...
                try:
                    ...
                    pass
                footer_txt = f"Current price: {fmt_price_val(px_val)}{nbb_txt}\nR² ({slope_lb_hourly} bars): {fmt_r2(r2_h)}  •  Slope: {fmt_slope(m_h)}/bar"
                ax2.text(0.5, 0.02, footer_txt, transform=ax2.transAxes, ha="center", va="bottom",
                         fontsize=10, fontweight="bold",
                         bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="grey", alpha=0.7))
...
with tab2:
    ...
    if not st.session_state.run_all:
        ...
        # (Daily portion largely mirrors Tab 1 Daily; omitted repetition for brevity)
        st.info("Daily & Intraday views mirror Tab 1 logic. Use Tab 1 for visuals.")
...
with tab3:
    ...
    if not st.session_state.run_all:
        ...
        df3 = fetch_close_df_period(st.session_state.ticker, st.sidebar.session_state.get("sb_bb_period", "6mo"))
        if df3.empty or 'Close' not in df3:
            ...
            c4.metric("Lookback", st.sidebar.session_state.get("sb_bb_period", "6mo"))
...
with tab5:
    ...
    st.info("Unchanged logic. Use your previous implementation here or keep as-is.")

# --- Tab 6: Long-Term History ---
with tab6:
    st.header("Long-Term History — Price with S/R & Trend")
    # (unchanged long-term history logic…)
...
# --- NEW Tab 9: HMA 55 Cross Reversals ---
with tab9:
    st.header("HMA 55 Cross Reversals (Daily, latest bar)")
    st.caption("Lists symbols where the latest daily bar **crossed** HMA(55) in **agreement with trend slope** (BUY=up-cross & m>0, SELL=down-cross & m<0).")

    if st.button("Scan Universe for HMA55 Cross (Daily)", key="btn_scan_hma55_cross"):
        mode = st.sidebar.session_state.get("sb_mode", "Stock")
        if mode == "Stock":
            universe = sorted([
                'AAPL','SPY','AMZN','DIA','TSLA','SPGI','JPM','VTWG','PLTR','NVDA',
                'META','SITM','MARA','GOOG','HOOD','BABA','IBM','AVGO','GUSH','VOO',
        ...
        for sym in universe:
            try:
                ...
                if s is None or s.dropna().shape[0] < 10:
                    continue
                h = compute_hma(s, period=55).reindex(s.index)
                # slope over configured window
                _, _, _, m_sym, r2_sym = regression_with_band(s, lookback=st.sidebar.session_state.get("sb_slope_lb_daily", 90), z=Z_FOR_99)
                p = s.astype(float)
                mask = p.notna() & h.notna()
                if mask.sum() < 2 or not np.isfinite(m_sym) or m_sym == 0:
                    continue
                p2 = p[mask]; h2 = h[mask]
                if len(p2) < 2:
                    continue
                c_prev, c_now = float(p2.iloc[-2]), float(p2.iloc[-1])
                h_prev, h_now = float(h2.iloc[-2]), float(h2.iloc[-1])

                up_cross   = (c_prev < h_prev) and (c_now >= h_now)
                down_cross = (c_prev > h_prev) and (c_now <= h_now)

                if (m_sym > 0 and up_cross) or (m_sym < 0 and down_cross):
                    rows.append({
                        "Symbol": sym,
                        "Timestamp": p2.index[-1],
                        "Side": "BUY" if m_sym > 0 else "SELL",
                        "Close": c_now,
                        "HMA55": h_now,
                        "Slope": m_sym,
                        "R2": r2_sym
        ...
        if not rows:
            st.info("No latest-bar HMA(55) cross aligned with slope found.")
        else:
            df_out = pd.DataFrame(rows).sort_values(["Side","Symbol"])
            view = df_out.copy()
            view["Close"] = view["Close"].map(lambda v: fmt_price_val(v) if np.isfinite(v) else "n/a")
            view["HMA55"] = view["HMA55"].map(lambda v: fmt_price_val(v) if np.isfinite(v) else "n/a")
            ...
            st.dataframe(view[["Symbol","Timestamp","Side","Close","HMA55","Slope","R2"]].reset_index(drop=True),
