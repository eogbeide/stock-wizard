...
with tab1:
    ...
    if st.session_state.run_all and st.session_state.ticker == sel:
        df, intr = st.session_state.df_hist, st.session_state.intraday
        ...
        if chart in ("Hourly","Both"):
            ...
            try:
                ...
                base = 1.0
            slope_pct = float(coeff_h[0] * (len(hc)-1) / base * 100)
            ...
            ax2.set_title(
                ...
                f"↑{p_up*100:.1f}%  ↓{p_dn*100:.1f}%  Trend: {slope_pct:.2f}%"
            )
        ...
        if chart in ("Daily","Both"):
            ...
            ax0.set_title(
                f"{sel} Daily  ↑{p_up*100:.1f}%  ↓{p_dn*100:.1f}%  Trend: {tlb}"
            )
...
# Tabs 2–4 remain unchanged (Enhanced Forecast; Bull vs Bear; Metrics).
