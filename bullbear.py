...
def _reset_run_state_for_mode_switch():
    ...
    for k in list(st.session_state.keys()):
        # keep asset_mode + sidebar settings; clear tab controls safely
        if k.startswith("tab1_") or k.startswith("tab2_"):
            pass
...
def hour_range_to_yf_period(hour_range: str) -> str:
    # UI labels → yfinance period string
    m = {
        "24h": "1d",
        "48h": "2d",
        "72h": "3d",
...
def fetch_hist_ohlc(ticker: str) -> pd.DataFrame:
    df = yf.download(ticker, period="max")[["Open","High","Low","Close"]].dropna()
    try:
...
def annotate_fib_extreme_warning(ax, close: pd.Series, fibs: dict, r2_val: float,
                                min_r2: float = 0.99, tol_frac: float = 0.0015, lookback_bars: int = 5):
    try:
        r2 = float(r2_val)
    except Exception:
        return
...
def slope_reversal_probability(series_like, current_slope: float,
                               hist_window: int = 240, slope_window: int = 60, horizon: int = 15) -> float:
    s = _coerce_1d_series(series_like).dropna()
    n = len(s)
...
# ---------------------------
# Stars (HMA cross + NPX ±0.5 within ±2 bars)
# ---------------------------
...
# ---------------------------
# Slope trigger after band reversal (leaderline)
# ---------------------------
def find_slope_trigger_after_band_reversal(price: pd.Series,
    ...
    if last_buy_cross is not None:
        ...
        if t_touch is not None:
            buy_tr = {"side": "BUY", "touch_time": t_touch, "touch_price": float(p.loc[t_touch]),
                      "cross_time": last_buy_cross, "cross_price": float(p.loc[last_buy_cross])}

    sell_tr = None
    if last_sell_cross is not None:
        t_touch = _last_touch_before(last_sell_cross, above)
        if t_touch is not None:
            sell_tr = {"side": "SELL", "touch_time": t_touch, "touch_price": float(p.loc[t_touch]),
                       "cross_time": last_sell_cross, "cross_price": float(p.loc[last_sell_cross])}
...
def annotate_slope_trigger(ax, trig: dict):
    if trig is None:
        return
    side = trig.get("side", "")
    t0 = trig.get("touch_time"); p0 = trig.get("touch_price")
    t1 = trig.get("cross_time"); p1 = trig.get("cross_price")
    ...
    ax.annotate("", xy=(t1, p1), xytext=(t0, p0),
...
def render_daily(sel: str, show_forecast: bool = True):
    ...
    close = _coerce_1d_series(ohlc_show["Close"]).dropna()
    if close.empty:
    ...
    # Plot layout: Price + optional NTD panel
    if show_ntd:
        fig, (ax, ax_ntd) = plt.subplots(2, 1, sharex=True, figsize=(14, 7),
    ...
    # S/R
    res_val = float(res.iloc[-1]); sup_val = float(sup.iloc[-1]); px_val = float(close.iloc[-1])
    ...
    try:
        ...
        pass
    ax.text(0.99, 0.02, f"Current price: {fmt_price_val(px_val)}{nbb_txt}",
    ...
    # Legend outside (like your original UI request)
...
def render_hourly(sel: str, hour_range_label: str):
    ...
    # keep real times for sessions; use range index for gapless plotting spacing
    ...
    # S/R
    res_val = float(res.iloc[-1]); sup_val = float(sup.iloc[-1]); px_val = float(hc.iloc[-1])
    ...
    # Sessions (Forex) drawn using real_times mapped to nearest bar index
    ...
    if mode == "Forex" and show_sessions_pst and isinstance(real_times, pd.DatetimeIndex) and not real_times.empty:
        ...
        def _map_to_pos(times):
            ...
            return [int(i) for i in idxer if int(i) >= 0 and int(i) < len(hc)]
    ...
    # Sessions legend bottom (outside), like your older layout
    if session_handles and session_labels:
        fig.legend(handles=session_handles, labels=session_labels,
                   loc="lower center", bbox_to_anchor=(0.5, 0.01),
                   ncol=2, frameon=True, fontsize=9,
                   title="Sessions (PST)", title_fontsize=9)
...
def scan_star_daily(symbol: str, max_gap: int = 2):
    try:
        ...
        if last_sell is None:
            t = last_buy; side = "BLUE ★ (Buy)"
        elif last_buy is None:
            t = last_sell; side = "PURPLE ★ (Sell)"
        else:
        ...
        return {
            "Symbol": symbol, "Side": side, "Bars Since": bars_since,
            "Signal Time": t, "Signal Price": float(close.loc[t]),
...
def scan_recent_npx0_up(symbol: str, max_bars: int = 2):
    try:
        ...
        if bars_since > int(max_bars):
            return None
        return {
            "Symbol": symbol, "Bars Since": bars_since,
            "Cross Time": t, "Current Price": float(close.iloc[-1]),
...
def scan_daily_sr_reversal_bbmid(symbol: str, want_side: str):
    """
    Buy:  slope>0, touch/breach LOWER 2σ, then cross above slope line, and price above BB midline, R²>=threshold
    Sell: slope<0, touch/breach UPPER 2σ, then cross below slope line, and price below BB midline, R²>=threshold
    """
    try:
        ohlc = fetch_hist_ohlc(symbol)
        if ohlc is None or ohlc.empty:
...
def original_run_controls(prefix: str, default_symbol: str):
    ...
    sel = st.selectbox("Ticker:", universe, index=universe.index(default_symbol), key=f"{prefix}_ticker")
...
# ---------------------------
# Tab 1 — Original Forecast (restored original interface)
# ---------------------------
with tab1:
...
# ---------------------------
# Tab 2 — Enhanced Forecast (same original interface controls)
# ---------------------------
with tab2:
    ...
    if not params or params.get("mode") != mode:
        ...
        # Enhanced = charts without forecast + optional MACD emphasis
        if run_view in ("Daily", "Both"):
...
with tab3:
    ...
    s = fetch_hist_close(sym)
    if s is None or s.empty:
        st.warning("No data.")
    else:
        df = s.to_frame("Close").dropna()
        if bb_period != "max":
            df2 = yf.download(sym, period=bb_period)[["Close"]].dropna()
            if not df2.empty:
                df = df2
        chg = df["Close"].diff()
        bull = int((chg > 0).sum())
        ...
        st.metric("Bull days", bull)
        st.metric("Bear days", bear)
        st.metric("Bull %", f"{(bull/total)*100:.1f}%")
...
with tab4:
    ...
    if s is None or s.empty:
        ...
        s = subset_by_daily_view(s, daily_view).dropna()
    ...
    if run_scan:
        ...
        for sym in universe:
            try:
                ...
                if ohlc is None or ohlc.empty:
                    continue
                close = _coerce_1d_series(subset_by_daily_view(ohlc, daily_view)["Close"]).dropna()
    ...
    if run_scan:
        ...
        for sym in universe:
            try:
                ...
                candidates = []
                if up_m05.any(): candidates.append(("NPX -0.5 ↑", up_m05[up_m05].index[-1]))
                if dn_m05.any(): candidates.append(("NPX -0.5 ↓", dn_m05[dn_m05].index[-1]))
                if up_p05.any(): candidates.append(("NPX +0.5 ↑", up_p05[up_p05].index[-1]))
                if dn_p05.any(): candidates.append(("NPX +0.5 ↓", dn_p05[dn_p05].index[-1]))
                if not candidates:
                    continue
                ...
                if bars_since > int(max_bars):
                    continue
                rows.append({"Symbol": sym, "Signal": sig, "Bars Since": bars_since,
                             "Time": t, "Price": float(close.iloc[-1]), "NPX (last)": float(npx.dropna().iloc[-1])})
            except Exception:
                continue
        if not rows:
            st.info("No matches.")
...
# ---------------------------
# Tab 9 — Daily Slope+BB Reversal Scanner (99% confidence via R²)
# ---------------------------
with tab9:
