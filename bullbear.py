...
if st.button("Forecast"):
    ...
    try:
        ...
        if data.empty:
            st.error("No data found for the selected ticker. Please choose a different ticker.")
            st.stop()
    ...
    try:
        ...
        # Check for matrix singularity before inverting
        try:
            # Example matrix (replace this with your actual data matrix if needed)
            matrix = np.corrcoef(prices)  # Example: correlation matrix of prices

            if np.linalg.det(matrix) == 0:
                st.error("The correlation matrix is singular and cannot be inverted. Using regularization.")
                epsilon = 1e-10  # Small value for regularization
                inverted_matrix = np.linalg.inv(matrix + epsilon * np.eye(matrix.shape[0]))
            else:
                inverted_matrix = np.linalg.inv(matrix)

            st.write("Inverted Matrix:", inverted_matrix)

        except Exception as e:
            st.error(f"An error occurred during matrix inversion: {e}")
