...
# =========================
# Batch 2/3 — Parts 5/10 to 8/10 (UPDATED per request)
#   • Adds NEW helper: last_daily_kijun_cross_down(...)
#   • Updates last_daily_kijun_cross_up(...) to enforce current Price > current Kijun
#   • Both helpers support "within last N bars" filters (sliders will be wired in Batch 3)
# =========================
...
#   UPDATED (THIS REQUEST):
#     - Enforce current price > current kijun for the "upward cross" meaning price line value > Ichimoku line value
# ---------------------------
@st.cache_data(ttl=120)
def last_daily_kijun_cross_up(symbol: str,
    ...
    try:
        ...
        # Enforce "price line value > Ichimoku line value" CURRENTLY
        px_last = float(close_show.iloc[-1]) if np.isfinite(close_show.iloc[-1]) else np.nan
        kij_last = float(kijun.iloc[-1]) if np.isfinite(kijun.iloc[-1]) else np.nan
        if not (np.isfinite(px_last) and np.isfinite(kij_last) and (px_last > kij_last)):
            return None
        ...
        # Heading up confirmation (existing behavior)
...
# ---------------------------
# NEW (THIS REQUEST): Ichimoku Kijun Daily Cross-DOWN helper (Daily only / matches Price Chart)
#   - For "downward cross" meaning price line value < Ichimoku line value (CURRENTLY)
#   - Same output shape as cross_up
# ---------------------------
@st.cache_data(ttl=120)
def last_daily_kijun_cross_down(symbol: str,
                                daily_view_label: str,
                                slope_lb: int,
                                conv: int,
                                base: int,
    ...
    try:
        ...
        _, cross_dn = _cross_series(close_show, kijun)
        ...
        # Heading down confirmation: down-close on cross bar AND current close <= cross close
        px_cross = float(close_show.loc[t_cross]) if np.isfinite(close_show.loc[t_cross]) else np.nan
        px_prev = float(close_show.shift(1).loc[t_cross]) if (t_cross in close_show.index and np.isfinite(close_show.shift(1).loc[t_cross])) else np.nan
        heading_down = (np.isfinite(px_cross) and np.isfinite(px_prev) and (px_cross < px_prev) and np.isfinite(px_last) and (px_last <= px_cross))
        if not heading_down:
            return None
...
# =========================
# Batch 3/3 — Parts 9/10 to 10/10
#   • Adds TWO new sliders for Kijun cross scanners (UP within N, DOWN within N)
#   • Wires them into Tab 15 (UP and DOWN tables) WITHOUT changing tab count/layout
#   • Wires Tab 17 (Daily/Hourly R² scanners) + Tab 18 (Daily R² + ±2σ proximity) if those tabs already exist
# =========================
...
# ----------------------------------------
# (A) SIDEBAR — ADD THESE TWO SLIDERS
# Place near your other "Scanner" controls
# ----------------------------------------
# (keep existing controls unchanged; just add these)
kijun_up_within_n = st.sidebar.slider(
    "Kijun Cross UP — within last N bars (Daily)",
    min_value=0, max_value=60, value=5, step=1,
    help="For Tab 15 bullish: last Daily Price↑Kijun cross must be within this many bars AND current price > current Kijun."
)
kijun_down_within_n = st.sidebar.slider(
    "Kijun Cross DOWN — within last N bars (Daily)",
    min_value=0, max_value=60, value=5, step=1,
    help="For Tab 15 bearish: last Daily Price↓Kijun cross must be within this many bars AND current price < current Kijun."
)

# ----------------------------------------
# (B) Small helper for scanners (if you don't already have one)
# ----------------------------------------
def _scan_symbols(symbols, fn, *args, **kwargs) -> pd.DataFrame:
    rows = []
    for sym in symbols:
        try:
            out = fn(sym, *args, **kwargs)
            if isinstance(out, dict) and len(out):
                rows.append(out)
        except Exception:
            continue
    df = pd.DataFrame(rows)
...
# ----------------------------------------
# TAB 15 — REPLACE ONLY THE CONTENT INSIDE TAB 15
# (Keep your tabs list, tab ordering, and everything else unchanged)
# ----------------------------------------
# Example assumes you already have:
#   tabs = st.tabs([... 18 labels ...])
# and tab15 is accessible as tabs[14] or similar.
#
# >>> Replace the existing body of Tab 15 with the block below <<<

with tab15:  # <-- keep your actual tab variable name
    st.subheader("Ichimoku Kijun Cross (Daily) — Recent Cross + Regression Fit")

    # Keep your existing selectors (e.g., universe list, daily_view_label, slope_lb_daily, ichi_conv/base/spanb)
    # This code assumes these already exist in your app:
    #   symbols_to_scan, daily_view_label, slope_lb_daily, ichi_conv, ichi_base, ichi_spanb

    cA, cB = st.columns(2)

    with cA:
        st.markdown("### Bullish: Price ↑ Kijun (recent) + slope > 0")
        df_up = _scan_symbols(
            symbols_to_scan,
            ...
            within_last_n_bars=int(kijun_up_within_n),
        )

        if df_up.empty:
            st.info("No bullish Kijun cross-up matches found.")
        else:
            # Enforce slope > 0 (Tab header condition)
            df_up = df_up[pd.to_numeric(df_up.get("Slope", pd.Series(dtype=float)), errors="coerce") > 0].copy()
            if df_up.empty:
                st.info("Matches found but none with positive slope.")
            else:
                # Nice sorting: most recent first, then higher R²
                if "Bars Since Cross" in df_up.columns:
                    df_up = df_up.sort_values(["Bars Since Cross", "R2"], ascending=[True, False], na_position="last")
                st.dataframe(df_up, use_container_width=True)

    with cB:
        st.markdown("### Bearish: Price ↓ Kijun (recent) + slope < 0")
        df_dn = _scan_symbols(
            symbols_to_scan,
            last_daily_kijun_cross_down,
            daily_view_label=daily_view_label,
            slope_lb=int(slope_lb_daily),
            conv=int(ichi_conv),
            base=int(ichi_base),
            span_b=int(ichi_spanb),
            within_last_n_bars=int(kijun_down_within_n),
        )

        if df_dn.empty:
            st.info("No bearish Kijun cross-down matches found.")
        else:
            # Enforce slope < 0 (Tab header condition)
            df_dn = df_dn[pd.to_numeric(df_dn.get("Slope", pd.Series(dtype=float)), errors="coerce") < 0].copy()
            if df_dn.empty:
                st.info("Matches found but none with negative slope.")
            else:
                if "Bars Since Cross" in df_dn.columns:
                    df_dn = df_dn.sort_values(["Bars Since Cross", "R2"], ascending=[True, False], na_position="last")
                st.dataframe(df_dn, use_container_width=True)

    st.caption(
        "Notes: Cross-UP requires current Close > current Kijun. Cross-DOWN requires current Close < current Kijun. "
        "Both are filtered by 'within last N bars' sliders in the sidebar."
    )


# ----------------------------------------
# OPTIONAL: If your app already has Tab 17 and Tab 18 (as per earlier additions),
# wire the new R² helpers into them.
# If those tabs already had content, only replace their inner logic similarly.
# ----------------------------------------

with tab17:  # keep your actual tab variable name
    st.subheader("Regression Fit (R²) — Daily & Hourly Scanners")

    c1, c2 = st.columns(2)

    with c1:
        st.markdown("### Daily R² (Regression Band)")
        df_r2_d = []
        for sym in symbols_to_scan:
            r2, m, ts = daily_regression_r2(sym, slope_lb=int(slope_lb_daily))
            if np.isfinite(r2):
                df_r2_d.append({"Symbol": sym, "R2": float(r2), "Slope": float(m) if np.isfinite(m) else np.nan, "AsOf": ts})
        df_r2_d = pd.DataFrame(df_r2_d)
        if df_r2_d.empty:
            st.info("No daily R² results.")
        else:
            df_r2_d = df_r2_d.sort_values(["R2"], ascending=False, na_position="last")
            st.dataframe(df_r2_d, use_container_width=True)

    with c2:
        st.markdown("### Hourly R² (Regression Band)")
        # assumes you already have hour_range -> intraday period string mapping, like period_intraday
        df_r2_h = []
        for sym in symbols_to_scan:
            r2, m, ts = hourly_regression_r2(sym, period=str(period_intraday), slope_lb=int(slope_lb_hourly))
            if np.isfinite(r2):
                df_r2_h.append({"Symbol": sym, "R2": float(r2), "Slope": float(m) if np.isfinite(m) else np.nan, "AsOf": ts})
        df_r2_h = pd.DataFrame(df_r2_h)
        if df_r2_h.empty:
            st.info("No hourly R² results.")
        else:
            df_r2_h = df_r2_h.sort_values(["R2"], ascending=False, na_position="last")
            st.dataframe(df_r2_h, use_container_width=True)

    st.caption("Higher R² generally means price is tracking the regression channel more tightly over the lookback window.")


with tab18:  # keep your actual tab variable name
    st.subheader("Daily: R² + ±2σ Band Proximity (Near Upper/Lower)")

    # Uses existing sr_prox_pct as proximity threshold (fraction), as requested earlier.
    # If you want a dedicated slider, add it — but this keeps your UI unchanged.

    rows = []
    for sym in symbols_to_scan:
        d = daily_r2_band_proximity(
            sym,
            daily_view_label=daily_view_label,
            slope_lb=int(slope_lb_daily),
    ...
    df = pd.DataFrame(rows)
    if df.empty:
        st.info("No Daily R² band proximity results.")
    else:
        # Show "near" first, then stronger fits
        if "R2" in df.columns:
            df["R2"] = pd.to_numeric(df["R2"], errors="coerce")
        df["Near Any"] = df.get("Near Lower", False).astype(bool) | df.get("Near Upper", False).astype(bool)
        df = df.sort_values(["Near Any", "R2"], ascending=[False, False], na_position="last")
        st.dataframe(df, use_container_width=True)

    st.caption("Near Lower/Upper uses sr_prox_pct (fractional distance) to the latest ±2σ regression band.")
