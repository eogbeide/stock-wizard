...
with tab1:
    ...
    if mode == "Stock":
        ticker = st.selectbox("Select Stock Ticker:", sorted([
        ...
        pair = st.selectbox("Select Forex Pair:", [
        ...
        if st.button("Run Forex Forecast", key="orig_forex_btn"):
            ...
            model = safe_sarimax(df,(1,1,1),(1,1,1,12))
            fc    = model.get_forecast(steps=30)
            idx, vals, ci = pd.date_range(df.index[-1]+timedelta(1),periods=30,freq='D'), fc.predicted_mean, fc.conf_int()
...
with tab2:
    ...
    if mode == "Stock":
        ...
        if st.button("Run Enhanced Stock Forecast", key="enh_stock_btn"):
            ...
            fc    = model.get_forecast(steps=30)
            idx, vals, ci = pd.date_range(daily.index[-1]+timedelta(1),periods=30,freq='D'), fc.predicted_mean, fc.conf_int()
        ...
        if st.button("Run Enhanced Forex Forecast", key="enh_forex_btn"):
            ...
            if view in ('Daily','Both'):
                ...
                ax.plot(idx, vals, label='Forecast')
                ax.fill_between(idx, ci.iloc[:,0], ci.iloc[:,1], alpha=0.3)
                ax.plot(lb[-360:], '--', label='Lower BB'); ax.plot(ub[-360:], '--', label='Upper BB')
                high, low = daily[-360:].max(), daily[-360:].min(); diff=high-low
                for lev in (0.236,0.382,0.5,0.618):
                    ax.hlines(high-diff*lev, daily.index[-360], daily.index[-1], linestyles='dotted')
                ax.set_title(f"{pair} Daily + Fib"); ax.legend(loc='lower left', framealpha=0.5)
                st.pyplot(fig)

                fig2, ax2 = plt.subplots(figsize=(14,3))
                ax2.plot(ri[-360:], label='RSI(14)'); ax2.axhline(70, linestyle='--'); ax2.axhline(30, linestyle='--')
            ...
            if view in ('Intraday','Both'):
                ...
                if intraday.empty:
                    ...
                    ax3.plot(ic,label='Intraday'); ax3.plot(ie,'--',label='20 EMA')
                    ax3.plot(lb2,'--',label='Lower BB'); ax3.plot(ub2,'--',label='Upper BB')
                    ax3.set_title(f"{pair} Intraday + Fib"); ax3.legend(loc='lower left', framealpha=0.5)
                    st.pyplot(fig3)
...
with tab3:
    ...
    nigeria_mode = st.radio("Choose Nigeria App:", ["Stock Forecast","Forex Forecast"], key="nigeria_mode")
        ...
        if st.button("Run Nigeria FX Forecast", key="nig_fx_btn"):
            ...
            if chart in ('Hourly','Both'):
                ...
                if intraday.empty:
                    ...
                    hc = intraday['Close'].ffill(); he = hc.ewm(span=20).mean()
                    fig2, ax2 = plt.subplots(figsize=(14,5))
                    ax2.plot(hc,label='Intraday'); ax2.plot(he,'--',label='20 EMA')
                    ax2.set_title(f"{sel} Nigeria Intraday (5m)"); ax2.legend(loc='lower left', framealpha=0.5)
                    st.pyplot(fig2)
