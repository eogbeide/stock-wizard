...
def session_times_pst_for_day(day: pd.Timestamp):
    ...
    # London: 00:00–09:00 PST (varies with DST)
    # New York: 06:00–15:00 PST (varies with DST)
...
def plot_daily_panels(ticker: str, df_ohlc: pd.DataFrame):
    ...
    ax1.set_title(f"{ticker} — Daily Price")
    ax1.plot(close.index, close.values, "-", linewidth=1.6, label="Close")

    # Global trendline + band
    if not yhat_g.dropna().empty:
        ax1.plot(yhat_g.index, yhat_g.values, "--", linewidth=2.0, alpha=0.9, label=f"Global Trend (R² {fmt_r2(r2_g)})")
        ax1.plot(up_g.index, up_g.values, "-", linewidth=1.0, alpha=0.35, label="Global +2σ")
        ax1.plot(lo_g.index, lo_g.values, "-", linewidth=1.0, alpha=0.35, label="Global -2σ")
        label_on_left(ax1, float(yhat_g.dropna().iloc[-1]), f"Global Slope: {fmt_slope(m_g)}")

    # Local trendline + band
    if not yhat_l.dropna().empty:
        ax1.plot(yhat_l.index, yhat_l.values, "--", linewidth=2.2, alpha=0.8, label=f"Local Trend (R² {fmt_r2(r2_l)})")
        ax1.plot(up_l.index, up_l.values, "-", linewidth=1.0, alpha=0.30, label="Local +2σ")
        ax1.plot(lo_l.index, lo_l.values, "-", linewidth=1.0, alpha=0.30, label="Local -2σ")
    ...
    if show_ntd:
        ...
        ax2.set_title(f"{ticker} — Daily NTD (Normalized Trend)")
...
def plot_intraday_panels(ticker: str, df_intra: pd.DataFrame):
    ...
    axp.set_title(f"{ticker} — Intraday (5m, gapless x-axis)")
    axp.plot(x, close.values, "-", linewidth=1.4, label="Close")

    # Plot regression trend + ±2σ (position based)
    if not yhat_b.dropna().empty:
        axp.plot(yhat_b.index, yhat_b.values, "--", linewidth=2.0, alpha=0.85, label=f"Trend (R² {fmt_r2(r2_b)})")
        axp.plot(up_b.index, up_b.values, "-", linewidth=1.0, alpha=0.28, label="+2σ")
        axp.plot(lo_b.index, lo_b.values, "-", linewidth=1.0, alpha=0.28, label="-2σ")
    ...
    axi.set_title(f"{ticker} — Hourly NTD / Supertrend / PSAR")
    ...
    if show_macd:
        ...
        axm.set_title(f"{ticker} — Hourly MACD")
    ...
    if show_mom_hourly:
        ...
        axq.set_title(f"{ticker} — Hourly Momentum (ROC %)")
...
# =========================
# ---------------------------
# Main UI — select ticker, run, and render
# ---------------------------
...
if st.session_state.get("run_all", False):
    try:
        ...
        axf.set_title(f"{ticker} — 30-Day Forecast (SARIMAX)")
        ...
        if np.all(np.isfinite([f0, fN])) and f0 != 0:
            st.caption(f"Forecast drift (day 1 → day 30): **{((fN/f0)-1.0)*100:.2f}%**")
    except Exception as e:
