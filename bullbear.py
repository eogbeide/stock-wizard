...
# (2) Remove forex news (UI + markers). Keep sessions control unchanged.
if mode == "Forex":
    show_fx_news = False
    news_window_days = 7
...
# =========================
# (unchanged in this batch – continues in Batch 2/3)
# ---------------------------

# ... (CONTENT CONTINUES EXACTLY AS PROVIDED THROUGH Part 8 + Tabs list) ...
...
# Add NEW tab (19th) at the end; keep original 18 tab labels unchanged.
...
# ============================================================
# BATCH 3
# - Update TAB LIST to include new "NTD Buy Signal" + final tabs
# - Add TAB 9: NTD Buy Signal (Daily)
# - Add TAB 10: News (Stocks only)  ✅ Forex news removed/disabled
# - Add TAB 11: Help / Notes (final tab)
# ============================================================

# ---------------------------
# (REPLACE) TAB DECLARATION
# ---------------------------
# Find your existing st.tabs([...]) and replace it with this block.
tab_labels = [
    "Dashboard",
    "Hourly",
    "Daily",
...
# ---------------------------
# Helpers (Daily NTD buy signal + News)
# ---------------------------
DAILY_VIEW_TO_PERIOD = {
    "1M": "2mo",
    "3M": "4mo",
    "6M": "7mo",
    "1Y": "13mo",
...
def fetch_daily_close_for_view(symbol: str, daily_view_label: str) -> pd.Series:
    period = DAILY_VIEW_TO_PERIOD.get(str(daily_view_label).upper(), "7mo")
    df = yf.download(symbol, period=period, interval="1d", auto_adjust=True, progress=False)
    if df is None or df.empty or "Close" not in df.columns:
        return pd.Series(dtype=float)
    s = df["Close"].dropna().copy()
    # normalize tz-naive index (yfinance gives tz-naive DatetimeIndex for daily)
    try:
...
def compute_npx_from_close(close: pd.Series, window: int) -> pd.Series:
    # NPX in [0, 1] using rolling min/max
...
def compute_ntd_raw_from_npx(npx: pd.Series) -> pd.Series:
    # NTD in [-1, 1] (matches the -0.75 scanner concept)
    # If your earlier code defines NTD differently, keep this tab self-contained.
    if npx is None or npx.dropna().empty:
        return pd.Series(dtype=float)
    return (2.0 * npx - 1.0).clip(-1, 1)

def global_slope(series: pd.Series) -> float:
    # Simple global slope of series over the visible range
    if series is None:
        return np.nan
    s = series.dropna()
    if len(s) < 3:
...
def last_cross_through_threshold(
    s: pd.Series,
    threshold: float,
    direction: str = "up",
) -> tuple[int | None, pd.Timestamp | None]:
    """
    Returns (bars_since_cross, timestamp_of_cross) for the LAST crossing.
    direction="up": prev <= thr and curr > thr
    direction="down": prev >= thr and curr < thr
    """
    if s is None:
        return None, None
    ss = s.dropna()
    ...
    prev = ss.shift(1)
    if direction == "up":
        mask = (prev <= threshold) & (ss > threshold)
    else:
        mask = (prev >= threshold) & (ss < threshold)
    ...
    cross_ts = ss.index[mask].max()
    idx = ss.index.get_loc(cross_ts)
    bars_since = (len(ss) - 1) - idx
    return int(bars_since), cross_ts
...
def last_daily_ntd_buy_signal(
    ...
    close = fetch_daily_close_for_view(symbol, daily_view_label)
    if close.empty:
        return None

    npx = compute_npx_from_close(close, ntd_win)
    ntd = compute_ntd_raw_from_npx(npx)

    bars_since, cross_ts = last_cross_through_threshold(ntd, threshold, direction="up")
    if bars_since is None or cross_ts is None:
        return None
    if int(bars_since) > int(max_bars):
        return None

    m = global_slope(close)
    if require_uptrend and (not np.isfinite(m) or m <= 0):
        return None

    latest_ntd = float(ntd.dropna().iloc[-1]) if not ntd.dropna().empty else np.nan
    latest_close = float(close.dropna().iloc[-1]) if not close.dropna().empty else np.nan

    return {
        "Symbol": symbol,
        "Bars Since": int(bars_since),
        "Cross Date": pd.to_datetime(cross_ts).date() if cross_ts is not None else None,
        "NTD @ Cross": float(ntd.loc[cross_ts]) if cross_ts in ntd.index else np.nan,
        "NTD Latest": latest_ntd,
        "Close Latest": latest_close,
        "Global Slope": float(m) if np.isfinite(m) else np.nan,
    }

@st.cache_data(ttl=60 * 30, show_spinner=False)
def fetch_stock_news(symbol: str, limit: int = 20) -> list[dict]:
    """
    Uses yfinance's Ticker.news for stocks. Forex news removed/disabled by design.
    """
    try:
        t = yf.Ticker(symbol)
        ...
        for it in items[: max(1, int(limit))]:
            ...
            if ts is not None:
                try:
                    ...
                    dt = None
            out.append({"title": title, "link": link, "publisher": pub, "time": dt})
        return out
    except Exception:
        return []

# ============================================================
# TAB 9: NTD BUY SIGNAL (Daily)
# ============================================================
with tab9:
    st.header("NTD Buy Signal — Daily")
    st.caption(
        "Buy signal = **NTD crosses UP through a threshold** (default **-0.75**) within the last N bars. "
        "Optionally requires the **daily global trend (close) slope** to be positive."
    )

    c1, c2, c3, c4 = st.columns([1, 1, 1, 1])
    thr = c1.slider("NTD threshold", -1.00, 0.00, -0.75, 0.05, key="ntd_buy_thr")
    max_bars_sig = c2.slider("Max bars since cross", 0, 20, 3, 1, key="ntd_buy_max_bars")
    req_up = c3.checkbox("Require UP global trend", True, key="ntd_buy_req_up")
    show_chart = c4.checkbox("Show quick chart for selected", False, key="ntd_buy_show_chart")

    run_ntd_buy = st.button("Run NTD Buy Signal Scan", key="btn_run_ntd_buy_scan")
    ...
    if run_ntd_buy:
        ...
        for sym in universe:
            r = last_daily_ntd_buy_signal(
                ...
                threshold=float(thr),
                max_bars=int(max_bars_sig),
                require_uptrend=bool(req_up),
            )
            if r is not None:
                rows.append(r)
        ...
        if not rows:
            ...
            out["Global Slope"] = pd.to_numeric(out["Global Slope"], errors="coerce")
            out = out.sort_values(["Bars Since", "Global Slope"], ascending=[True, False])
            st.dataframe(out.reset_index(drop=True), use_container_width=True)

            if show_chart:
                pick = st.selectbox("Preview symbol:", out["Symbol"].tolist(), key="ntd_buy_preview_symbol")
                close = fetch_daily_close_for_view(pick, daily_view)
                npx = compute_npx_from_close(close, ntd_window)
                ntd = compute_ntd_raw_from_npx(npx)

                fig, ax = plt.subplots(figsize=(14, 4))
                ax.set_title(f"{pick} — Daily NTD Buy Signal Preview")
                ax.plot(ntd.index, ntd.values, label="NTD (raw)")
                ax.axhline(float(thr), linestyle="--", label=f"Threshold {thr:.2f}")
...
# ============================================================
# TAB 10: NEWS (Stocks only) — Forex news removed/disabled
# ============================================================
with tab10:
    st.header("News (Stocks only)")
    st.caption("Forex news is intentionally disabled to avoid unreliable/missing feeds for FX tickers.")

    if str(mode).lower().startswith("forex"):
        st.info("News is disabled in Forex mode. Switch to Stocks mode to view ticker news.")
    else:
        sel_news = st.selectbox("Ticker:", universe, key="news_symbol_pick")
        lim = st.slider("Max headlines", 5, 50, 15, 1, key="news_limit")

        items = fetch_stock_news(sel_news, limit=int(lim))
        if not items:
            st.info("No news items returned for this ticker.")
        else:
            # newest first if timestamps exist
            items_sorted = sorted(
                items,
                key=lambda x: (x["time"] is not None, x["time"] or pd.Timestamp(0)),
                reverse=True,
            )
            ...
            for it in items_sorted:
                ...
                if t is not None:
                    try:
                        t_local = t.tz_localize("UTC").tz_convert("America/Los_Angeles")
                        tstr = t_local.strftime("%Y-%m-%d %H:%M %Z")
                    except Exception:
                        tstr = str(t)

                # In-app markdown links are fine
                if link:
                    st.markdown(f"- [{title}]({link})  \n  {pub} {('• ' + tstr) if tstr else ''}")
                else:
                    st.write(f"- {title}  {pub} {('• ' + tstr) if tstr else ''}")

# ============================================================
# TAB 11: HELP / NOTES (Final Tab)
# ============================================================
with tab11:
...
- **NTD -0.75 Scanner:** finds symbols where the most recent NTD is below -0.75.
- **Recent BUY Scanner:** looks for a recent NPX↑NTD cross in an uptrend (as defined by your existing logic).
- **NPX 0.5-Cross Scanner:** finds recent NPX crosses of 0.5 with local slope confirmation.
- **NTD Buy Signal:** finds recent **NTD upward crosses** through a threshold (default -0.75), optionally requiring a positive global slope.

**About News**
- News is shown **only in Stocks mode**.
- Forex mode shows a disabled message (forex news removed).
"""
    )
