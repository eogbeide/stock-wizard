...
# Streamlit app title
st.title("Stock Price Forecasting using SARIMA with EMA, MA, and Momentum Oscillator")
...
if st.button("Forecast"):
    ...
    momentum_oscillator = compute_momentum_oscillator(data)
    ...
    # Step 5: Plot historical data, forecast, EMA, daily moving average, Bollinger Bands, and Momentum Oscillator
    fig, ax1 = plt.subplots(figsize=(14, 7))

    # Plot price and 200-day EMA
    ax1.set_title(f'{ticker} Price Forecast, EMA, MA, Bollinger Bands, and Momentum Oscillator', fontsize=16)
    ...
    # Plot the Momentum Oscillator in a separate subplot
    fig2, ax2 = plt.subplots(figsize=(14, 4))
    ax2.set_title(f'{ticker} Momentum Oscillator', fontsize=16)
    ax2.plot(momentum_oscillator[-360:], label='Momentum Oscillator (14)', color='purple')
    ax2.axhline(0, color='black', linestyle='--', linewidth=0.7)
    ax2.set_xlabel('Date')
    ax2.set_ylabel('Momentum Value', color='purple')
    ax2.tick_params(axis='y', labelcolor='purple')
    ax2.legend(loc='upper left')

    # Display the Momentum Oscillator plot in Streamlit
    st.pyplot(fig2)
