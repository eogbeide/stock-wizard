...
def annotate_signal_box(ax, ts, px, side: str, note: str = "", ypad_frac: float = 0.045):
    """
    (Kept for any other signals) In-axes callout with arrow pointing to (ts, px).
    """
    try:
        ymin, ymax = ax.get_ylim()
        yr = ymax - ymin if np.isfinite(ymax) and np.isfinite(ymin) else 1.0
...
def annotate_band_rev_outside(ax, ts, px, side: str, note: str = "Band REV"):
    """
    NEW: Draw the Band REV label OUTSIDE the chart area (above the axes),
    with an arrow pointing from the label to the (ts, px) point inside the chart.

    Strategy:
      • Place the text in FIGURE coordinates just above the axes top (outside plot).
      • Align the text horizontally with the x-position of the point (clamped to margins).
      • Use xycoords='data' and textcoords=fig.transFigure so the arrow connects across spaces.
    """
    try:
        fig = ax.figure
        ...
        # Emphasize the actual point as well
        ax.scatter([ts], [px], s=60, c=("tab:green" if side=="BUY" else "tab:red"), zorder=1001)
    except Exception:
        # Fallback: in-axes label if figure transform fails for any reason
...
with tab1:
    ...
    if st.session_state.run_all and st.session_state.ticker == sel:
        ...
        if chart in ("Daily","Both"):
            ...
            fig, ax = plt.subplots(figsize=(14, 6))
            # Use full width; leave room at top for banner and outside labels
            plt.subplots_adjust(top=0.86, right=0.98, left=0.06)
