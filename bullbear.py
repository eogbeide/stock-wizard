...
# =========================
# ---------------------------
# Cached last values for scanning (Daily/Hourly)
# ---------------------------
@st.cache_data(ttl=120)
...
with tab1:
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        if mode == "Forex" and show_fx_news:
            fx_news = pd.DataFrame()  # kept unchanged in this batch
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        st.info("Enhanced Forecast content continues in later batches (unchanged).")
...
with tab3:
    ...
    st.info("Bull vs Bear content continues in later batches (unchanged).")
...
with tab4:
    st.header("Metrics")
    st.info("Metrics content continues in later batches (unchanged).")
# ---------------------------
...
# =====================================================================
# Batch 3 — Tabs 9–13 (incl. NEW: NTD Buy Signal tab)
# Requirement (b): Daily view range (Historical/6M/12M/24M) MUST be the
# SAME behavior as Tabs 12/13. We therefore REUSE the same daily_view
# variable and (if present) the same slicer helper used in Tabs 12/13.
# =====================================================================

# ---------------------------
# Helpers (safe re-use of Tabs 12/13 daily-view slicer)
# ---------------------------
def _apply_daily_view_range_fallback(s: pd.Series, daily_view_label: str) -> pd.Series:
    """Fallback only if your Tabs 12/13 slicer isn't defined elsewhere."""
    ...
    # Unknown label -> return full
    return s


def apply_daily_view_range_series(s: pd.Series, daily_view_label: str) -> pd.Series:
    """
    IMPORTANT: This tries to call the exact slicer you already use in Tabs 12/13.
    If not found, it uses the fallback above.
    """
    # Try common helper names you may already have in the earlier code:
    for fn_name in ("apply_daily_view_range", "slice_by_daily_view_range", "slice_daily_view_range"):
        fn = globals().get(fn_name, None)
        if callable(fn):
            try:
                return fn(s, daily_view_label)
            except Exception:
                pass
    return _apply_daily_view_range_fallback(s, daily_view_label)


def _linreg_slope(y: np.ndarray) -> float:
    """Simple linear regression slope vs bar index."""
...
def _compute_npx_series(close: pd.Series, win: int) -> pd.Series:
    # Prefer your existing implementation if present:
    for fn_name in ("compute_npx", "calc_npx", "make_npx"):
        fn = globals().get(fn_name, None)
        if callable(fn):
            return fn(close, win)

    # Fallback: rolling min/max normalization (0..1)
...
def _compute_ntd_series(close: pd.Series, win: int) -> pd.Series:
    # Prefer your existing implementation if present:
    for fn_name in ("compute_ntd", "calc_ntd", "make_ntd"):
        fn = globals().get(fn_name, None)
        if callable(fn):
            return fn(close, win)

    # Fallback: z-scored return smoothed (rough - can be negative)
    r = close.pct_change()
    mu = r.rolling(win).mean()
    sd = r.rolling(win).std().replace(0, np.nan)
    z = (r - mu) / sd
    return z.rolling(max(3, win // 10)).mean()


def _last_cross_up_level(s: pd.Series, level: float):
    s = s.dropna().sort_index()
    if s.size < 2:
        return None
...
def last_daily_ntd_cross_up_in_uptrend(
    sym: str,
    ntd_win: int,
    ...
    - AND DAILY chart-area global trendline (over selected daily_view range) is UP
    """
    # Close series (max)
    try:
        s_close = fetch_hist_max(sym)  # your existing helper (used in Tab 6)
        if isinstance(s_close, pd.DataFrame):
            s_close = s_close["Close"]
    except Exception:
        try:
            df = fetch_hist(sym, period="max", interval="1d")
            s_close = df["Close"]
        except Exception:
            return None

    if s_close is None or s_close.dropna().empty:
        return None

    s_close = s_close.dropna().sort_index()
    s_view = apply_daily_view_range_series(s_close, daily_view_label)
    if s_view is None or s_view.dropna().empty:
        return None

    # Trend slope on PRICE (chart-area) over selected daily view range
    slope = _linreg_slope(s_view.values)
    if not np.isfinite(slope) or slope <= 0:
        return None

    # Compute NTD/NPX series on full close, then reindex to view window
    ntd = _compute_ntd_series(s_close, ntd_win).reindex(s_view.index)
    npx = _compute_npx_series(s_close, ntd_win).reindex(s_view.index)

    cross_dt = _last_cross_up_level(ntd, threshold)
    if cross_dt is None:
        return None

    # Bars since cross (within view)
    ntd_view = ntd.dropna()
    try:
    ...
    out = {
        ...
        "NTD@Cross": float(ntd.loc[cross_dt]) if cross_dt in ntd.index and np.isfinite(ntd.loc[cross_dt]) else np.nan,
        "NPX@Cross": float(npx.loc[cross_dt]) if cross_dt in npx.index and np.isfinite(npx.loc[cross_dt]) else np.nan,
        "Global Slope": float(slope),
...
def _plot_price_and_indicators_daily(sym: str, ntd_win: int, daily_view_label: str):
    # Fetch close series
    try:
        s_close = fetch_hist_max(sym)
        if isinstance(s_close, pd.DataFrame):
            s_close = s_close["Close"]
    except Exception:
        df = fetch_hist(sym, period="max", interval="1d")
    ...
    # ---- Price chart (with trendline on view range) ----
    fig1, ax1 = plt.subplots(figsize=(14, 4))
    fig1.subplots_adjust(bottom=0.28)
    ax1.set_title(f"{sym} — Daily Close ({daily_view_label})")
    ax1.plot(s_view.index, s_view.values, label="Close")

    # Use your existing trendline drawer if available (to stay consistent),
    # otherwise just draw nothing extra beyond the slope you compute elsewhere.
    if callable(globals().get("draw_trend_direction_line", None)):
        try:
            draw_trend_direction_line(ax1, s_view, label_prefix=f"Trend ({daily_view_label})")
        except Exception:
            pass

    ax1.legend(loc="upper center", bbox_to_anchor=(0.5, -0.18), ncol=4, framealpha=0.65, fontsize=9, fancybox=True)
    if callable(globals().get("style_axes", None)):
        style_axes(ax1)
    st.pyplot(fig1)

    # ---- Indicator chart (NPX + NTD) ----
    npx = _compute_npx_series(s_close, ntd_win).reindex(s_view.index)
    ntd = _compute_ntd_series(s_close, ntd_win).reindex(s_view.index)

    fig2, ax2 = plt.subplots(figsize=(14, 3.5))
    fig2.subplots_adjust(bottom=0.28)
    ax2.set_title(f"{sym} — Indicators (NPX / NTD) ({daily_view_label})")
...
with tab9:
    ...
    st.caption(
        "Signal = **NTD crosses UP through -0.75** (oversold recovery) "
        "**AND** the DAILY chart-area global trendline (over the selected **Daily view range**) is **upward**.\n\n"
        f"Daily view range used here is the SAME selection as Tabs 12/13: **{daily_view}**."
    )

    c1, c2 = st.columns(2)
    max_bars_ntd = c1.slider("Max bars since NTD↑(-0.75) cross", 0, 40, 4, 1, key=f"ntd_buy_max_bars_{mode}")
    thr_ntd = c2.slider("NTD cross threshold", -2.0, 0.0, -0.75, 0.05, key=f"ntd_buy_thr_{mode}")

    run_ntd_buy = st.button("Run NTD Buy Signal Scan", key=f"btn_run_ntd_buy_{mode}")

    if run_ntd_buy:
        rows = []
        for sym in universe:
            r = last_daily_ntd_cross_up_in_uptrend(
                sym, ntd_win=ntd_window, daily_view_label=daily_view, threshold=float(thr_ntd)
            )
            if r is not None and int(r.get("Bars Since", 9999)) <= int(max_bars_ntd):
                rows.append(r)

        if not rows:
            st.info("No matches found (within the selected bar window + uptrend filter).")
        else:
            out = pd.DataFrame(rows)
            if "Bars Since" in out.columns:
                out["Bars Since"] = out["Bars Since"].astype(int)
...
# ---------------------------
# TAB 10: SYMBOL SNAPSHOT (optional but useful)
# ---------------------------
with tab10:
    st.header("Symbol Snapshot (Daily)")
    sel_snap = st.selectbox("Ticker:", universe, key=f"snap_ticker_{mode}")

    try:
        s_close = fetch_hist_max(sel_snap)
        if isinstance(s_close, pd.DataFrame):
            s_close = s_close["Close"]
    except Exception:
        s_close = pd.Series(dtype=float)

    if s_close is None or s_close.dropna().empty:
        st.warning("No data.")
    else:
        s_close = s_close.dropna().sort_index()
        s_view = apply_daily_view_range_series(s_close, daily_view)

        npx = _compute_npx_series(s_close, ntd_window).reindex(s_view.index)
        ntd = _compute_ntd_series(s_close, ntd_window).reindex(s_view.index)
        slope = _linreg_slope(s_view.values)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Last Close", f"{float(s_close.iloc[-1]):.4f}")
        c2.metric("Last NPX", f"{float(npx.dropna().iloc[-1]):.4f}" if npx.dropna().size else "—")
        c3.metric("Last NTD", f"{float(ntd.dropna().iloc[-1]):.4f}" if ntd.dropna().size else "—")
        c4.metric("Global Slope", f"{float(slope):.6f}" if np.isfinite(slope) else "—")

        _plot_price_and_indicators_daily(sel_snap, ntd_window, daily_view)


# ---------------------------
# TAB 11: (Keep/replace with your existing content)
# ---------------------------
with tab11:
    st.header("Notes / Diagnostics")
    st.write(
        "• This tab is a safe placeholder. If you already have Tab 11 content, keep yours and remove this.\n"
        "• Your scanners (Tabs 7/8/9) all use the same **Daily view range** setting as Tabs 12/13.\n"
        "• If you have a dedicated daily-view slicer used in Tabs 12/13, the NTD Buy tab will automatically "
        "use it (see `apply_daily_view_range_series`)."
    )


# ---------------------------
# TAB 12: DAILY CHART (Daily view range selector lives elsewhere in your app)
# ---------------------------
with tab12:
    st.header("Daily Chart")
    st.caption(f"Daily view range: **{daily_view}** (Historical / 6M / 12M / 24M)")

    sel_d = st.selectbox("Ticker:", universe, key=f"daily_ticker_{mode}")
    _plot_price_and_indicators_daily(sel_d, ntd_window, daily_view)
...
with tab13:
    ...
    st.caption("Intraday view (hourly). This does NOT change the Daily view range setting used by other tabs.")

    sel_h = st.selectbox("Ticker:", universe, key=f"hourly_ticker_{mode}")
    period_h = st.selectbox("Hourly period:", ["1d", "5d", "1mo", "3mo", "6mo"], index=1, key=f"hourly_period_{mode}")

    # Fetch hourly close
    try:
        df_h = fetch_hist(sel_h, period=period_h, interval="60m")
        s_h = df_h["Close"].dropna().sort_index()
    except Exception:
    ...
    if s_h.empty:
        ...
        fig1, ax1 = plt.subplots(figsize=(14, 4))
        fig1.subplots_adjust(bottom=0.28)
        ax1.set_title(f"{sel_h} — Hourly Close ({period_h})")
        ax1.plot(s_h.index, s_h.values, label="Close")
        ax1.legend(loc="upper center", bbox_to_anchor=(0.5, -0.18), ncol=4, framealpha=0.65, fontsize=9, fancybox=True)
        if callable(globals().get("style_axes", None)):
            style_axes(ax1)
        st.pyplot(fig1)
        ...
        # Indicator chart (NPX + NTD)
        npx_h = _compute_npx_series(s_h, ntd_window)
        ntd_h = _compute_ntd_series(s_h, ntd_window)

        fig2, ax2 = plt.subplots(figsize=(14, 3.5))
        fig2.subplots_adjust(bottom=0.28)
        ax2.set_title(f"{sel_h} — Hourly Indicators (NPX / NTD) ({period_h})")
