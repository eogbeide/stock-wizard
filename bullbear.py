...
# ---------------------------
# UPDATED (THIS REQUEST): Combined NTD + NPX last-value helpers for scanning (single fetch path)
# ---------------------------
@st.cache_data(ttl=120)
def last_daily_ntd_npx_values(symbol: str, ntd_win: int):
    """
    Returns: (ntd_last, npx_last, timestamp)
    """
    try:
        ...
        ntd = compute_normalized_trend(close, window=ntd_win)
        npx = compute_normalized_price(close, window=ntd_win)

        ntd_s = _coerce_1d_series(ntd).dropna()
        npx_s = _coerce_1d_series(npx).dropna()

        ntd_last = float(ntd_s.iloc[-1]) if len(ntd_s) else np.nan
        npx_last = float(npx_s.iloc[-1]) if len(npx_s) else np.nan
        ...
        if len(ntd_s):
            ...
            ts = close.index[-1] if isinstance(close.index, pd.DatetimeIndex) and len(close) else None

        return ntd_last, npx_last, ts
    except Exception:
        return np.nan, np.nan, None

@st.cache_data(ttl=120)
def last_hourly_ntd_npx_values(symbol: str, ntd_win: int, period: str = "1d"):
    """
    Returns: (ntd_last, npx_last, timestamp)
    Uses intraday Close series.
    """
    try:
        df = fetch_intraday(symbol, period=period)
...
def last_daily_fib_npx_zero_signal(symbol: str,
    ...
    UPDATED (THIS REQUEST):
      Scanner helper now requires:
        ...
        • Slope reversal aligned with the side
...
# ---------------------------
# NEW (THIS REQUEST): Daily slope direction helper for slope-based tabs
# ---------------------------
@st.cache_data(ttl=120)
...
# ---------------------------
# NEW (THIS REQUEST): Last NPX (Norm Price) in the selected Daily view range
# ---------------------------
@st.cache_data(ttl=120)
def last_daily_npx_in_view(symbol: str, daily_view_label: str, ntd_win: int):
    """
    Returns: (npx_last, timestamp)
    NPX computed from full history then reindexed to the selected daily-view window.
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if close_full.empty:
        ...
        npx_valid = npx_show.dropna()
        if npx_valid.empty:
            return np.nan, close_show.index[-1]
        return float(npx_valid.iloc[-1]), npx_valid.index[-1]
    except Exception:
        return np.nan, None
...
# UPDATED (THIS REQUEST):
#   - Removed 3 tabs: Daily Slope+BB Reversal Scanner, Fib 0%/100% Reversal Watchlist, Fib 0%/100% 99.9% Reversal
#   - Added new tab: NTD Hot List
...
with tab5:
    ...
    # UPDATED (THIS REQUEST): add NPX (Norm Price) column
    if run_scan:
        rows = []
        if scan_frame.startswith("Hourly"):
            period = "1d"
            for sym in universe:
                ntd_val, npx_val, ts = last_hourly_ntd_npx_values(sym, ntd_window, period=period)
                if np.isfinite(ntd_val) and ntd_val < -0.75:
                    rows.append({"Symbol": sym, "NTD": float(ntd_val), "NPX (Norm Price)": float(npx_val) if np.isfinite(npx_val) else np.nan, "Time": ts})
        else:
            for sym in universe:
                ntd_val, npx_val, ts = last_daily_ntd_npx_values(sym, ntd_window)
                if np.isfinite(ntd_val) and ntd_val < -0.75:
                    rows.append({"Symbol": sym, "NTD": float(ntd_val), "NPX (Norm Price)": float(npx_val) if np.isfinite(npx_val) else np.nan, "Time": ts})
...
# ---------------------------
# TAB 6: NTD Hot List (NEW)
# ---------------------------
with tab6:
    st.header("NTD Hot List")
    st.caption(
        "Symbols where:\n"
        "• **Regression slope > 0** (price trending upward in the selected Daily view range)\n"
        "• **NPX (Norm Price) is between 0.0 and 0.5**"
    )
    ...
    if run_hot:
        ...
        for sym in universe:
            ...
            npx_last, npx_ts = last_daily_npx_in_view(sym, daily_view_label=daily_view, ntd_win=ntd_window)
...
with tab12:
    ...
    # UPDATED (THIS REQUEST):
    #   - Adds NPX (Norm Price) column
    #   - Filters:
    #       • Upward list: NPX < 0.0
    #       • Downward list: NPX > 0.5
    ...
    if run_trend_lists:
        ...
        for sym in universe:
            ...
            npx_last, _ = last_daily_npx_in_view(sym, daily_view_label=daily_view, ntd_win=ntd_window)
            if not np.isfinite(npx_last):
                continue

            if float(m) >= 0.0:
                if float(npx_last) < 0.0:
                    up_rows.append({"Symbol": sym, "NPX (Norm Price)": float(npx_last)})
            else:
                if float(npx_last) > 0.5:
        ...
        with left:
            ...
            if not up_rows:
                ...
                out_up = pd.DataFrame(up_rows).sort_values(["NPX (Norm Price)", "Symbol"], ascending=[True, True])
                st.dataframe(out_up.reset_index(drop=True), use_container_width=True)
        ...
        with right:
            ...
            if not dn_rows:
                ...
                out_dn = pd.DataFrame(dn_rows).sort_values(["NPX (Norm Price)", "Symbol"], ascending=[False, True])
                st.dataframe(out_dn.reset_index(drop=True), use_container_width=True)
    else:
        st.info("Click **Run Trendline Direction Lists** to scan the current universe.")

# ---------------------------
# TAB 13: Enhanced Forecast Buy and Sell
# ---------------------------
with tab13:
