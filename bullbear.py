...
mode = st.sidebar.selectbox("Mode", ["Stock", "Forex"])
...
# Parameters
npo_win = st.sidebar.slider("NPO window", 30, 600, 240, 10)
ntd_win = st.sidebar.slider("NTD window", 30, 600, 240, 10)

ichi_conv = st.sidebar.slider("Ichimoku Conversion (Tenkan)", 5, 20, 9, 1)
ichi_base = st.sidebar.slider("Ichimoku Base (Kijun)", 20, 40, 26, 1)
ichi_spanb = st.sidebar.slider("Ichimoku Span B", 40, 80, 52, 1)
ichi_norm_win = st.sidebar.slider("Ichimoku norm window", 30, 600, 240, 10)
ichi_weight = st.sidebar.slider("Ichimoku price vs cloud weight", 0.0, 1.0, 0.6, 0.05)
...
def compute_normalized_rsi(close: pd.Series, period: int = 14, norm_win: int = 240) -> pd.Series:
    ...
    nrsi = np.tanh(((rsi-mean)/std)/2)
    return nrsi.reindex(s.index)
...
def draw_ew_panel(df, wave_norm, npo=None, ntd=None, ichi=None, nrsi=None, title="EW Panel"):
    ...
    if npo is not None and not npo.dropna().empty:
        ax.plot(npo, "--", color="red", label="NPO")
    if ntd is not None and not ntd.dropna().empty:
        ax.plot(ntd, ":", color="green", label="NTD")
    ...
    if nrsi is not None and not nrsi.dropna().empty:
        ...
        ax.plot(nrsi, "--", color="purple", linewidth=1.2, label=f"NRSI ({nrsi_period})")

    # Reference lines
    for y, c in [(0,"grey"),(0.5,"red"),(-0.5,"green"),(0.75,"black"),(-0.75,"black")]:
        ax.axhline(y, linestyle="--", linewidth=1, color=c)
...
if st.sidebar.button("Run"):
    ...
    npo_d = compute_npo(df, norm_win=npo_win) if show_npo else None
    ntd_d = compute_ntd(df, window=ntd_win) if show_ntd else None
    nrsi_d = compute_normalized_rsi(df, nrsi_period, nrsi_norm_win) if show_nrsi else None
    ichi_d = compute_normalized_ichimoku(ohlc["High"],ohlc["Low"],ohlc["Close"],
                                         conv=ichi_conv, base=ichi_base,
                                         span_b=ichi_spanb, norm_win=ichi_norm_win,
                                         w=ichi_weight) if show_ichi else None
    _, kijun_d,_,_,_ = ichimoku_lines(ohlc["High"],ohlc["Low"],ohlc["Close"])

    # Hourly indicators
    hc = intraday["Close"]
    wave_norm_h = compute_ntd(hc)
    npo_h = compute_npo(hc, norm_win=npo_win) if show_npo else None
    ntd_h = compute_ntd(hc, window=ntd_win) if show_ntd else None
    nrsi_h = compute_normalized_rsi(hc, nrsi_period, nrsi_norm_win) if show_nrsi else None
    ichi_h = compute_normalized_ichimoku(intraday["High"],intraday["Low"],intraday["Close"],
                                         conv=ichi_conv, base=ichi_base,
                                         span_b=ichi_spanb, norm_win=ichi_norm_win,
                                         w=ichi_weight) if show_ichi else None
    ...
    fig_d, _ = draw_daily_chart(df, ohlc, kijun_d); st.pyplot(fig_d)
    ...
    fig_h, _ = draw_hourly_chart(hc, kijun_h); st.pyplot(fig_h)
