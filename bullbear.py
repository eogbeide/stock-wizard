...
# UPDATED (THIS REQUEST): Remove forex/stock news from UI + logic
show_fx_news = False
news_window_days = 7
...
# UPDATED (THIS REQUEST): add NEW tab "NTD Buy Signal" to make 19 tabs (existing tabs unchanged)
...
# ============================================================
# NEW: Daily NTD "Buy Signal" (NTD < -0.75 AND heading up)
#      (a) Daily only
#      (b) configurable 2–5 bars slider
# ============================================================

def _maybe_apply_daily_view(df: pd.DataFrame, daily_view_label: str | None):
    """Uses your existing daily-range slicer if it exists; otherwise no-op."""
    if df is None or df.empty:
        return df
    if daily_view_label and "apply_daily_view" in globals():
        try:
            return apply_daily_view(df, daily_view_label)
        except Exception:
            return df
    return df


def _daily_df_with_indicators(sym: str, ntd_win: int) -> pd.DataFrame | None:
    """
    Tries to reuse whatever indicator-builder your app already uses.
    Expected output: daily DataFrame indexed by datetime with a column named 'NTD'.
    """
    # If you already have a canonical builder, this will pick it up:
    for fname in (
        "get_daily_df_with_indicators",
        "build_daily_df_with_indicators",
        "daily_df_with_indicators",
        "build_daily_df",
    ):
        if fname in globals():
            try:
                return globals()[fname](sym, ntd_win)
            except Exception:
                pass

    # Fallback: if you at least have raw daily history + calc functions
    if "fetch_hist_daily" in globals():
        try:
            df = fetch_hist_daily(sym).copy()
        except Exception:
            return None
        ...
        # Try common calc function names used elsewhere in your app
        if "calc_ntd_series" in globals():
            try:
                df["NTD"] = calc_ntd_series(close, ntd_win)
                return df
            except Exception:
                return None
        if "calc_npx_ntd" in globals():
            try:
                npx, ntd = calc_npx_ntd(close, ntd_win)
                df["NTD"] = ntd
                return df
            except Exception:
                return None
...
def last_daily_ntd_buy_signal_heading_up(
    sym: str,
    ntd_win: int,
    bars_up: int,
    daily_view_label: str | None,
    threshold: float = -0.75,
    require_below_threshold: bool = True,
):
    """
    Signal = last NTD value is below threshold (default -0.75) AND NTD has risen
    consecutively for the last `bars_up` bars.
    """
    df = _daily_df_with_indicators(sym, ntd_win)
    if df is None or df.empty or "NTD" not in df.columns:
        return None

    df = _maybe_apply_daily_view(df, daily_view_label)

    ntd = df["NTD"].dropna()
    if len(ntd) < bars_up + 1:
        return None

    tail = ntd.iloc[-(bars_up + 1):]
    diffs = np.diff(tail.values)

    # "Heading up" = all consecutive increases across the window
    if not np.all(diffs > 0):
        return None

    last_val = float(tail.iloc[-1])
    if require_below_threshold and not (last_val < threshold):
        return None

    dt = df.index[-1] if hasattr(df.index, "__len__") else None
    dt_str = str(dt) if dt is not None else ""

    net_change = float(tail.iloc[-1] - tail.iloc[0])
    per_bar = net_change / float(bars_up)

    return {
        "Symbol": sym,
        "NTD": last_val,
        "Bars Up": int(bars_up),
        "ΔNTD (window)": net_change,
        "Slope (per bar)": per_bar,
        "Last Date": dt_str,
    }


# ---------------------------
# TAB 9: NTD BUY SIGNAL (NEW TAB)
# ---------------------------
with tab9:
    st.header("NTD Buy Signal — Daily (NTD < -0.75 AND Heading Up)")
    st.caption(
        "Daily-only scanner. A symbol appears if its **latest Daily NTD** is below **-0.75** "
        "and the **NTD line is rising for the last N bars** (N configurable)."
    )

    c1, c2, c3 = st.columns([1, 1, 2])
    bars_up = c1.slider("Heading up bars (N)", 2, 5, 3, 1, key="ntd_buy_heading_bars")
    require_below = c2.checkbox("Require NTD < -0.75 now", value=True, key="ntd_buy_require_below")
    run_ntd_buy = c3.button("Run NTD Buy Scan", key="btn_run_ntd_buy_scan")

    if run_ntd_buy:
        rows = []
        for sym in universe:
            r = last_daily_ntd_buy_signal_heading_up(
                sym,
                ntd_win=ntd_window,
                bars_up=bars_up,
                daily_view_label=daily_view,
                threshold=-0.75,
                require_below_threshold=require_below,
            )
            if r is not None:
                rows.append(r)
        ...
        if not rows:
            ...
            # Nice numeric formatting/sorting
            for col in ("NTD", "ΔNTD (window)", "Slope (per bar)"):
                if col in out.columns:
                    out[col] = out[col].astype(float)

            # Most oversold first; if ties, strongest slope first
            out = out.sort_values(["NTD", "Slope (per bar)"], ascending=[True, False])

            st.dataframe(out.reset_index(drop=True), use_container_width=True)

# ---------------------------
# (OPTIONAL) TAB 10: ABOUT / NOTES (only if you have tab10)
# ---------------------------
# with tab10:
#     st.header("Notes")
#     st.markdown(
#         "- **NTD Buy Signal** is computed on **Daily** data only.\n"
#         "- 'Heading up bars' means NTD increased consecutively across that many bars.\n"
#         "- The scanner uses your current universe (Stocks vs Forex) from the mode toggle."
#     )
