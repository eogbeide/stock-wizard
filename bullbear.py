...
# (THIS REQUEST) Remove Forex/Stock news UI + logic
show_fx_news = False
news_window_days = 7
...
# ... (UNCHANGED code continues: Parts 5/10–8/10 helpers, Ichimoku/Supertrend/PSAR, scanners, render_hourly_views, etc.)
# NOTE: Forex/Stock news fetch + markers were removed per request (no UI/logic for news).
...
def render_hourly_views(sel: str,
    ...
    # (UNCHANGED hourly internals continue in later batches; news markers removed)

    st.warning("Batch 1 ends here (code continues in Batch 2/3).")  # placeholder line to prevent partial snippet confusion
...
# - Added tabs: "Buy Candidates", "NTD Buy Signal"
...
with tab1:
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        # (UNCHANGED Tab 1 continues; Forex/Stock news section removed per request)
        st.info("Batch 1 ends here (Tab 1 continues in your original code).")  # placeholder line
...
with tab2:
    ...
    st.info("Batch 1 ends here (Tab 2 full code continues in Batch 3).")  # placeholder line
...
with tab3:
    ...
    st.info("Batch 1 ends here (Tab 3 full code continues in Batch 3).")  # placeholder line
...
with tab4:
    st.header("Metrics")
    st.info("Batch 1 ends here (Tab 4 full code continues in Batch 3).")  # placeholder line
# ---------------------------
...
with tab8:
    ...
    st.caption(
        ...
        "and the **Regression line slope > 0** (Daily, using the selected Daily view range)."
    )
    ...
    if run_buy_candidates:
        ...
        for sym in universe:
            r = last_daily_kijun_cross_up(
                ...
                within_last_n_bars=2,   # we will further filter to 1–2 bars specifically
            )
            if r is None:
                continue
            ...
            # Requirement: crossed by 1–2 bars (exclude 0)
...
# UPDATED (THIS REQUEST): Removed "NPX 0.5-Cross Scanner" and "Fib NPX 0.0 Signal Scanner"
# and replaced them with "Buy Candidates" and "NTD Buy Signal"
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10, tab11, tab12, tab13, tab14, tab15, tab16, tab17, tab18 = st.tabs([
    "Original Forecast",
    "Enhanced Forecast",
    "Bull vs Bear",
...
# =========================
# NOTE:
# Tabs 1–8 are defined earlier (Batch 1 + Batch 2).
# Below continues with Tab 9 onward (THIS BATCH).
# ---------------------------
...
with tab9:
    ...
    st.caption(
        "Shows symbols where the **Regression line slope > 0** and **NPX (Norm Price)** is between **+0.5** and **+0.6**, "
        "and NPX is **heading up** (Daily, using the selected Daily view range)."
    )

    c1, c2 = st.columns(2)
    npx_up_bars_sig = c1.slider("NPX heading-up confirmation (consecutive bars)", 1, 5, 1, 1, key="ntd_buy_npx_up_bars")
    run_ntd_buy = c2.button("Run NTD Buy Signal Scan", key=f"btn_run_ntd_buy_signal_{mode}")
    ...
    if run_ntd_buy:
        ...
        for sym in universe:
            ...
            if not _series_heading_up(npx_s, confirm_bars=int(npx_up_bars_sig)):
                continue
