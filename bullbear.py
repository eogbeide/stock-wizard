    ...
    if st.session_state.run_all and st.session_state.ticker == sel:
        ...
        if chart in ("Daily","Both"):
            ...
            if not upper_d_show.empty and not lower_d_show.empty:
                ax.plot(upper_d_show.index, upper_d_show.values, ":", linewidth=3.0, color="black", alpha=0.95, label="_nolegend_")
                ax.plot(lower_d_show.index, lower_d_show.values, ":", linewidth=3.0, color="black", alpha=0.95, label="_nolegend_")
            if len(df_show) > 1:
        ...
        if chart in ("Hourly","Both"):
            ...
            if intraday is None or intraday.empty or "Close" not in intraday:
                ...
                if not upper_h.empty and not lower_h.empty:
                    ax2.plot(upper_h.index, upper_h.values, ":", linewidth=3.0, color="black", alpha=0.95, label="_nolegend_")
                    ax2.plot(lower_h.index, lower_h.values, ":", linewidth=3.0, color="black", alpha=0.95, label="_nolegend_")
    ...
    if not st.session_state.run_all:
        ...
        if view in ("Daily","Both"):
            ...
            if not up_d_show.empty and not lo_d_show.empty:
                ax.plot(up_d_show.index, up_d_show.values, ":", linewidth=3.0, color="black", alpha=0.95, label="_nolegend_")
            if not lo_d_show.empty:
                ax.plot(lo_d_show.index, lo_d_show.values, ":", linewidth=3.0, color="black", alpha=0.95, label="_nolegend_")
            if len(df_show) > 1:
                draw_trend_direction_line(ax, df_show, label_prefix="")
