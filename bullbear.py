...
# =========================
# NEW: Cross info helpers (used for Trend and Slope Align tables)
# =========================
@st.cache_data(ttl=120)
def last_npx_ntd_cross_info_daily(symbol: str,
                                 daily_view_label: str,
                                 ntd_win: int = 60):
    """
    Returns the most recent NPX↔NTD cross within the selected daily view.
    Output keys match requested columns:
      - Bars Since Cross
      - Cross Direction
      - Time Crossed
    """
    try:
        ...
        if len(close_show) < 5:
            return {"Bars Since Cross": np.nan, "Cross Direction": "", "Time Crossed": pd.NaT}
        ...
        direction = "Up" if bool(up_mask.reindex(ntd.index, fill_value=False).loc[t_cross]) else "Down"

        return {"Bars Since Cross": int(bars_since), "Cross Direction": direction, "Time Crossed": t_cross}
    except Exception:
        return {"Bars Since Cross": np.nan, "Cross Direction": "", "Time Crossed": pd.NaT}

@st.cache_data(ttl=120)
def last_npx_ntd_cross_info_hourly(symbol: str,
                                  period: str,
                                  ntd_win: int = 60):
    """
    Returns the most recent NPX↔NTD cross on intraday (5m) bars within selected period.
    Output keys match requested columns:
      - Bars Since Cross
      - Cross Direction
      - Time Crossed
    ...
    try:
        ...
        if df is None or df.empty or "Close" not in df.columns:
            return {"Bars Since Cross": np.nan, "Cross Direction": "", "Time Crossed": np.nan}
        ...
        direction = "Up" if bool(up_mask.reindex(ntd.index, fill_value=False).loc[bar]) else "Down"

        cross_time = bar
        if isinstance(real_times, pd.DatetimeIndex) and (0 <= bar < len(real_times)):
            cross_time = real_times[bar]

        return {"Bars Since Cross": int(bars_since), "Cross Direction": direction, "Time Crossed": cross_time}
    except Exception:
        return {"Bars Since Cross": np.nan, "Cross Direction": "", "Time Crossed": np.nan}
...
# =========================
# Trend & Slope Align helpers (Daily + Hourly)
# =========================
@st.cache_data(ttl=120)
...
def trend_slope_align_row_hourly(symbol: str, period: str, slope_lb: int):
    """
    Hourly counterpart for Trend and Slope Align:
    - Trendline Slope = global slope on intraday (5m bars) over selected period
    - Regression Slope = local regression slope over hourly lookback
    """
    try:
        df = fetch_intraday(symbol, period=period)
        if df is None or df.empty or "Close" not in df.columns:
        ...
        mg, _ = np.polyfit(xg, hc.to_numpy(dtype=float), 1)
        _, _, _, ml, r2 = regression_with_band(hc, lookback=min(len(hc), int(slope_lb)))
...
def _sr_reversal_event_time(price: pd.Series,
                            ...
                            prox: float = 0.0025):
    """
    Finds the most recent support/resistance "reversal" event time.
    direction:
      - 'BUY'  : touched support recently then moved up (p[t] > p[t-1])
      - 'SELL' : touched resistance recently then moved down (p[t] < p[t-1])
    """
    p = _coerce_1d_series(price).astype(float)
    sup = _coerce_1d_series(support).reindex(p.index).ffill()
    res = _coerce_1d_series(resistance).reindex(p.index).ffill()

    ok = p.notna() & sup.notna() & res.notna()
    if ok.sum() < 3:
        return None

    p = p[ok]; sup = sup[ok]; res = res[ok]

    if str(direction).upper() == "BUY":
        touched = p.shift(1) <= sup.shift(1) * (1.0 + float(prox))
        moved = p > p.shift(1)
        ev = (touched & moved).fillna(False)
    else:
        touched = p.shift(1) >= res.shift(1) * (1.0 - float(prox))
        moved = p < p.shift(1)
        ev = (touched & moved).fillna(False)
...
# =========================
# NEW: HMA Trade Signal helpers (Daily + Hourly)
# =========================
@st.cache_data(ttl=120)
def hma_trade_signal_row_daily(symbol: str,
                               daily_view_label: str,
                               max_bars_since: int = 5,
                               sr_lb: int = 60,
                               hma_len: int = 55,
                               prox: float = 0.0025):
    """
    HMA Trade Signal (Daily):
      - BUY: trendline is upward AND (HMA crossed UP through support OR HMA reversed up from support)
      - SELL: trendline is downward AND (HMA crossed DOWN through resistance OR HMA reversed down from resistance)
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        ...
        # Support/Resistance series (same style as charts)
        sup = close_full.rolling(int(sr_lb), min_periods=1).min().reindex(close_show.index).ffill()
        res = close_full.rolling(int(sr_lb), min_periods=1).max().reindex(close_show.index).ffill()

        hma = _coerce_1d_series(compute_hma(close_show, period=int(hma_len))).reindex(close_show.index)
        ok = hma.notna() & sup.notna() & res.notna()
        if ok.sum() < 3:
            return None
        hma = hma[ok]; sup = sup[ok]; res = res[ok]

        last_price = float(close_show.iloc[-1]) if np.isfinite(close_show.iloc[-1]) else np.nan
        hma_last = float(hma.iloc[-1]) if np.isfinite(hma.iloc[-1]) else np.nan
        sup_last = float(sup.iloc[-1]) if np.isfinite(sup.iloc[-1]) else np.nan
        res_last = float(res.iloc[-1]) if np.isfinite(res.iloc[-1]) else np.nan

        # BUY side (trend up)
        if float(g) > 0.0:
            cross_up_sup, _ = _cross_series(hma, sup)
            t_cross = cross_up_sup[cross_up_sup].index[-1] if cross_up_sup.any() else None
            t_rev = _sr_reversal_event_time(hma, sup, res, direction="BUY", prox=float(prox))

            candidates = [t for t in [t_cross, t_rev] if t is not None]
            if not candidates:
                return None
            ...
            src = "HMA Cross ↑ Support" if (t_cross is not None and t == t_cross) else "HMA Reversal @ Support"
            return {
                "Symbol": symbol,
                "Frame": "Daily",
                "Side": "BUY",
                ...
                "HMA(last)": hma_last,
                "Support(last)": sup_last,
                "Resistance(last)": res_last,
                "Last Price": last_price,
            }

        # SELL side (trend down)
        if float(g) < 0.0:
            _, cross_dn_res = _cross_series(hma, res)
            t_cross = cross_dn_res[cross_dn_res].index[-1] if cross_dn_res.any() else None
            t_rev = _sr_reversal_event_time(hma, sup, res, direction="SELL", prox=float(prox))
            ...
            src = "HMA Cross ↓ Resistance" if (t_cross is not None and t == t_cross) else "HMA Reversal @ Resistance"
            return {
                "Symbol": symbol,
                "Frame": "Daily",
                "Side": "SELL",
...
@st.cache_data(ttl=120)
def hma_trade_signal_row_hourly(symbol: str,
                                period: str,
                                max_bars_since: int = 10,
                                sr_lb: int = 60,
                                hma_len: int = 55,
                                prox: float = 0.0025):
    """
    HMA Trade Signal (Hourly / intraday 5m bars):
      - BUY: trendline is upward AND (HMA crossed UP through support OR HMA reversed up from support)
      - SELL: trendline is downward AND (HMA crossed DOWN through resistance OR HMA reversed down from resistance)
    """
    try:
        ...
        # Support/Resistance series (same style as hourly chart)
        sup = hc.rolling(int(sr_lb), min_periods=1).min()
        res = hc.rolling(int(sr_lb), min_periods=1).max()
        hma = _coerce_1d_series(compute_hma(hc, period=int(hma_len)))
        ...
        sup_last = float(sup.dropna().iloc[-1]) if len(sup.dropna()) else np.nan
        res_last = float(res.dropna().iloc[-1]) if len(res.dropna()) else np.nan

        if float(g) > 0.0:
            cross_up_sup, _ = _cross_series(hma, sup)
            bar_cross = int(cross_up_sup[cross_up_sup].index[-1]) if cross_up_sup.any() else None
            bar_rev = _sr_reversal_event_time(hma, sup, res, direction="BUY", prox=float(prox))
            bar_rev = int(bar_rev) if bar_rev is not None else None

            candidates = [b for b in [bar_cross, bar_rev] if b is not None]
            if not candidates:
                return None

            bar = int(max(candidates))
            bars_since = int((len(hc) - 1) - bar)
            if int(bars_since) > int(max_bars_since):
                return None

            sig_time = real_times[bar] if (isinstance(real_times, pd.DatetimeIndex) and (0 <= bar < len(real_times))) else bar
            src = "HMA Cross ↑ Support" if (bar_cross is not None and bar == bar_cross) else "HMA Reversal @ Support"
        ...
        if float(g) < 0.0:
            _, cross_dn_res = _cross_series(hma, res)
            bar_cross = int(cross_dn_res[cross_dn_res].index[-1]) if cross_dn_res.any() else None
            bar_rev = _sr_reversal_event_time(hma, sup, res, direction="SELL", prox=float(prox))
            bar_rev = int(bar_rev) if bar_rev is not None else None
            ...
            sig_time = real_times[bar] if (isinstance(real_times, pd.DatetimeIndex) and (0 <= bar < len(real_times))) else bar
            src = "HMA Cross ↓ Resistance" if (bar_cross is not None and bar == bar_cross) else "HMA Reversal @ Resistance"
...
@st.cache_data(ttl=120)
def regression_reversal_row_daily(symbol: str,
                                  daily_view_label: str,
                                  slope_lb: int,
                                  max_bars_since: int = 5,
                                  sr_lb: int = 60,
                                  prox: float = 0.0025):
    """
    Regression Reversal (Daily):
      (a) global trendline >0 AND regression slope >0 AND recent reversal from -2σ OR support, going up
      (b) global trendline <0 AND regression slope <0 AND recent reversal from +2σ OR resistance, going down
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        ...
        # S/R in the same style as charts (rolling on full history, reindexed to view)
        res = close_full.rolling(int(sr_lb), min_periods=1).max().reindex(close_show.index)
        sup = close_full.rolling(int(sr_lb), min_periods=1).min().reindex(close_show.index)

        # Band reversal (existing logic)
        band_sig = find_band_bounce_signal(close_show, up, lo, local_m)
        band_time = band_sig.get("time") if isinstance(band_sig, dict) else None

        # S/R reversal
        sr_time_buy = _sr_reversal_event_time(close_show, sup, res, direction="BUY", prox=float(prox))
        sr_time_sell = _sr_reversal_event_time(close_show, sup, res, direction="SELL", prox=float(prox))

        # Decide side + most recent qualifying event
        if (float(global_m) > 0.0) and (float(local_m) > 0.0):
            # BUY: band BUY time OR support-reversal time
            candidates = [t for t in [band_time, sr_time_buy] if t is not None]
            if not candidates:
                return None
            t = max(candidates)
            bars_since = _bars_since_last_event(close_show.index, t)
            ...
            yhat_s = _coerce_1d_series(yhat).reindex(close_show.index)
            reg_px = float(yhat_s.iloc[-1]) if len(yhat_s.dropna()) and np.isfinite(yhat_s.iloc[-1]) else np.nan
            src = "Band -2σ" if (band_time is not None and t == band_time) else "Support"
            return {
                "Symbol": symbol,
                "Frame": "Daily",
                "Side": "BUY",
        ...
        if (float(global_m) < 0.0) and (float(local_m) < 0.0):
            # SELL: band SELL time OR resistance-reversal time
            candidates = [t for t in [band_time, sr_time_sell] if t is not None]
            if not candidates:
                return None
            t = max(candidates)
            bars_since = _bars_since_last_event(close_show.index, t)
            ...
            yhat_s = _coerce_1d_series(yhat).reindex(close_show.index)
            reg_px = float(yhat_s.iloc[-1]) if len(yhat_s.dropna()) and np.isfinite(yhat_s.iloc[-1]) else np.nan
            src = "Band +2σ" if (band_time is not None and t == band_time) else "Resistance"
            return {
                "Symbol": symbol,
                "Frame": "Daily",
                "Side": "SELL",
...
@st.cache_data(ttl=120)
def regression_reversal_row_hourly(symbol: str,
                                   period: str,
                                   slope_lb: int,
                                   max_bars_since: int = 10,
                                   sr_lb: int = 60,
                                   prox: float = 0.0025):
    """
    Regression Reversal (Hourly/Intraday 5m bars):
      (a) global trendline >0 AND regression slope >0 AND recent reversal from -2σ OR support, going up
      (b) global trendline <0 AND regression slope <0 AND recent reversal from +2σ OR resistance, going down
    """
    try:
        ...
        # S/R on intraday close
        res = hc.rolling(int(sr_lb), min_periods=1).max()
        sup = hc.rolling(int(sr_lb), min_periods=1).min()

        # Band reversal
        band_sig = find_band_bounce_signal(hc, up, lo, local_m)
        band_bar = band_sig.get("time") if isinstance(band_sig, dict) else None

        # S/R reversal
        sr_bar_buy = _sr_reversal_event_time(hc, sup, res, direction="BUY", prox=float(prox))
        sr_bar_sell = _sr_reversal_event_time(hc, sup, res, direction="SELL", prox=float(prox))

        if (float(global_m) > 0.0) and (float(local_m) > 0.0):
            candidates = [t for t in [band_bar, sr_bar_buy] if t is not None]
            if not candidates:
                return None
            bar = int(max(candidates))
            bars_since = int((len(hc) - 1) - bar)
            ...
            yhat_s = _coerce_1d_series(yhat)
            reg_px = float(yhat_s.dropna().iloc[-1]) if len(yhat_s.dropna()) else np.nan
            src = "Band -2σ" if (band_bar is not None and bar == int(band_bar)) else "Support"
            return {
                "Symbol": symbol,
                "Frame": f"Hourly ({period})",
                "Side": "BUY",
        ...
        if (float(global_m) < 0.0) and (float(local_m) < 0.0):
            candidates = [t for t in [band_bar, sr_bar_sell] if t is not None]
            if not candidates:
                return None
            bar = int(max(candidates))
            bars_since = int((len(hc) - 1) - bar)
            ...
            yhat_s = _coerce_1d_series(yhat)
            reg_px = float(yhat_s.dropna().iloc[-1]) if len(yhat_s.dropna()) else np.nan
            src = "Band +2σ" if (band_bar is not None and bar == int(band_bar)) else "Resistance"
            return {
                "Symbol": symbol,
                "Frame": f"Hourly ({period})",
                "Side": "SELL",
...
# =========================
@st.cache_data(ttl=120)
def magic_cross_row_daily(symbol: str,
                          daily_view_label: str,
                          slope_lb: int,
                          ntd_win: int = 60,
                          max_bars_since: int = 5):
    """
    Magic Cross (Daily):
    NPX recently crossed ABOVE NTD (cross-up), and the cross occurs in the positive zone
    (between 0.0 and 1.0 on the NTD/NPX panel scale).
    Returns slopes so the tab can filter by:
      (1) Regression Slope > 0
      (2) Global Trendline > 0
      (3) Global Trendline > 0 AND Regression Slope > 0
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        ...
        # Positive-zone cross on NTD panel (0.0 to 1.0 area)
        zone_mask = (
            (npx >= 0.0) & (npx <= 1.0) &
            (ntd >= 0.0) & (ntd <= 1.0)
        ).fillna(False)

        cross_mask = up_mask & zone_mask
        if not cross_mask.any():
            return None
        ...
        return {
            ...
            "NPX(last)": npx_last,
            "NTD(last)": ntd_last,
            "Trendline Slope": float(global_m) if np.isfinite(global_m) else np.nan,
            "Regression Slope": float(local_m) if np.isfinite(local_m) else np.nan,
            "R2": float(r2) if np.isfinite(r2) else np.nan,
            "Last Price": last_px,
        }
...
@st.cache_data(ttl=120)
def magic_cross_row_hourly(symbol: str,
                           period: str,
                           slope_lb: int,
                           ntd_win: int = 60,
                           max_bars_since: int = 10,
    ...
    """
    Magic Cross (Hourly):
    Intraday 5m bars over selected period.
    NPX recently crossed ABOVE NTD (cross-up), and the cross occurs in the positive zone
    (between 0.0 and 1.0 on the NTD/NPX panel scale).
    """
    try:
        ...
        last_px = float(hc.iloc[-1]) if np.isfinite(hc.iloc[-1]) else np.nan
        npx_cross = float(npx.loc[bar]) if np.isfinite(npx.loc[bar]) else np.nan
        ntd_cross = float(ntd.loc[bar]) if np.isfinite(ntd.loc[bar]) else np.nan
        npx_last = float(npx.iloc[-1]) if np.isfinite(npx.iloc[-1]) else np.nan
        ntd_last = float(ntd.iloc[-1]) if np.isfinite(ntd.iloc[-1]) else np.nan

        frame_txt = f"Hourly ({frame_label})" if frame_label else f"Hourly ({period})"
...
with tab22:
    ...
    st.caption(
        "Existing lists:\n"
        "• **NTD -0.5 Cross**: regression slope > 0 and **NTD crossed up into [-0.5, -0.4]**.\n\n"
        "NEW lists (NTD Positive Cross, trendline UP):\n"
        "• **NTD 0.0 Cross Up**: **trendline is upward** and **NTD crossed up through 0.0**.\n"
        "• **NTD +0.5 Cross Up**: **trendline is upward** and **NTD crossed up through +0.5**.\n\n"
    ...
    run22 = st.button("Run NTD Cross Scan", key=f"btn_run_ntd05_{mode}", use_container_width=True)
    ...
    if run22:
        ...
        for sym in universe:
            ...
            if r:
                d_minus05_rows.append(r)

            r0 = ntd_zero_cross_row_daily(
                symbol=sym,
                daily_view_label=daily_view,
                ntd_win=60,
                max_bars_since=int(within_daily),
            ...
            if r0:
                d_zero_rows.append(r0)

            r5 = ntd_plus05_cross_row_daily(
                symbol=sym,
                daily_view_label=daily_view,
                ntd_win=60,
                max_bars_since=int(within_daily),
            ...
            if r5:
                d_plus05_rows.append(r5)

        st.subheader("Daily")
        cD1, cD2, cD3 = st.columns(3)

        with cD1:
            st.subheader("NTD -0.5 Cross (into [-0.5, -0.4])")
            if not d_minus05_rows:
                st.write("No matches.")
            else:
                df = pd.DataFrame(d_minus05_rows).sort_values(["Bars Since", "NTD(last)"], ascending=[True, False])
                st.dataframe(df.head(max_rows).reset_index(drop=True), use_container_width=True)

        with cD2:
            st.subheader("NTD 0.0 Cross Up (Trendline UP)")
            if not d_zero_rows:
                st.write("No matches.")
            else:
                df = pd.DataFrame(d_zero_rows).sort_values(["Bars Since", "NTD(last)", "Global Slope"], ascending=[True, False, False])
                st.dataframe(df.head(max_rows).reset_index(drop=True), use_container_width=True)

        with cD3:
            st.subheader("NTD +0.5 Cross Up (Trendline UP)")
            if not d_plus05_rows:
                st.write("No matches.")
            else:
                df = pd.DataFrame(d_plus05_rows).sort_values(["Bars Since", "NTD(last)", "Global Slope"], ascending=[True, False, False])
                st.dataframe(df.head(max_rows).reset_index(drop=True), use_container_width=True)
        ...
        for sym in universe:
            ...
            if r:
                h_minus05_rows.append(r)

            r0 = ntd_zero_cross_row_hourly(
                symbol=sym,
                period=period_map[hours],
                ntd_win=60,
                max_bars_since=int(within_hourly),
            ...
            if r0:
                h_zero_rows.append(r0)

            r5 = ntd_plus05_cross_row_hourly(
                symbol=sym,
                period=period_map[hours],
                ntd_win=60,
                max_bars_since=int(within_hourly),
        ...
        with cH1:
            st.subheader("NTD -0.5 Cross (into [-0.5, -0.4])")
            if not h_minus05_rows:
                st.write("No matches.")
            else:
                df = pd.DataFrame(h_minus05_rows).sort_values(["Bars Since", "NTD(last)"], ascending=[True, False])
                st.dataframe(df.head(max_rows).reset_index(drop=True), use_container_width=True)
        ...
        with cH2:
            ...
            if not h_zero_rows:
                ...
                df = pd.DataFrame(h_zero_rows).sort_values(["Bars Since", "NTD(last)", "Global Slope"], ascending=[True, False, False])
                st.dataframe(df.head(max_rows).reset_index(drop=True), use_container_width=True)

        with cH3:
            st.subheader("NTD +0.5 Cross Up (Trendline UP)")
            if not h_plus05_rows:
                st.write("No matches.")
            else:
                df = pd.DataFrame(h_plus05_rows).sort_values(["Bars Since", "NTD(last)", "Global Slope"], ascending=[True, False, False])
                st.dataframe(df.head(max_rows).reset_index(drop=True), use_container_width=True)
...
with tab23:
    ...
    tsa_hours = c3.selectbox("Hourly scan window", ["24h", "48h", "96h"], index=0, key=f"tsa_hr_win_{mode}")
    ...
    if run23:
        ...
        for sym in universe:
            ...
            cross_info = last_npx_ntd_cross_info_daily(sym, daily_view_label=daily_view, ntd_win=int(ntd_window))
            r2 = r.copy()
            r2.update(cross_info)
            r2["Frame"] = "Daily"
        ...
        for sym in universe:
            ...
            cross_info = last_npx_ntd_cross_info_hourly(sym, period=tsa_period, ntd_win=int(ntd_window))
            r2 = r.copy()
            r2.update(cross_info)
        ...
        with cL:
            ...
            if not buys:
                ...
                dfb = dfb.sort_values(["Bars Since Cross", "_score", "R2"], ascending=[True, False, False])
                st.dataframe(dfb[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)
        ...
        with cR:
            ...
            if not sells:
                ...
                dfs = dfs.sort_values(["Bars Since Cross", "_score", "R2"], ascending=[True, True, False])
                st.dataframe(dfs[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)
...
# =========================
# TAB 24 — NPX 0.0 Cross Trend/PTD
# =========================
with tab24:
...
# =========================
# TAB 28 — Regression Reversal
# =========================
with tab28:
    st.header("Regression Reversal")
    st.caption(
        "Daily + Hourly scanner:\n"
        "(a) **Trendline > 0** and **Regression Slope > 0** and price recently reversed **up** from **-2σ** or **Support**.\n"
        "(b) **Trendline < 0** and **Regression Slope < 0** and price recently reversed **down** from **+2σ** or **Resistance**.\n\n"
        "Outputs include the triggering source (Band or S/R), bars since, and slopes."
    )

    c1, c2, c3, c4, c5 = st.columns(5)
    within_daily = c1.selectbox("Daily: within N bars", [3, 5, 10, 15, 20], index=1, key=f"regrev_within_d_{mode}")
    within_hourly = c2.selectbox("Hourly: within N bars", [3, 5, 10, 15, 20], index=2, key=f"regrev_within_h_{mode}")
    hours = c3.selectbox("Hourly scan window", ["24h", "48h", "96h"], index=0, key=f"regrev_hrwin_{mode}")
    max_rows = c4.slider("Max rows per list", 10, 300, 60, 10, key=f"regrev_rows_{mode}")
    prox = c5.slider("S/R touch proximity (%)", 0.05, 1.00, 0.25, 0.05, key=f"regrev_prox_{mode}") / 100.0

    run28 = st.button("Run Regression Reversal Scan", key=f"btn_run_regrev_{mode}", use_container_width=True)

    if run28:
        show_cols = [
            "Symbol", "Frame", "Side", "Source",
            "Bars Since", "Signal Time",
        ...
        d_rows = []
        for sym in universe:
            r = regression_reversal_row_daily(
                symbol=sym,
                daily_view_label=daily_view,
                slope_lb=slope_lb_daily,
                max_bars_since=int(within_daily),
        ...
        hr_period = period_map[hours]
        for sym in universe:
            r = regression_reversal_row_hourly(
                symbol=sym,
                period=hr_period,
                slope_lb=slope_lb_hourly,
                max_bars_since=int(within_hourly),
...
# =========================
# TAB 29 — HMA Trade Signal  ✅ NEW
# =========================
with tab29:
    st.header("HMA Trade Signal")
    st.caption(
        "(a) **BUY**: Symbols where **HMA** has recently **crossed UP through Support** OR **reversed up from Support**, "
        "when the **trendline is upward**.\n"
        "(b) **SELL**: Symbols where **HMA** has recently **crossed DOWN through Resistance** OR **reversed down from Resistance**, "
        "when the **trendline is downward**.\n\n"
        "Daily uses selected Daily view range; Hourly uses 24/48/96 lookback."
    )

    c1, c2, c3, c4, c5 = st.columns(5)
    within_daily = c1.selectbox("Daily: within N bars", [3, 5, 10, 15, 20], index=1, key=f"hma_sig_within_d_{mode}")
    within_hourly = c2.selectbox("Hourly: within N bars", [3, 5, 10, 15, 20], index=2, key=f"hma_sig_within_h_{mode}")
    hours = c3.selectbox("Hourly scan window", ["24h", "48h", "96h"], index=0, key=f"hma_sig_hrwin_{mode}")
    max_rows = c4.slider("Max rows per list", 10, 300, 60, 10, key=f"hma_sig_rows_{mode}")
    _ = c5.empty()

    run_hma = st.button("Run HMA Trade Signal Scan", key=f"btn_run_hma_trade_signal_{mode}", use_container_width=True)

    if run_hma:
        show_cols = [
            "Symbol", "Frame", "Side", "Bars Since", "Signal Time", "Source",
            "Trendline Slope", "HMA(last)", "Support(last)", "Resistance(last)", "Last Price"
        ]

        # ---------- DAILY ----------
        d_buy, d_sell = [], []
        for sym in universe:
            r = hma_trade_signal_row_daily(
                symbol=sym,
                daily_view_label=daily_view,
                max_bars_since=int(within_daily),
                sr_lb=int(sr_lb_daily),
        ...
        with cL:
            st.subheader("Buy — HMA crossed/reversed at Support (Trendline UP)")
            if not d_buy:
                st.write("No matches.")
            else:
                dfb = pd.DataFrame(d_buy).sort_values(["Bars Since", "Trendline Slope"], ascending=[True, False])
                st.dataframe(dfb[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)

        with cR:
            st.subheader("Sell — HMA crossed/reversed at Resistance (Trendline DOWN)")
            if not d_sell:
                st.write("No matches.")
            else:
                dfs = pd.DataFrame(d_sell).sort_values(["Bars Since", "Trendline Slope"], ascending=[True, True])
                st.dataframe(dfs[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)
        ...
        with cL2:
            ...
            if not h_buy:
                ...
                dfb = pd.DataFrame(h_buy).sort_values(["Bars Since", "Trendline Slope"], ascending=[True, False])
                st.dataframe(dfb[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)
        ...
        with cR2:
            ...
            if not h_sell:
                ...
                dfs = pd.DataFrame(h_sell).sort_values(["Bars Since", "Trendline Slope"], ascending=[True, True])
                st.dataframe(dfs[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)
