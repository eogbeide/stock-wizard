...
def render_daily_enhanced_chart(sel: str, close_full: pd.Series, ohlc_full: pd.DataFrame):
    ...
    if show_bbands and not bb_up_d.dropna().empty and not bb_lo_d.dropna().empty:
        ax.fill_between(close_show.index, bb_lo_d, bb_up_d, alpha=0.06, label=f"BB (×{bb_mult:.1f})")
    ...
    ax.text(0.01, 0.02,
            f"Slope: {fmt_slope(m_d)}/bar  |  R²: {fmt_r2(r2_d)}  |  P(rev≤{rev_horizon}): {fmt_pct(rev_prob_d)}",
...
if st.sidebar.button("▶ Run / Refresh", use_container_width=True, key="btn_run_all"):
...
if not st.session_state.get("run_all", False) or (st.session_state.get("ticker") is None):
    ...
    with tabs[0]:
        ...
        if close_full.empty:
            ...
            st.write(f"**Symbol:** {sel}  •  **Mode:** {mode}")
            st.write("")
    ...
    with tabs[5]:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("### 📈 Retreated UP from Support (from below → above)")
            if sup_rows:
        ...
        with c2:
            st.markdown("### 📉 Retreated DOWN from Resistance (from above → below)")
            if res_rows:
