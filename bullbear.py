...
# ---------------------------
# NEW (THIS REQUEST): R² sign + ±2σ proximity scanner helper (Daily only)
#   - Produces proximity to regression ±2σ band endpoints on the most recent bar
#   - R² can be negative; split lists by R² > 0 vs R² < 0 (as requested)
#   - "Near" uses the existing sidebar S/R proximity (%) (sr_prox_pct)
# ---------------------------
@st.cache_data(ttl=120)
def daily_r2_band_proximity(symbol: str, daily_view_label: str, slope_lb: int, prox: float):
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if close_full.empty:
            return None
        ...
        if yhat is None or up is None or lo is None:
            return None
        if not (len(_coerce_1d_series(yhat).dropna()) and len(_coerce_1d_series(up).dropna()) and len(_coerce_1d_series(lo).dropna())):
            return None

        px = float(close_show.iloc[-1]) if np.isfinite(close_show.iloc[-1]) else np.nan
        up_last = float(_coerce_1d_series(up).dropna().iloc[-1]) if len(_coerce_1d_series(up).dropna()) else np.nan
        lo_last = float(_coerce_1d_series(lo).dropna().iloc[-1]) if len(_coerce_1d_series(lo).dropna()) else np.nan
        ts = close_show.index[-1] if isinstance(close_show.index, pd.DatetimeIndex) and len(close_show.index) else None

        if not np.isfinite(px) or not np.isfinite(up_last) or not np.isfinite(lo_last):
            return None
        ...
        def _near(a: float, b: float, pct: float) -> bool:
            denom = abs(b) if np.isfinite(b) and abs(b) > 0 else abs(a) if np.isfinite(a) and abs(a) > 0 else 1.0
            return bool(abs(a - b) <= denom * pct)

        near_lower = _near(px, lo_last, prox)
        near_upper = _near(px, up_last, prox)

        dist_lo_pct = (abs(px - lo_last) / (abs(lo_last) if abs(lo_last) > 0 else abs(px) if abs(px) > 0 else 1.0))
        dist_up_pct = (abs(px - up_last) / (abs(up_last) if abs(up_last) > 0 else abs(px) if abs(px) > 0 else 1.0))

        return {
            "Symbol": symbol,
            "AsOf": ts,
            "Price": float(px),
            "Lower 2σ": float(lo_last),
            "Upper 2σ": float(up_last),
            "AbsDist Lower (%)": float(dist_lo_pct),
            "AbsDist Upper (%)": float(dist_up_pct),
            "Slope": float(m) if np.isfinite(m) else np.nan,
            "R2": float(r2) if np.isfinite(r2) else np.nan,
...
# UPDATED (THIS REQUEST): add NEW tab for R² sign + ±2σ band proximity (Daily only), without changing existing tabs
...
# NOTE: Tabs 1–17 are unchanged below; Tab 18 is appended at the end.
