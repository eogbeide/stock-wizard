...
def overlay_hma_reversal_on_ntd(ax, price: pd.Series, hma: pd.Series,
    ...
    if ntd is not None:
        ntd = _coerce_1d_series(ntd).reindex(buy_rev.index)
        buy_rev = (buy_rev & (ntd <= lower_thr)).fillna(False)
        sell_rev = (sell_rev & (ntd >= upper_thr)).fillna(False)
        idx_up = list(buy_rev[buy_rev].index)
        idx_dn = list(sell_rev[sell_rev].index)
    if len(idx_up):
        ax.scatter(idx_up, [y_up]*len(idx_up), marker="s", s=70, color="tab:green", zorder=8,
                   label=f"HMA({period}) BUY opp")
    if len(idx_dn):
        ax.scatter(idx_dn, [y_dn]*len(idx_dn), marker="D", s=70, color="tab:red",   zorder=8,
                   label=f"HMA({period}) SELL opp")

# NPX ↔ NTD overlay/helpers
def _ntd_buy_sell_opportunity_masks(ntd: pd.Series,
                                    buy_level: float = -0.75,
                                    sell_level: float = 0.75):
    """
    NTD opportunity-only signal masks:
      - BUY opportunity: NTD crosses upward through the lower buy_level.
      - SELL opportunity: NTD crosses downward through the upper sell_level.
    These masks intentionally ignore ordinary mid-range crosses to reduce noise.
    """
    s = _coerce_1d_series(ntd).dropna()
    if s.shape[0] < 2:
        idx = _coerce_1d_series(ntd).index if ntd is not None else pd.Index([])
        return pd.Series(False, index=idx), pd.Series(False, index=idx)

    buy_opp = (s.shift(1) < buy_level) & (s >= buy_level) & (s.diff() > 0)
    sell_opp = (s.shift(1) > sell_level) & (s <= sell_level) & (s.diff() < 0)
    return buy_opp.reindex(s.index, fill_value=False), sell_opp.reindex(s.index, fill_value=False)
...
def overlay_npx_on_ntd(ax, npx: pd.Series, ntd: pd.Series, mark_crosses: bool = True):
    ...
    # Only show green/red NPX↔NTD cross markers when the cross happens inside an actionable NTD zone.
    # This prevents ordinary mid-range crosses from looking like buy/sell opportunities.
    if mark_crosses and not ntd.dropna().empty:
        up_mask, dn_mask = _cross_series(npx, ntd)
        buy_zone = (ntd <= -0.50) | (npx <= -0.50)
        sell_zone = (ntd >= 0.50) | (npx >= 0.50)

        buy_idx = list((up_mask.reindex(idx, fill_value=False) & buy_zone.fillna(False)).loc[lambda x: x].index)
        sell_idx = list((dn_mask.reindex(idx, fill_value=False) & sell_zone.fillna(False)).loc[lambda x: x].index)

        if len(buy_idx):
            ax.scatter(buy_idx, ntd.loc[buy_idx], marker="^", s=65, color="tab:green", zorder=9, label="NTD BUY opportunity")
        if len(sell_idx):
            ax.scatter(sell_idx, ntd.loc[sell_idx], marker="v", s=65, color="tab:red", zorder=9, label="NTD SELL opportunity")

# NTD opportunity triangles
def overlay_ntd_triangles_by_trend(ax, ntd: pd.Series, trend_slope: float = None,
                                   upper: float = 0.75, lower: float = -0.75):
    """
    Draw green/red triangles only at true NTD buy/sell opportunities:
      - Green ▲ BUY opportunity when NTD crosses upward through the lower level.
      - Red ▼ SELL opportunity when NTD crosses downward through the upper level.

    The trend_slope argument is kept for backward compatibility with existing calls,
    but the triangles are now based on NTD opportunity crosses instead of every trend-gated cross.
    """
    s = _coerce_1d_series(ntd).dropna()
    if s.empty:
        return
    buy_opp, sell_opp = _ntd_buy_sell_opportunity_masks(s, buy_level=lower, sell_level=upper)

    idx_buy = list(buy_opp[buy_opp].index)
    idx_sell = list(sell_opp[sell_opp].index)

    if idx_buy:
        ax.scatter(idx_buy, s.loc[idx_buy], marker="^", s=95, color="tab:green",
                   zorder=10, label=f"NTD BUY opp ({lower:+.2f}↑)")
    if idx_sell:
        ax.scatter(idx_sell, s.loc[idx_sell], marker="v", s=95, color="tab:red",
                   zorder=10, label=f"NTD SELL opp ({upper:+.2f}↓)")
