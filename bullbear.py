...
# =========================
# BATCH 2 (6–10) — CHANGES REQUESTED IN THIS "continue"
#   (2) Keep a slider (b)
#   (2) Add S/R(b) dotted lines (light blue above, dark blue below)
#       to BOTH Daily + Hourly PRICE charts
#
# NOTE: One small required insert for the NEW slider (b) is in Part 2/10 (sidebar).
#       Everything else below is in Parts 6–10.
# =========================


# ==========================================================
# (A) REQUIRED INSERT (Part 2/10 — Sidebar)
#   FIND this block:
#       st.sidebar.subheader("Hourly Support/Resistance Window")
#       sr_lb_hourly = st.sidebar.slider("Hourly S/R lookback (bars)", 20, 240, 60, 5, key="sb_sr_lb_hourly")
#
#   ADD this slider IMMEDIATELY AFTER it:
# ==========================================================
sr_lb_b = st.sidebar.slider(
    "Secondary S/R (b) lookback (bars)",
    20, 240, 60, 5,
    key="sb_sr_lb_b"
)
...
# ==========================================================
# (B) Part 9/10 — Tabs label rename to match request (IK Cross)
#   FIND:
#       ..."Uptrend vs Downtrend",
#       "Ichimoku Kijun Scanner"
#   REPLACE ONLY the LAST TAB LABEL with:
#       "IK Cross"
# ==========================================================
...
# ==========================================================
# (C) Part 8/10 — render_hourly_views(): ADD secondary S/R(b) series + dotted lines
#
#   1) FIND in render_hourly_views() right after:
#       res_h = hc.rolling(sr_lb_hourly, min_periods=1).max()
#       sup_h = hc.rolling(sr_lb_hourly, min_periods=1).min()
#
#      ADD:
# ==========================================================
res_b_h = hc.rolling(sr_lb_b, min_periods=1).max()
sup_b_h = hc.rolling(sr_lb_b, min_periods=1).min()


#   2) FIND later where PRIMARY hourly S/R are drawn:
#       if np.isfinite(res_val) and np.isfinite(sup_val):
#           ax2.hlines(res_val, ...)
#           ax2.hlines(sup_val, ...)
#           label_on_left(...)
#
#      ADD IMMEDIATELY AFTER that whole primary S/R block:
# ==========================================================
res_b_val = sup_b_val = np.nan
try:
    res_b_val = float(res_b_h.iloc[-1])
    sup_b_val = float(sup_b_h.iloc[-1])
except Exception:
    pass

if np.isfinite(res_b_val) and np.isfinite(sup_b_val):
    ax2.hlines(
        res_b_val, xmin=hc.index[0], xmax=hc.index[-1],
        colors="lightskyblue", linestyles="dotted", linewidth=1.6,
        label=f"Resistance (b w={sr_lb_b})"
    )
    ax2.hlines(
        sup_b_val, xmin=hc.index[0], xmax=hc.index[-1],
        colors="navy", linestyles="dotted", linewidth=1.6,
        label=f"Support (b w={sr_lb_b})"
    )



# ==========================================================
# (D) Part 9/10 — TAB 1 (Daily price chart): ADD secondary S/R(b) series + dotted lines
#
#   1) FIND in Tab 1 daily section, right after:
#       res_d = df.rolling(sr_lb_daily, min_periods=1).max()
#       sup_d = df.rolling(sr_lb_daily, min_periods=1).min()
#
#      ADD:
# ==========================================================
res_b_d = df.rolling(sr_lb_b, min_periods=1).max()
sup_b_d = df.rolling(sr_lb_b, min_periods=1).min()


#   2) FIND where show-series are created:
#       res_d_show = res_d.reindex(df_show.index)
#       sup_d_show = sup_d.reindex(df_show.index)
#
#      ADD:
# ==========================================================
res_b_d_show = res_b_d.reindex(df_show.index)
sup_b_d_show = sup_b_d.reindex(df_show.index)


#   3) FIND where PRIMARY daily S/R lines are drawn:
#       if np.isfinite(res_val_d) and np.isfinite(sup_val_d):
#           ax.hlines(... tab:red ...)
#           ax.hlines(... tab:green ...)
#           label_on_left(...)
#
#      ADD IMMEDIATELY AFTER that primary S/R block:
# ==========================================================
res_b_val_d = sup_b_val_d = np.nan
try:
    res_b_val_d = float(res_b_d_show.iloc[-1])
    sup_b_val_d = float(sup_b_d_show.iloc[-1])
except Exception:
    pass

if np.isfinite(res_b_val_d) and np.isfinite(sup_b_val_d) and len(df_show.index) > 0:
    ax.hlines(
        res_b_val_d, xmin=df_show.index[0], xmax=df_show.index[-1],
        colors="lightskyblue", linestyles="dotted", linewidth=1.6,
        label=f"Resistance (b w={sr_lb_b})"
    )
    ax.hlines(
        sup_b_val_d, xmin=df_show.index[0], xmax=df_show.index[-1],
        colors="navy", linestyles="dotted", linewidth=1.6,
        label=f"Support (b w={sr_lb_b})"
    )
...
# ==========================================================
# (E) Part 10/10 — TAB 2 (Enhanced Forecast Daily chart): ADD secondary S/R(b) dotted lines
#
#   FIND in Tab 2 daily section after:
#       res_d_show = df_show.rolling(sr_lb_daily, min_periods=1).max()
#       sup_d_show = df_show.rolling(sr_lb_daily, min_periods=1).min()
#
#   ADD:
# ==========================================================
res_b2_show = df_show.rolling(sr_lb_b, min_periods=1).max()
sup_b2_show = df_show.rolling(sr_lb_b, min_periods=1).min()


#   FIND where PRIMARY S/R hlines are drawn in Tab 2:
#       if not res_d_show.empty and not sup_d_show.empty:
#           ax.hlines(float(res_d_show.iloc[-1]) ... tab:red ...)
#           ax.hlines(float(sup_d_show.iloc[-1]) ... tab:green ...)
#
#   ADD IMMEDIATELY AFTER that block:
# ==========================================================
try:
    r_b2 = float(res_b2_show.iloc[-1]) if not res_b2_show.empty else np.nan
    s_b2 = float(sup_b2_show.iloc[-1]) if not sup_b2_show.empty else np.nan
except Exception:
    r_b2 = np.nan
    s_b2 = np.nan

if np.isfinite(r_b2) and np.isfinite(s_b2) and len(df_show.index) > 0:
    ax.hlines(
        r_b2, xmin=df_show.index[0], xmax=df_show.index[-1],
        colors="lightskyblue", linestyles="dotted", linewidth=1.6,
        label=f"Resistance (b w={sr_lb_b})"
    )
    ax.hlines(
        s_b2, xmin=df_show.index[0], xmax=df_show.index[-1],
        colors="navy", linestyles="dotted", linewidth=1.6,
        label=f"Support (b w={sr_lb_b})"
    )
...
# ==========================================================
# (F) Part 10/10 — TAB 15 header text (rename to match IK Cross)
#   FIND:
#       with tab15:
...
with tab15:
    st.header("IK Cross")
    st.caption(
        "Daily-only list (matches the **Price Chart**):\n"
        "• Symbols where price **just crossed ABOVE** the **Ichimoku Kijun (26)** line, **heading up**.\n\n"
        "Split into:\n"
        "• **List 1:** regression slope **> 0**\n"
        "• **List 2:** regression slope **< 0**\n\n"
        "Includes **Price@Cross**, **Kijun@Cross**, and **R²**."
    )
    # (leave the rest of the tab logic exactly as-is)
