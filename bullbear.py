...
# =========================
# UPDATED (THIS REQUEST #1):
# Global trendline hidden by default + optional checkbox
# =========================
show_global_trendline = st.sidebar.checkbox("Show Global Trendline", value=False, key="sb_show_global_trendline")
...
# =========================
# UPDATED (THIS REQUEST #1):
# - Compute global slope always
# - Only PLOT the global trendline when checkbox is enabled
# =========================
...
# UPDATED (THIS REQUEST): global trendline display flag (checkbox lives in sidebar in Batch 1)
show_global_trendline = bool(st.session_state.get("sb_show_global_trendline", False))
...
def render_hourly_views(sel: str,
    ...
    # UPDATED (THIS REQUEST): global trendline hidden by default; slope still computed for logic
    global_m_h = draw_trend_direction_line(ax2, hc, label_prefix="Trend (global)", show=show_global_trendline)
...
with tab1:
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        if chart in ("Daily", "Both"):
            ...
            # UPDATED: global trendline hidden by default; slope still computed for logic
            global_m_d = draw_trend_direction_line(ax, df_show, label_prefix="Trend (global)", show=show_global_trendline)
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        if view in ("Daily", "Both"):
            ...
            if show_fibs and len(df_show) > 0:
                fibs_d = fibonacci_levels(df_show)
                if fibs_d:
                    x0, x1 = df_show.index[0], df_show.index[-]()
