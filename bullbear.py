...
# Definition per user: "support crossed" = BELOW only, support line going UP
# Here: price CROSSED DOWN below support recently AND is currently below support, while support slope > 0
# ---------------------------
@st.cache_data(ttl=120)
def enhanced_support_crossed_candidate(symbol: str,
    ...
    try:
        ...
        c_last = float(close.iloc[-1]) if np.isfinite(close.iloc[-1]) else np.nan
        s_last = float(sup.iloc[-1]) if np.isfinite(sup.iloc[-1]) else np.nan
        if not np.all(np.isfinite([c_last, s_last])):
            return None
        ...
        # Cross DOWN through support: was above support, now at/below support
        _, cross_dn_sup = _cross_series(close, sup)
        cross_dn_sup = cross_dn_sup.reindex(close.index, fill_value=False)

        t_cross = cross_dn_sup[cross_dn_sup].index[-1] if cross_dn_sup.any() else None
        if t_cross is None:
            return None
        ...
        recent_bars = max(0, int(recent_bars))
        if bars_since > recent_bars:
            return None

        below_now = (np.isfinite(c_last) and np.isfinite(s_last) and (c_last < s_last))
        if not below_now:
            return None

        # "going upward" per user = support line slope > 0
        if not (np.isfinite(sup_slope) and float(sup_slope) > 0.0):
            return None
        ...
        return {
            ...
            "Close": c_last,
            "Support": s_last,
            "Support Slope": float(sup_slope) if np.isfinite(sup_slope) else np.nan,
            "Dist vs Support": dist_sup_pct,
            "Cross Time": t_cross,
            "Bars Since Cross": bars_since,
        }
...
# UPDATED (THIS REQUEST): add new tab "Enhanced Support Crossed" (definition: below only + support slope up)
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        if view in ("Daily", "Both"):
            ...
            # UPDATED (THIS REQUEST): show Support/Resistance VALUES on the Enhanced Forecast chart
            res_val = sup_val = np.nan
            try:
                res_val = float(res_d_show.iloc[-1]) if len(res_d_show) else np.nan
                sup_val = float(sup_d_show.iloc[-1]) if len(sup_d_show) else np.nan
            except Exception:
                pass

            if np.isfinite(res_val) and np.isfinite(sup_val) and len(df_show.index) > 0:
                ax.hlines(res_val, xmin=df_show.index[0], xmax=df_show.index[-1],
                          colors="tab:red", linestyles="-", linewidth=1.6, label="Resistance")
                ax.hlines(sup_val, xmin=df_show.index[0], xmax=df_show.index[-1],
...
with tab16:
    ...
    st.caption(
        "Definition (per request): **Support crossed** = price is **below** the Enhanced (Daily) support line, "
        "and the support line is **going upward**.\n\n"
        "This tab lists symbols where price **crossed DOWN below** the Enhanced Daily support line recently, "
        "is **still below** it now, and the **support slope is > 0**."
    )

    c1, c2 = st.columns(2)
    recent_bars_sc = c1.slider("Recent cross window (bars)", 0, 20, 3, 1, key=f"enh_supcross_recent_{mode}")
    sr_slope_lb_sc = c2.slider("Support slope lookback (bars)", 5, 60, min(20, int(sr_lb_daily)), 5, key=f"enh_supcross_slope_{mode}")

    run_sc = st.button("Run Enhanced Support Crossed Scan", key=f"btn_run_enh_supcross_{mode}")
    ...
    if run_sc:
        ...
        for sym in universe:
            r = enhanced_support_crossed_candidate(
                ...
                daily_view_label=daily_view,
                sr_lb=sr_lb_daily,
                recent_bars=int(recent_bars_sc),
                sr_slope_lb=int(sr_slope_lb_sc),
            )
        ...
        if not rows:
            ...
            out = pd.DataFrame(rows)
            for c in ["Bars Since Cross", "Support Slope", "Dist vs Support"]:
                if c in out.columns:
