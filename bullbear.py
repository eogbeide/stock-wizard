...
# =========================
# NEW (THIS REQUEST): HMA(55) + Support Cross-Up scanner helper (Daily / for Enhanced Forecast list)
#   - price > HMA(55)
#   - crossed ABOVE HMA(55) recently and heading up
#   - crossed ABOVE the Support line recently and heading up
#   - Support uses prior rolling-min (shifted) so a true "cross up" can occur
# =========================
@st.cache_data(ttl=120)
def last_daily_hma_support_cross_up(symbol: str,
                                   daily_view_label: str,
                                   sr_lb: int,
                                   hma_period: int,
                                   within_last_n_bars: int = 5,
    ...
    try:
        ...
        # "Support line" consistent with chart concept, but shifted so a true cross-up can happen.
        sup = close.rolling(int(sr_lb), min_periods=1).min().shift(1)

        ok = close.notna() & hma.notna()
        close = close[ok]
        hma = hma.reindex(close.index)
        sup = _coerce_1d_series(sup).reindex(close.index)

        if len(close) < 3:
            return None

        # Cross up through HMA
        hma_cross_up = (close >= hma) & (close.shift(1) < hma.shift(1))
        hma_cross_up = hma_cross_up.fillna(False)

        # Cross up through Support (only where support exists)
        sup_ok = sup.notna() & sup.shift(1).notna()
        sup_cross_up = sup_ok & (close >= sup) & (close.shift(1) < sup.shift(1))
        sup_cross_up = sup_cross_up.fillna(False)

        if not hma_cross_up.any() or not sup_cross_up.any():
            return None

        t_hma = hma_cross_up[hma_cross_up].index[-1]
        ...
        bs_hma = _bars_since(t_hma)
        bs_sup = _bars_since(t_sup)
        if bs_hma is None or bs_sup is None:
            return None

        within_last_n_bars = max(0, int(within_last_n_bars))
        if bs_hma > within_last_n_bars or bs_sup > within_last_n_bars:
            return None

        confirm_bars = max(1, int(confirm_bars))

        # "Heading up" confirmation
        price_heading_up = _n_consecutive_increasing(close, confirm_bars)
        hma_heading_up = _series_heading_up(hma, confirm_bars=confirm_bars)
        ...
        if not (price_heading_up and hma_heading_up):
            return None

        # Small extra: make sure after the support cross the price didn't immediately fall back under it
        # (still "heading up" / above support now)
        if np.isfinite(s_last) and not (c_last >= s_last):
            return None

        return {
            "Symbol": symbol,
            "Daily View": daily_view_label,
            "Bars Since HMA Cross": int(bs_hma),
            "Bars Since Support Cross": int(bs_sup),
...
# UPDATED: last tab name must be "IK Cross"
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        # =========================
        # NEW (THIS REQUEST): List of symbols where:
        #   - price > HMA(55) (crossed and heading up)
        #   - price recently crossed SUPPORT line going up
        # Displayed on Enhanced Forecast (Daily logic).
        # =========================
        st.subheader("HMA(55) + Support Cross-Up List (Daily)")
        st.caption(
            "Lists symbols where **price is above HMA(55)** and is **heading up**, AND price has **recently crossed above the Support line** (daily) going up. "
            "Support cross uses a shifted rolling-min so a true cross-up can be detected."
        )
        c1, c2, c3 = st.columns(3)
        hma_sup_within = c1.slider("Cross must be within last N bars", 0, 60, 5, 1, key=f"hma_sup_within_{mode}")
        hma_sup_confirm = c2.slider("Heading-up confirmation (consecutive bars)", 1, 5, 1, 1, key=f"hma_sup_confirm_{mode}")
        run_hma_sup = c3.button("Run HMA55 + Support Cross Scan", key=f"btn_run_hma_sup_scan_{mode}")

        if run_hma_sup:
            rows = []
            for sym in universe:
                r = last_daily_hma_support_cross_up(
                    symbol=sym,
                    daily_view_label=daily_view,
                    sr_lb=int(sr_lb_daily),
                    hma_period=int(hma_period),
                    within_last_n_bars=int(hma_sup_within),
                    confirm_bars=int(hma_sup_confirm),
                )
                if r is not None:
                    rows.append(r)
            ...
            if not rows:
                ...
                out = out.sort_values(["Bars Since Support Cross", "Bars Since HMA Cross"], ascending=[True, True])
