...
def plot_price_chart(ax,
                     ...
                     news_times=None):
    """
    Price chart for DAILY view. Uses OHLC when overlays need it (Ichimoku, Supertrend, PSAR, BBands).
    """
    c = _coerce_1d_series(close).dropna()
    if c.empty:
    ...
    ax.plot(c.index, c.values, linewidth=1.6, label="Close")

    # Moving averages
    if show_sma50:
        sma50 = c.rolling(50, min_periods=1).mean()
        ax.plot(sma50.index, sma50.values, linewidth=1.1, alpha=0.9, label="SMA 50")
    if show_sma200:
        sma200 = c.rolling(200, min_periods=1).mean()
        ax.plot(sma200.index, sma200.values, linewidth=1.0, alpha=0.85, label="SMA 200")
    if show_ema21:
        ema21 = c.ewm(span=21, adjust=False).mean()
        ax.plot(ema21.index, ema21.values, linewidth=1.0, alpha=0.85, label="EMA 21")
    ...
    if show_reg_band:
        ...
        if not yhat.dropna().empty:
            ax.plot(yhat.index, yhat.values, linewidth=1.2, alpha=0.9, label=f"Reg (lb={slope_lb})")
        if not up.dropna().empty:
            ax.plot(up.index, up.values, "--", linewidth=1.0, alpha=0.7, label="+2σ")
        if not lo.dropna().empty:
            ax.plot(lo.index, lo.values, "--", linewidth=1.0, alpha=0.7, label="-2σ")
        band = {"yhat": yhat, "up": up, "lo": lo, "slope": m, "r2": r2}

    # BBands (optional)
    if show_bbands and (ohlc is not None) and (not ohlc.empty) and {"Close"}.issubset(ohlc.columns):
        bb_mid = ohlc["Close"].rolling(20, min_periods=1).mean()
        bb_std = ohlc["Close"].rolling(20, min_periods=1).std(ddof=0)
        bb_u = bb_mid + 2 * bb_std
        bb_l = bb_mid - 2 * bb_std
        ax.plot(bb_mid.index, bb_mid.values, linewidth=0.9, alpha=0.7, label="BB mid")
        ax.plot(bb_u.index, bb_u.values, linewidth=0.9, alpha=0.6, label="BB +2")
        ax.plot(bb_l.index, bb_l.values, linewidth=0.9, alpha=0.6, label="BB -2")

    # Ichimoku (no forward shift on daily overlay)
    if show_ichimoku and (ohlc is not None) and (not ohlc.empty) and {"High","Low","Close"}.issubset(ohlc.columns):
        tenkan, kijun, sen_a, sen_b, chikou = ichimoku_lines(
            ohlc["High"], ohlc["Low"], ohlc["Close"],
            conv=9, base=26, span_b=52, shift_cloud=False
        )
        tenkan = tenkan.reindex(c.index)
        kijun  = kijun.reindex(c.index)
    ...
    # Supertrend (default ON for DAILY price chart)
    if show_supertrend and (ohlc is not None) and (not ohlc.empty) and {"High","Low","Close"}.issubset(ohlc.columns):
        st = compute_supertrend(ohlc[["High","Low","Close"]].reindex(c.index), atr_period=int(st_atr_period), atr_mult=float(st_mult))
        if not st.empty:
            st_line = _coerce_1d_series(st["ST"]).reindex(c.index)
            ax.plot(st_line.index, st_line.values, linewidth=1.2, alpha=0.9, label=f"Supertrend ({st_atr_period},{st_mult:g})")

    # PSAR
    if show_psar and (ohlc is not None) and (not ohlc.empty) and {"High","Low"}.issubset(ohlc.columns):
        ps = compute_psar_from_ohlc(ohlc[["High","Low"]].reindex(c.index), step=float(psar_step), max_step=float(psar_max))
        if not ps.empty:
            psar = _coerce_1d_series(ps["PSAR"]).reindex(c.index)
            ax.scatter(psar.index, psar.values, s=14, alpha=0.8, label="PSAR")
    ...
    if show_fibs:
        ...
        if fibs:
            for k, v in fibs.items():
                if np.isfinite(v):
                    ax.axhline(v, linewidth=0.8, alpha=0.25)
            ax.plot([], [], linewidth=0, label="Fib levels")

    # Support/Resistance rolling bands
    sup = res = None
    if show_sr:
        sup = c.rolling(int(sr_lb), min_periods=1).min()
        res = c.rolling(int(sr_lb), min_periods=1).max()
        ax.plot(sup.index, sup.values, linewidth=1.0, alpha=0.8, label=f"Support (lb={sr_lb})")
        ax.plot(res.index, res.values, linewidth=1.0, alpha=0.8, label=f"Resistance (lb={sr_lb})")

        # Proximity shading (same sr_prox slider)
        last_t = c.index[-1]
        if last_t in sup.index and last_t in res.index and np.isfinite(sup.loc[last_t]) and np.isfinite(res.loc[last_t]):
            s0 = float(sup.loc[last_t])
            r0 = float(res.loc[last_t])
            if s0 > 0:
                ax.axhspan(s0, s0*(1+sr_prox), alpha=0.08)
            if r0 > 0:
                ax.axhspan(r0*(1-sr_prox), r0, alpha=0.08)

    # News markers
    if show_news and news_times:
        draw_news_markers(ax, news_times, label="News")

    ax.grid(True, alpha=0.25)
    ax.legend(loc="best", fontsize=8)
    return {"band": band, "sup": sup, "res": res}
...
def plot_ntd_panel(ax,
                   ...
                   show_sr_reversal_stars: bool = True):
    c = _coerce_1d_series(close).dropna()
    if c.empty:
        ax.text(0.5, 0.5, "No data", transform=ax.transAxes, ha="center", va="center")
        ax.set_axis_off()
    ...
    # Trend slope on price for determining uptrend/downtrend context
    yhat, up, lo, m, r2 = regression_with_band(c, lookback=int(slope_lb))
    trend_slope = float(m) if np.isfinite(m) else np.nan

    ax.set_title(f"NTD (window={ntd_win})", fontsize=11, fontweight="bold")
    ax.plot(ntd.index, ntd.values, linewidth=1.5, label="NTD")
    ax.axhline(0.0, linewidth=1.0, alpha=0.35)

    # Standard reference zones
    ax.axhline(0.75, linewidth=0.8, alpha=0.25)
    ax.axhline(-0.75, linewidth=0.8, alpha=0.25)

    # Optional: NPX (normalized price) + crosses vs NTD
    if show_npx:
    ...
    # HMA reversal markers
    if show_hma_rev:
        hma = hull_moving_average(c, period=int(hma_period))
        hma = _coerce_1d_series(hma).reindex(c.index)
        overlay_hma_reversal_on_ntd(ax, price=c, hma=hma, lookback=int(hma_rev_lb), period=int(hma_period), ntd=ntd)

    # Triangles based on trend
    if show_triangles and np.isfinite(trend_slope):
        overlay_ntd_triangles_by_trend(ax, ntd=ntd, trend_slope=trend_slope, upper=0.75, lower=-0.75)

    # In-range shading + stars (support/resistance reversal on NTD)
    if (sup is not None) and (res is not None):
        overlay_inrange_on_ntd(ax, price=c, sup=sup, res=res)

        if show_sr_reversal_stars and np.isfinite(trend_slope):
            overlay_ntd_sr_reversal_stars(
                ax,
                price=c, sup=sup, res=res,
    ...
    ax.grid(True, alpha=0.25)
    ax.legend(loc="best", fontsize=8)
    return {"ntd": ntd, "trend_slope": trend_slope, "r2": float(r2) if np.isfinite(r2) else np.nan}
...
# =========================
# ---------------------------
# Scanner utilities (Slope Direction + ±2σ proximity using SAME S/R proximity slider)
# ---------------------------
def _band_close_to(series_close: pd.Series, band_series: pd.Series, prox: float) -> bool:
    c = _coerce_1d_series(series_close).dropna()
    b = _coerce_1d_series(band_series).reindex(c.index)
    if c.empty or b.dropna().empty:
        return False
    x = float(c.iloc[-1])
    y = float(b.iloc[-1])
    if not (np.isfinite(x) and np.isfinite(y) and y != 0):
        return False
    return bool(abs(x / y - 1.0) <= float(prox))

@st.cache_data(ttl=120)
def daily_close_near_plus2sigma(symbol: str, slope_lb: int, prox: float):
    try:
        close = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if close.empty or len(close) < max(10, int(slope_lb)):
            return None
        yhat, up, lo, m, r2 = regression_with_band(close, lookback=int(slope_lb))
        if up.dropna().empty:
            return None
        if not _band_close_to(close, up, prox=float(prox)):
            return None
        return {
            "Symbol": symbol,
            "Frame": "Daily",
            ...
            "+2σ": float(_coerce_1d_series(up).reindex(close.index).iloc[-1]),
            "Slope": float(m) if np.isfinite(m) else np.nan,
            "R2": float(r2) if np.isfinite(r2) else np.nan,
            "AsOf": close.index[-1],
        }
...
@st.cache_data(ttl=120)
def daily_close_near_minus2sigma(symbol: str, slope_lb: int, prox: float):
    try:
        close = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if close.empty or len(close) < max(10, int(slope_lb)):
            return None
        ...
        if lo.dropna().empty:
            return None
        if not _band_close_to(close, lo, prox=float(prox)):
            return None
        return {
            "Symbol": symbol,
            "Frame": "Daily",
            "Where": "Close ~ -2σ",
            "Close": float(close.iloc[-1]),
            "-2σ": float(_coerce_1d_series(lo).reindex(close.index).iloc[-1]),
            "Slope": float(m) if np.isfinite(m) else np.nan,
            "R2": float(r2) if np.isfinite(r2) else np.nan,
            "AsOf": close.index[-1],
        }
...
@st.cache_data(ttl=120)
def hourly_close_near_plus2sigma(symbol: str, period: str, slope_lb: int, prox: float):
    try:
        df = fetch_intraday(symbol, period=period)
        if df is None or df.empty or "Close" not in df.columns:
            return None
...
@st.cache_data(ttl=120)
def hourly_close_near_minus2sigma(symbol: str, period: str, slope_lb: int, prox: float):
    try:
        df = fetch_intraday(symbol, period=period)
        if df is None or df.empty or "Close" not in df.columns:
            return None
...
def main():
    st.set_page_config(page_title="BullBear — Regression Bands + NTD", layout="wide")

    st.title("BullBear — Regression Bands + NTD")

    with st.sidebar:
        st.header("Inputs")
        symbol = st.text_input("Ticker / Symbol", value="SPY").strip().upper()
        daily_view_label = st.selectbox("Daily View", ["6M", "1Y", "2Y", "5Y", "MAX"], index=1)
        hourly_period = st.selectbox("Intraday Period", ["1d", "5d", "1mo", "3mo", "6mo"], index=0)

        st.divider()
        st.header("Core Params")
        slope_lb = st.slider("Regression lookback (bars)", 20, 300, 120, step=5)
        ntd_win  = st.slider("NTD window", 10, 200, 55, step=1)
        sr_lb    = st.slider("S/R rolling lookback", 10, 250, 50, step=5)

        # EXISTING slider — reused for “close to ±2σ” proximity (no extra UI, per request)
        sr_prox_pct = st.slider("S/R proximity (%)", 0.05, 2.00, 0.25, step=0.05)
        sr_prox = float(sr_prox_pct) / 100.0

        st.divider()
        st.header("Daily Overlays")
        show_reg_band = st.checkbox("Regression ±2σ band", value=True)
        show_ichimoku = st.checkbox("Ichimoku (Tenkan/Kijun)", value=True)
        show_supertrend = st.checkbox("Supertrend (Daily)", value=True)  # default ON
        st_atr_period = st.slider("Supertrend ATR period", 5, 30, 10, step=1)
        st_mult = st.slider("Supertrend multiplier", 1.0, 6.0, 3.0, step=0.25)
        show_psar = st.checkbox("PSAR (Daily)", value=False)

        st.divider()
        st.header("NTD Panel")
        show_npx = st.checkbox("Show NPX on NTD panel", value=True)
        show_hma_rev = st.checkbox("Show HMA reversals", value=True)
        hma_period = st.slider("HMA period", 20, 200, 55, step=1)
        hma_rev_lb = st.slider("HMA reversal slope lookback", 2, 10, 3, step=1)
        show_triangles = st.checkbox("Show NTD triangles (trend-based)", value=True)
        show_sr_stars = st.checkbox("Show S/R reversal stars on NTD", value=True)

        st.divider()
        st.header("News (Yahoo)")
        show_news = st.checkbox("Show recent news markers (daily)", value=False)
        news_days = st.slider("News window (days)", 1, 30, 7, step=1)

        st.divider()
        st.header("Scanners")
        scan_symbols_raw = st.text_area(
            "Symbols to scan (comma/space/newline separated)",
            value="SPY, QQQ, IWM, DIA, AAPL, MSFT, NVDA, TSLA"
        )
        scan_within_n_bars = st.slider("Kijun cross-up: within last N bars", 0, 30, 5, step=1)

    # Parse scan list
    scan_syms = []
    for tok in re.split(r"[,\s]+", scan_symbols_raw.strip()):
        t = tok.strip().upper()
        if t:
            scan_syms.append(t)
    scan_syms = list(dict.fromkeys(scan_syms))  # de-dupe, preserve order

    tab1, tab2 = st.tabs(["Chart", "Scan"])

    # -----------------------
    # Chart tab (Daily view)
    # -----------------------
    with tab1:
        ...
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        if close_show.empty:
            st.error("No data in selected Daily View range.")
            return

        ohlc_full = fetch_hist_ohlc(symbol)
        ohlc_show = None
        if ohlc_full is not None and (not ohlc_full.empty) and isinstance(ohlc_full.index, pd.DatetimeIndex):
            x0, x1 = close_show.index[0], close_show.index[-1]
            ohlc_show = ohlc_full.sort_index().loc[(ohlc_full.index >= x0) & (ohlc_full.index <= x1)].copy()

        news_df = pd.DataFrame()
        news_times = []
        if show_news:
            news_df = fetch_yf_news(symbol, window_days=int(news_days))
            if not news_df.empty and "time" in news_df.columns:
                news_times = list(news_df["time"].dropna().tolist())

        with colA:
            fig1, ax1 = plt.subplots(figsize=(10, 5.2))
            out = plot_price_chart(
                ax1,
                ohlc=ohlc_show,
                close=close_show,
                title=f"{symbol} — Daily ({daily_view_label})",
                slope_lb=int(slope_lb),
                show_reg_band=bool(show_reg_band),
                show_sma50=True,
                show_sma200=False,
                show_ema21=False,
                show_bbands=False,
                ...
                show_sr=True,
                sr_lb=int(sr_lb),
                sr_prox=float(sr_prox),
                show_news=bool(show_news),
                news_times=news_times
            )
            st.pyplot(fig1, clear_figure=True)

            band = out.get("band", {}) if isinstance(out, dict) else {}
            m = band.get("slope", np.nan)
            r2 = band.get("r2", np.nan)
            if np.isfinite(m) or np.isfinite(r2):
                st.caption(f"Regression slope: {m:.6g}   |   R²: {r2:.4f}" if np.isfinite(r2) else f"Regression slope: {m:.6g}")

            if show_news and (not news_df.empty):
                st.subheader("Recent News (Yahoo)")
                st.dataframe(news_df.sort_values("time", ascending=False), use_container_width=True)
        ...
        with colB:
            ...
            _ = plot_ntd_panel(
                ...
                show_sr_reversal_stars=bool(show_sr_stars)
            )
    ...
    with tab2:
        ...
        if not scan_syms:
            st.info("Add symbols in the sidebar to run scanners.")
            return

        st.write(f"Scanning **{len(scan_syms)}** symbols. (Proximity uses **S/R proximity (%)** = **{sr_prox_pct:.2f}%** for both S/R and ±2σ lists.)")

        # 1) Kijun cross-up (Daily)
        kij_rows = []
        for sym in scan_syms:
            row = last_daily_kijun_cross_up(
                sym,
                daily_view_label=daily_view_label,
                slope_lb=int(slope_lb),
                conv=9, base=26, span_b=52,
                within_last_n_bars=int(scan_within_n_bars)
            )
            if row:
                kij_rows.append(row)

        kij_df = _df_from_rows(kij_rows)
        st.markdown("### Daily — Price Crossed Above Kijun (Heading Up)")
        if kij_df.empty:
            st.caption("No matches.")
        else:
            st.dataframe(kij_df, use_container_width=True)

        # 2) Band bounce (Daily + Hourly) — uses same logic as chart band-bounce
        bounce_rows = []
        for sym in scan_syms:
            r1 = last_band_bounce_signal_daily(sym, slope_lb=int(slope_lb))
            if r1:
                bounce_rows.append(r1)
            r2h = last_band_bounce_signal_hourly(sym, period=str(hourly_period), slope_lb=int(slope_lb))
            if r2h:
                bounce_rows.append(r2h)
        bounce_df = _df_from_rows(bounce_rows)
        st.markdown("### Band Bounce Signals (Daily + Hourly)")
        if bounce_df.empty:
            st.caption("No matches.")
        else:
            st.dataframe(bounce_df, use_container_width=True)

        # 3) Slope Direction Scan — plus “close to ±2σ” lists (uses SAME sr_prox slider; no extra UI)
        st.markdown("### Slope Direction Scan + Close to ±2σ (uses S/R proximity %)")
        ...
        daily_minus = []
        for sym in scan_syms:
            r = daily_close_near_plus2sigma(sym, slope_lb=int(slope_lb), prox=float(sr_prox))
            if r:
                daily_plus.append(r)
            r = daily_close_near_minus2sigma(sym, slope_lb=int(slope_lb), prox=float(sr_prox))
            if r:
                daily_minus.append(r)

        # Hourly ±2σ proximity
        hourly_plus = []
        hourly_minus = []
        for sym in scan_syms:
            r = hourly_close_near_plus2sigma(sym, period=str(hourly_period), slope_lb=int(slope_lb), prox=float(sr_prox))
            if r:
                hourly_plus.append(r)
            r = hourly_close_near_minus2sigma(sym, period=str(hourly_period), slope_lb=int(slope_lb), prox=float(sr_prox))
            if r:
                hourly_minus.append(r)

        with col1:
            st.markdown("**Daily — Close ~ +2σ**")
            dfp = _df_from_rows(daily_plus)
            st.dataframe(dfp, use_container_width=True) if not dfp.empty else st.caption("No matches.")

            st.markdown("**Daily — Close ~ -2σ**")
            dfm = _df_from_rows(daily_minus)
            st.dataframe(dfm, use_container_width=True) if not dfm.empty else st.caption("No matches.")

        with col2:
            st.markdown(f"**Hourly ({hourly_period}) — Close ~ +2σ**")
            dfp = _df_from_rows(hourly_plus)
            st.dataframe(dfp, use_container_width=True) if not dfp.empty else st.caption("No matches.")

            st.markdown(f"**Hourly ({hourly_period}) — Close ~ -2σ**")
            dfm = _df_from_rows(hourly_minus)
            st.dataframe(dfm, use_container_width=True) if not dfm.empty else st.caption("No matches.")

        # 4) R² scanners (Daily + Hourly)
        st.markdown("### R² (Trend Fit Strength) — Daily + Hourly")
        r2_rows = []
        for sym in scan_syms:
            r2d, md, tsd = daily_regression_r2(sym, slope_lb=int(slope_lb))
            r2_rows.append({"Symbol": sym, "Frame": "Daily", "R2": r2d, "Slope": md, "AsOf": tsd})
            r2h, mh, tsh = hourly_regression_r2(sym, period=str(hourly_period), slope_lb=int(slope_lb))
            r2_rows.append({"Symbol": sym, "Frame": f"Hourly ({hourly_period})", "R2": r2h, "Slope": mh, "AsOf": tsh})
        r2_df = pd.DataFrame(r2_rows)
        if not r2_df.empty:
            r2_df = r2_df.sort_values(["Frame", "R2"], ascending=[True, False])
