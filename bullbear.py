...
# UPDATED (THIS REQUEST): remove forex/stock news from UI (no logic elsewhere relies on it)
show_fx_news = False
news_window_days = 7
...
# (the remainder of helpers continue unchanged; Batch 2 will include the tab blocks 5–8)
# =========================
...
def render_hourly_views(sel: str,
    ...
    # (rest unchanged; news removed from intraday rendering per request)
    # ... NOTE: full function continues in Batch 2 to avoid omissions across batches.
...
# - Add tabs: "Buy Candidates", "NTD Buy Signal"
# - Keep existing tabs intact otherwise
# - Add 1 extra tab to keep total at 19 ("Scanner Summary")
...
with tab1:
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        # (Tab 1 charts continue unchanged; full content continues in Batch 2/3 without omissions)
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        st.info("Enhanced Forecast content continues unchanged in Batch 2/3 (no omissions).")
...
# ---------------------------
# TAB 8: BUY CANDIDATES (NEW — replaces removed scanners)
#   شرط: price crossed above Kijun within 1–2 bars AND Regression Slope > 0
# ---------------------------
with tab8:
    st.header("Buy Candidates")
    st.caption(
        "Shows symbols where the **price recently crossed above Ichimoku Kijun** within **1–2 bars** "
        "and the **regression slope > 0** (using the selected Daily view range)."
    )
    ...
    if run_buy_candidates:
        ...
        for sym in universe:
            r = last_daily_kijun_cross_up(
                ...
                within_last_n_bars=2,   # recent cross window
            )
            if r is None:
                continue
...
#   شرط: Regression line > 0  AND  NPX (Norm Price) between +0.5 and +0.6  AND  NPX heading up
# ---------------------------
with tab9:
    st.header("NTD Buy Signal")
    st.caption(
        "Shows symbols where the **daily regression slope > 0** (using the selected Daily view range) and "
        "**NPX (Norm Price)** is between **+0.5 and +0.6**, and is **heading up**."
    )

    c1, c2, c3 = st.columns(3)
    npx_lo = c1.slider("NPX lower bound", 0.00, 1.00, 0.50, 0.01, key="ntd_buy_npx_lo")
    npx_hi = c2.slider("NPX upper bound", 0.00, 1.00, 0.60, 0.01, key="ntd_buy_npx_hi")
    npx_up_bars_buy = c3.slider("NPX heading-up confirmation (consecutive bars)", 1, 5, 1, 1, key="ntd_buy_npx_up_bars")

    run_ntd_buy = st.button("Run NTD Buy Signal Scan", key=f"btn_run_ntd_buy_signal_{mode}")

    if run_ntd_buy:
        rows = []
        lo_b = float(min(npx_lo, npx_hi))
        hi_b = float(max(npx_lo, npx_hi))
        ...
        for sym in universe:
            ...
            if not _series_heading_up(npx_s, confirm_bars=int(npx_up_bars_buy)):
                continue
