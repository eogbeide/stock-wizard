...
# =========================
# Part 1/10 — bullbear.py  (UPDATED: Ribbon Tabs + Beautiful Chart Styling + Requested Tab Changes)
# =========================
...
# (THIS REQUEST): remove forex and stock news (no news UI / no news markers)
if mode == "Forex":
...
def draw_trend_direction_line(ax, series_like: pd.Series, label_prefix: str = "Trend"):
    # (BUGFIX: NameError) Ensure this exists before any tabs call it.
    s = _coerce_1d_series(series_like).dropna()
    if s.shape[0] < 2:
        return np.nan
    x = np.arange(len(s), dtype=float)
...
# =========================
# STOPPOINT for Batch 1:
# We include full code through TAB 4 below.
# (Remaining helpers + tabs 5+ continue in Batch 2/3.)
# =========================
...
# (THIS REQUEST): remove tabs "NPX 0.5-Cross Scanner" and "Fib NPX 0.0 Signal Scanner"
# and add tabs "Buy Candidates" + "NTD Buy Signal" (titles updated here; bodies in Batch 2/3).
...
with tab1:
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        # NOTE: Full chart rendering + hourly rendering helpers continue in Batch 2/3.
        # Here, keep Tab 1 behavior intact (forecast table still shown).
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        # Full enhanced charts remain unchanged and continue in Batch 2/3 (no omissions across 3 batches).
        st.subheader("SARIMAX Forecast (30d)")
        st.write(pd.DataFrame({
            "Forecast": vals,
            "Lower":    ci.iloc[:, 0],
...
#   - Replaces the removed "NPX 0.5-Cross Scanner"
#   - Shows symbols where price crossed Ichimoku Kijun recently (1–2 bars ago)
#     AND regression slope > 0 (Daily, matches Price Chart daily-view window)
# ---------------------------
with tab8:
    st.header("Buy Candidates")
    st.caption(
        "Shows symbols where **price crossed ABOVE the Ichimoku Kijun** **1–2 bars ago** "
        "and the **daily regression slope > 0** (uses the selected Daily view range)."
    )
    ...
    if run_buy_candidates:
        ...
        for sym in universe:
            r = last_daily_kijun_cross_up(
                ...
                within_last_n_bars=2,   # ensures "recent"
            )
            if r is None:
                continue
            ...
            # "by 1–2 bars" and regression line > 0
            if bs in (1, 2) and np.isfinite(m) and m > 0.0:
                rows.append(r)
...
with tab9:
    ...
    st.caption(
        "Shows symbols where:\n"
        "• **Regression line slope > 0** (Daily, using selected Daily view range)\n"
        "• **NPX (Norm Price)** is between **+0.5** and **+0.6**\n"
        "• NPX is **heading up** (confirmed by consecutive bars)\n"
    )

    c1, c2 = st.columns(2)
    npx_up_bars_056 = c1.slider("NPX heading-up confirmation (consecutive bars)", 1, 5, 2, 1, key="npx_056_up_bars")
