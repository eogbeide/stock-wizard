...
# ---------------------------
# Ichimoku Kijun Daily Cross-Up Scanner helper (Daily only)
# ---------------------------
@st.cache_data(ttl=120)
...
# ---------------------------
# R² scanners: raw (kept for backward-compat)
# ---------------------------
@st.cache_data(ttl=120)
...
# ---------------------------
# NEW (THIS REQUEST): R² categorization helpers (STRICT proximity)
#   - "Near Support" = close <= support*(1+prox)
#   - "Near Resistance" = close >= resistance*(1-prox)
#   - "Both trend/regression lines upward" = (global_slope > 0) AND (reg_slope > 0)
#   - "Both downward" = (global_slope < 0) AND (reg_slope < 0)
# ---------------------------
def _polyfit_slope(series_like) -> float:
    s = _coerce_1d_series(series_like).dropna()
    if len(s) < 2:
        return np.nan
    x = np.arange(len(s), dtype=float)
...
def _near_support_strict(close_val: float, support_val: float, prox: float) -> bool:
    try:
        c = float(close_val); s = float(support_val); p = float(prox)
    except Exception:
        return False
    if not (np.isfinite(c) and np.isfinite(s) and np.isfinite(p)) or s == 0.0 or p < 0:
        return False
    return bool(c <= (s * (1.0 + p)))

def _near_resistance_strict(close_val: float, resist_val: float, prox: float) -> bool:
    try:
        c = float(close_val); r = float(resist_val); p = float(prox)
    except Exception:
        return False
    if not (np.isfinite(c) and np.isfinite(r) and np.isfinite(p)) or r == 0.0 or p < 0:
        return False
    return bool(c >= (r * (1.0 - p)))

def _both_up(global_slope: float, reg_slope: float) -> bool:
    try:
        g = float(global_slope); m = float(reg_slope)
    except Exception:
        return False
    return bool(np.isfinite(g) and np.isfinite(m) and (g > 0.0) and (m > 0.0))

def _both_down(global_slope: float, reg_slope: float) -> bool:
    try:
        g = float(global_slope); m = float(reg_slope)
    except Exception:
        return False
...
@st.cache_data(ttl=120)
def daily_r2_scan_metrics(symbol: str,
                          daily_view_label: str,
                          slope_lb: int,
                          sr_lb: int,
                          prox: float):
    """
    Returns dict with:
      - R2, RegSlope (regression_with_band over last slope_lb bars)
      - GlobalSlope (polyfit over Daily view range; matches chart global trendline)
      - CloseLast, SupportLast, ResistLast, NearSupport, NearResistance
      - AsOf (timestamp)
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        ...
        _, _, _, m_reg, r2 = regression_with_band(close_full, lookback=int(slope_lb))

        close_view = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        g_slope = _polyfit_slope(close_view)

        sup = close_full.rolling(int(sr_lb), min_periods=1).min()
        res = close_full.rolling(int(sr_lb), min_periods=1).max()

        c_last = float(close_full.iloc[-1]) if np.isfinite(close_full.iloc[-1]) else np.nan
        s_last = float(sup.iloc[-1]) if np.isfinite(sup.iloc[-1]) else np.nan
        r_last = float(res.iloc[-1]) if np.isfinite(res.iloc[-1]) else np.nan

        near_s = _near_support_strict(c_last, s_last, float(prox))
        near_r = _near_resistance_strict(c_last, r_last, float(prox))
        ...
        return {
            ...
            "Frame": "Daily",
            "R2": float(r2) if np.isfinite(r2) else np.nan,
            "Reg Slope": float(m_reg) if np.isfinite(m_reg) else np.nan,
            "Global Slope": float(g_slope) if np.isfinite(g_slope) else np.nan,
            "Close": c_last,
            "Support": s_last,
            "Resistance": r_last,
            "Near Support": bool(near_s),
            "Near Resistance": bool(near_r),
            "AsOf": ts
        }
    except Exception:
        return None
...
@st.cache_data(ttl=120)
def hourly_r2_scan_metrics(symbol: str,
                           period: str,
                           slope_lb: int,
                           sr_lb: int,
                           prox: float):
    """
    Hourly/intraday metrics on intraday Close:
      - Regression slope + R²: regression_with_band over last slope_lb bars
      - Global slope: polyfit over full intraday window
      - Support/Resistance: rolling min/max over sr_lb bars
      - NearSupport / NearResistance: STRICT proximity
    """
    try:
        df = fetch_intraday(symbol, period=str(period))
        if df is None or df.empty or "Close" not in df.columns:
            return None
        ...
        if hc.empty:
            ts = real_times[-1] if isinstance(real_times, pd.DatetimeIndex) and len(real_times) else None
            return {
                "Symbol": symbol, "Frame": f"Hourly ({period})",
                "R2": np.nan, "Reg Slope": np.nan, "Global Slope": np.nan,
                "Close": np.nan, "Support": np.nan, "Resistance": np.nan,
                "Near Support": False, "Near Resistance": False,
                "AsOf": ts, "Period": str(period)
            }

        _, _, _, m_reg, r2 = regression_with_band(hc, lookback=int(slope_lb))
        g_slope = _polyfit_slope(hc)

        sup = hc.rolling(int(sr_lb), min_periods=1).min()
        res = hc.rolling(int(sr_lb), min_periods=1).max()

        c_last = float(hc.iloc[-1]) if np.isfinite(hc.iloc[-1]) else np.nan
        s_last = float(sup.iloc[-1]) if np.isfinite(sup.iloc[-1]) else np.nan
        r_last = float(res.iloc[-1]) if np.isfinite(res.iloc[-1]) else np.nan
...
# =========================
# NOTE: Tabs 1–15 are unchanged from your provided code (Batch 1 contained Parts 1–5; Batch 2 contains Parts 6–10).
#       Below is the continuation including Tabs 2–17 exactly as in your code, with ONLY the requested R² categorization updates in Tabs 16–17.
...
# ---------------------------
# TAB 16: R² > 45% Daily/Hourly  (UPDATED: Near Support vs Others)
# ---------------------------
with tab16:
    st.header("R² > 45% Daily/Hourly")
    st.caption(
        "UPDATED CATEGORIES (THIS REQUEST):\n"
        "• **Near Support:** R² > threshold AND **Near Support (strict)** AND **Global slope > 0** AND **Regression slope > 0**\n"
        "• **Others:** the rest with R² > threshold\n\n"
        "Daily uses Daily view for the global trendline slope; Hourly uses the full intraday window for global slope."
    )
    ...
    if run_r2:
        daily_near_support, daily_others = [], []
        hourly_near_support, hourly_others = [], []

        for sym in universe:
            d = daily_r2_scan_metrics(
                symbol=sym,
                daily_view_label=daily_view,
                slope_lb=int(slope_lb_daily),
                sr_lb=int(sr_lb_daily),
            ...
            if d is not None and np.isfinite(d.get("R2", np.nan)) and float(d["R2"]) > float(r2_thr):
                is_near = bool(d.get("Near Support", False)) and _both_up(d.get("Global Slope", np.nan), d.get("Reg Slope", np.nan))
                row = {
                    "Symbol": sym,
                    "R2": float(d["R2"]),
                    "Reg Slope": float(d.get("Reg Slope", np.nan)),
                    "Global Slope": float(d.get("Global Slope", np.nan)),
                    "Close": float(d.get("Close", np.nan)),
                    "Support": float(d.get("Support", np.nan)),
                    "AsOf": d.get("AsOf", None),
                }
                (daily_near_support if is_near else daily_others).append(row)

            h = hourly_r2_scan_metrics(
                symbol=sym,
            ...
            if h is not None and np.isfinite(h.get("R2", np.nan)) and float(h["R2"]) > float(r2_thr):
                is_near = bool(h.get("Near Support", False)) and _both_up(h.get("Global Slope", np.nan), h.get("Reg Slope", np.nan))
                row = {
                    "Symbol": sym,
                    "R2": float(h["R2"]),
                    "Reg Slope": float(h.get("Reg Slope", np.nan)),
                    "Global Slope": float(h.get("Global Slope", np.nan)),
                    "Close": float(h.get("Close", np.nan)),
                    "Support": float(h.get("Support", np.nan)),
                    "AsOf": h.get("AsOf", None),
                    "Period": str(hour_period),
                }
                (hourly_near_support if is_near else hourly_others).append(row)

        left, right = st.columns(2)
        with left:
            st.subheader("Daily — Near Support (R² > threshold + Near Support + both slopes UP)")
            if not daily_near_support:
                st.info("No matches.")
            else:
                out = pd.DataFrame(daily_near_support).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[False, False, False])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

            st.subheader("Daily — Others (R² > threshold)")
            if not daily_others:
                st.info("No matches.")
            else:
                out = pd.DataFrame(daily_others).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[False, False, False])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

        with right:
            st.subheader(f"Hourly ({hour_period}) — Near Support (R² > threshold + Near Support + both slopes UP)")
            if not hourly_near_support:
                st.info("No matches.")
            else:
                out = pd.DataFrame(hourly_near_support).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[False, False, False])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

            st.subheader(f"Hourly ({hour_period}) — Others (R² > threshold)")
            if not hourly_others:
                st.info("No matches.")
            else:
                out = pd.DataFrame(hourly_others).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[False, False, False])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

# ---------------------------
# TAB 17: R² < 45% Daily/Hourly  (UPDATED: Near Resistance vs Others)
# ---------------------------
with tab17:
    st.header("R² < 45% Daily/Hourly")
    st.caption(
        "UPDATED CATEGORIES (THIS REQUEST):\n"
        "• **Near Resistance:** R² < ceiling AND **Near Resistance (strict)** AND **Global slope < 0** AND **Regression slope < 0**\n"
        "• **Others:** the rest with R² < ceiling\n\n"
        "Daily uses Daily view for the global trendline slope; Hourly uses the full intraday window for global slope."
    )

    c1, c2, c3 = st.columns(3)
    r2_ceiling = c1.slider("R² ceiling (show below)", 0.00, 1.00, 0.45, 0.01, key="r2_ceiling_scan")
    hour_period2 = c2.selectbox("Hourly intraday period", ["1d", "2d", "4d"], index=0, key="r2_hour_period2")
    run_r2_lo = c3.button("Run R² < Scan", key=f"btn_run_r2_low_scan_{mode}")

    if run_r2_lo:
        daily_near_res, daily_other = [], []
        hourly_near_res, hourly_other = [], []
        ...
        for sym in universe:
            ...
            if d is not None and np.isfinite(d.get("R2", np.nan)) and float(d["R2"]) < float(r2_ceiling):
                is_near = bool(d.get("Near Resistance", False)) and _both_down(d.get("Global Slope", np.nan), d.get("Reg Slope", np.nan))
                row = {
                    "Symbol": sym,
                    "R2": float(d["R2"]),
                    "Reg Slope": float(d.get("Reg Slope", np.nan)),
                    "Global Slope": float(d.get("Global Slope", np.nan)),
                    "Close": float(d.get("Close", np.nan)),
                    "Resistance": float(d.get("Resistance", np.nan)),
                    "AsOf": d.get("AsOf", None),
                }
                (daily_near_res if is_near else daily_other).append(row)

            h = hourly_r2_scan_metrics(
                symbol=sym,
                period=str(hour_period2),
                slope_lb=int(slope_lb_hourly),
                sr_lb=int(sr_lb_hourly),
                prox=float(sr_prox_pct)
            )
            if h is not None and np.isfinite(h.get("R2", np.nan)) and float(h["R2"]) < float(r2_ceiling):
                is_near = bool(h.get("Near Resistance", False)) and _both_down(h.get("Global Slope", np.nan), h.get("Reg Slope", np.nan))
                row = {
                    "Symbol": sym,
                    "R2": float(h["R2"]),
                    "Reg Slope": float(h.get("Reg Slope", np.nan)),
                    "Global Slope": float(h.get("Global Slope", np.nan)),
                    "Close": float(h.get("Close", np.nan)),
                    "Resistance": float(h.get("Resistance", np.nan)),
                    "AsOf": h.get("AsOf", None),
                    "Period": str(hour_period2),
                }
                (hourly_near_res if is_near else hourly_other).append(row)

        left, right = st.columns(2)
        with left:
            st.subheader("Daily — Near Resistance (R² < ceiling + Near Resistance + both slopes DOWN)")
            if not daily_near_res:
                st.info("No matches.")
            else:
                out = pd.DataFrame(daily_near_res).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[True, True, True])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

            st.subheader("Daily — Others (R² < ceiling)")
            if not daily_other:
                st.info("No matches.")
            else:
                out = pd.DataFrame(daily_other).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[True, True, True])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

        with right:
            st.subheader(f"Hourly ({hour_period2}) — Near Resistance (R² < ceiling + Near Resistance + both slopes DOWN)")
            if not hourly_near_res:
                st.info("No matches.")
            else:
                out = pd.DataFrame(hourly_near_res).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[True, True, True])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

            st.subheader(f"Hourly ({hour_period2}) — Others (R² < ceiling)")
            if not hourly_other:
                st.info("No matches.")
            else:
                out = pd.DataFrame(hourly_other).sort_values(["R2", "Global Slope", "Reg Slope"], ascending=[True, True, True])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)
