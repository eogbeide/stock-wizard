...
# NEWS REMOVED (THIS REQUEST): no sidebar Forex news controls, no news markers/tables
if mode == "Forex":
...
# =========================
# ---------------------------
# Ichimoku, Supertrend, PSAR (unchanged)
# ---------------------------
...
# ---------------------------
# Channel-in-range helpers for NTD panel (unchanged)
# ---------------------------
...
# =========================
# ---------------------------
# Cached last values for scanning (unchanged)
# ---------------------------
@st.cache_data(ttl=120)
...
# ---------------------------
# Daily slope direction helper (unchanged)
# ---------------------------
@st.cache_data(ttl=120)
...
# NEWS REMOVED: no news fetch/markers
# ---------------------------
...
# - Added new tabs placeholders: "Buy Candidates", "NBB -0.8 to -1.0 Lists"
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        if view in ("Daily", "Both"):
            ...
            # THIS REQUEST (3): add regression line + ±2σ on Enhanced Forecast Daily chart
            yhat_e, up_e, lo_e, m_e, r2_e = regression_with_band(df_show, lookback=int(slope_lb_daily), z=2.0)
            if not yhat_e.empty:
                ax.plot(yhat_e.index, yhat_e.values, "-", linewidth=2.0, label=f"Regression ({fmt_slope(m_e)}/bar)")
...
# ---------------------------
# NameError hardening (THIS REQUEST): ensure the symbol exists even if older code paths changed.
# ---------------------------
try:
    _ = last_daily_kijun_cross_up  # noqa: F841
except Exception:
    # Placeholder; real implementation is provided later in the file (Batch 3).
    def last_daily_kijun_cross_up(*args, **kwargs):  # type: ignore
        return None
# ---------------------------
...
with tab8:
    ...
    st.caption(
        "Daily-only scanner:\n"
        "• price **recently crossed ABOVE** Ichimoku **Kijun** within **1–2 bars**\n"
        "• **Regression slope > 0** (same daily-view range + slope lookback)\n"
        "Uses the existing sidebar Daily view range, Ichimoku settings, and Daily slope lookback."
    )
    ...
    if run_buy_candidates:
        ...
        for sym in universe:
            r = last_daily_kijun_cross_up(
                ...
                within_last_n_bars=2,  # recent window (max 2)
            )
            if r is None:
                continue
            ...
            # Must be 1–2 bars (explicit per request)
            if bars_since < 1 or bars_since > 2:
                continue
        ...
        if not rows:
            st.info("No matches (Kijun cross-up within 1–2 bars AND regression slope > 0).")
        else:
            out = pd.DataFrame(rows)
...
# ---------------------------
# TAB 12: NBB -0.8 to -1.0 Lists (NEW — THIS REQUEST)
...
with tab12:
    ...
    st.caption(
        "Daily-only scan using the selected Daily view range:\n"
        "(a) **Regression slope > 0** and **NBB** between **-0.8 and -1.0**\n"
        "(b) **Regression slope < 0** and **NBB** between **-0.8 and -1.0**\n\n"
        "NBB is derived from Bollinger %B scaled to [-1,+1]."
    )

    run_nbb = st.button("Run NBB -0.8 to -1.0 Scan", key=f"btn_run_nbb_scan_{mode}")
    ...
    if run_nbb:
        ...
        for sym in universe:
            ...
            bb_nbb = _coerce_1d_series(bb_nbb).dropna()
            bb_pctb = _coerce_1d_series(bb_pctb).dropna()
            if bb_nbb.empty:
                continue

            nbb_last = float(bb_nbb.iloc[-1]) if np.isfinite(bb_nbb.iloc[-1]) else np.nan
            pctb_last = float(bb_pctb.iloc[-1]) if (not bb_pctb.empty and np.isfinite(bb_pctb.iloc[-1])) else np.nan
        ...
        left, right = st.columns(2)
        with left:
            st.subheader("(a) Regression > 0  •  NBB [-1.0, -0.8]")
            if not rows_pos:
                st.info("No matches.")
            else:
        ...
        with right:
            st.subheader("(b) Regression < 0  •  NBB [-1.0, -0.8]")
            if not rows_neg:
                st.info("No matches.")
            else:
...
# ---------------------------
# TAB 13: NPX (Norm Price) 0.0–0.2 in NTD Panel (NEW — THIS REQUEST)
# ---------------------------
with tab13:
    st.header("NPX (Norm Price) 0.0–0.2 in NTD Panel")
    st.caption(
        "Daily-only scan:\n"
        "Lists symbols where **NPX (normalized price)** is between **0.0** and **0.2** (inclusive), "
        "split into two lists:\n"
        "• **List 1:** regression slope **> 0**\n"
        "• **List 2:** regression slope **< 0**\n\n"
        "Includes NPX (last) and R² for the daily regression slope."
    )

    run_npx02 = st.button("Run NPX 0.0–0.2 Scan", key=f"btn_run_npx02_{mode}")
    ...
    if run_npx02:
        ...
        with left:
            st.subheader("List 1 — Slope > 0 and NPX 0.0–0.2")
        ...
        with right:
            st.subheader("List 2 — Slope < 0 and NPX 0.0–0.2")
