...
def _direction_from_value(value: float) -> str:
    """Return Upward/Downward from a numeric slope-like value."""
    try:
        v = float(np.squeeze(value))
    except Exception:
        return "Downward"
    if not np.isfinite(v):
        return "Downward"
    return "Upward" if v >= 0.0 else "Downward"

def _latest_indicator_slope(series_like: pd.Series, lookback: int = 8) -> float:
    """Small regression slope over the latest bars of an indicator/price series."""
    s = _coerce_1d_series(series_like).replace([np.inf, -np.inf], np.nan).dropna()
    if s.shape[0] < 2:
        return float("nan")
    lb = max(2, min(int(lookback), s.shape[0]))
    _, m = slope_line(s, lookback=lb)
    return float(m) if np.isfinite(m) else float("nan")

@st.cache_data(ttl=120)
def ntd_scanner_context_daily(symbol: str, ntd_win: int, trend_lookback: int, slope_lookback: int = 8):
    """
    Daily scanner context:
      - Trend Direction is based on the price trendline slope.
      - Slope Direction is based on the recent NTD-line slope.
    """
    try:
        ...
        if close.empty:
            return {
                ...
                "Slope Direction": "Downward",
                "Trend Slope": np.nan,
                "NTD Slope": np.nan,
            }
        ntd = compute_normalized_trend(close, window=int(ntd_win)).dropna()
        ntd_val = float(ntd.iloc[-1]) if not ntd.empty else np.nan
        ts = ntd.index[-1] if not ntd.empty else None
        _, trend_slope = slope_line(close, lookback=int(trend_lookback))
        ntd_slope = _latest_indicator_slope(ntd, lookback=int(slope_lookback))
        return {
            "NTD_Daily": ntd_val,
            "Timestamp": ts,
            "Trend Direction": _direction_from_value(trend_slope),
            "Slope Direction": _direction_from_value(ntd_slope),
...
@st.cache_data(ttl=120)
def ntd_scanner_context_hourly(symbol: str, ntd_win: int, trend_lookback: int,
                               period: str = "1d", slope_lookback: int = 8):
    """
    Hourly/intraday scanner context:
      - Trend Direction is based on the intraday price trendline slope.
      - Slope Direction is based on the recent intraday NTD-line slope.
    """
    try:
        df = fetch_intraday(symbol, period=period)
...
def _format_ntd_scanner_dataframe(df: pd.DataFrame, ntd_col: str = None) -> pd.DataFrame:
    """Format scanner rows for display while preserving grouping columns."""
    out = df.copy()
    if "Timestamp" in out.columns:
        out["Timestamp"] = out["Timestamp"].astype(str)
    if ntd_col and ntd_col in out.columns:
        out[ntd_col] = pd.to_numeric(out[ntd_col], errors="coerce")
    for col in ["Trend Slope", "NTD Slope"]:
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors="coerce")
    return out

def _render_direction_grouped_scanner_tables(title: str,
                                             df: pd.DataFrame,
                                             ntd_col: str = None,
                                             sort_col: str = None,
                                             display_cols: list = None):
    """
    Render scanner data grouped first by Trend Direction and then by Slope Direction.
    Upward groups are shown before Downward groups.
    """
    st.subheader(title)
    if df is None or df.empty:
    ...
    work = _format_ntd_scanner_dataframe(df, ntd_col=ntd_col)
    trend_order = ["Upward", "Downward"]
    slope_order = ["Upward", "Downward"]

    if display_cols is None:
        display_cols = [c for c in work.columns if c not in {"AboveNow"}]

    for trend_dir in trend_order:
        trend_part = work[work["Trend Direction"] == trend_dir] if "Trend Direction" in work.columns else work.iloc[0:0]
        if trend_part.empty:
            continue
        st.markdown(f"**Trend: {trend_dir}**")
        for slope_dir in slope_order:
            part = trend_part[trend_part["Slope Direction"] == slope_dir] if "Slope Direction" in trend_part.columns else trend_part.iloc[0:0]
            if part.empty:
                continue
            st.caption(f"Slope: {slope_dir}")
            if sort_col and sort_col in part.columns and sort_col != "Symbol":
                part = part.sort_values([sort_col, "Symbol"], ascending=[True, True])
            elif "Symbol" in part.columns:
                part = part.sort_values("Symbol")
            show = part[[c for c in display_cols if c in part.columns]].copy()
            if ntd_col and ntd_col in show.columns:
                show[ntd_col] = show[ntd_col].map(lambda x: f"{x:+.3f}" if np.isfinite(x) else "n/a")
            for col in ["Trend Slope", "NTD Slope"]:
                if col in show.columns:
                    show[col] = show[col].map(lambda x: fmt_slope(x) if np.isfinite(x) else "n/a")
...
with tab5:
    ...
    st.caption(
        "Shows symbols with NTD below the selected threshold. Tables now include "
        "**Trend Direction** and **Slope Direction**, and are grouped by both directions "
        "with Upward groups shown first."
    )
    period_map = {"24h": "1d", "48h": "2d", "96h": "4d"}
    ...
    if run:
        daily_rows = []
        for sym in universe:
            ctx = ntd_scanner_context_daily(
                sym,
                ntd_win=ntd_window,
                trend_lookback=slope_lb_daily,
                slope_lookback=8,
            )
            daily_rows.append({
                "Symbol": sym,
                "NTD_Daily": ctx.get("NTD_Daily", np.nan),
                "Timestamp": ctx.get("Timestamp"),
                "Trend Direction": ctx.get("Trend Direction", "Downward"),
                "Slope Direction": ctx.get("Slope Direction", "Downward"),
                "Trend Slope": ctx.get("Trend Slope", np.nan),
                "NTD Slope": ctx.get("NTD Slope", np.nan),
            })

        df_daily = pd.DataFrame(daily_rows)
        below_daily = df_daily[
            np.isfinite(pd.to_numeric(df_daily["NTD_Daily"], errors="coerce"))
            & (pd.to_numeric(df_daily["NTD_Daily"], errors="coerce") < thresh)
        ].copy()
        ...
        _render_direction_grouped_scanner_tables(
            f"Daily — NTD < {thresh:+.2f}",
            below_daily,
            ntd_col="NTD_Daily",
            sort_col="NTD_Daily",
            display_cols=[
                "Symbol", "NTD_Daily", "Timestamp",
                "Trend Direction", "Slope Direction",
                "Trend Slope", "NTD Slope",
            ],
        )
        ...
        df_above_daily = df_above_daily[df_above_daily["AboveNow"] == True].copy()
        ...
        if df_above_daily.empty:
            ...
            view_above["Close"] = pd.to_numeric(view_above["Close"], errors="coerce")
            view_above["Kijun"] = pd.to_numeric(view_above["Kijun"], errors="coerce")
            ...
            _render_direction_grouped_scanner_tables(
                f"Daily Price > Kijun({ichi_base}) — grouped by Trend and Slope",
                view_above,
                ntd_col=None,
                sort_col="Symbol",
                display_cols=[
        ...
        if mode == "Forex":
            ...
            hourly_rows = []
            for sym in universe:
                ctx = ntd_scanner_context_hourly(
                    sym,
                    ntd_win=ntd_window,
                    trend_lookback=slope_lb_hourly,
                    period=scan_period,
                    slope_lookback=8,
                )
                hourly_rows.append({
                    "Symbol": sym,
                    "NTD_Hourly": ctx.get("NTD_Hourly", np.nan),
                    "Timestamp": ctx.get("Timestamp"),
                    "Trend Direction": ctx.get("Trend Direction", "Downward"),
                    "Slope Direction": ctx.get("Slope Direction", "Downward"),
                    "Trend Slope": ctx.get("Trend Slope", np.nan),
            ...
            below_hour = df_hour[
                np.isfinite(pd.to_numeric(df_hour["NTD_Hourly"], errors="coerce"))
                & (pd.to_numeric(df_hour["NTD_Hourly"], errors="coerce") < thresh)
            ].copy()
            ...
            _render_direction_grouped_scanner_tables(
                f"Forex Hourly — NTD < {thresh:+.2f} ({scan_hour_range})",
                below_hour,
                ntd_col="NTD_Hourly",
                sort_col="NTD_Hourly",
                display_cols=[
                    "Symbol", "NTD_Hourly", "Timestamp",
                    "Trend Direction", "Slope Direction",
                    "Trend Slope", "NTD Slope",
                ],
            )
            ...
            df_above_hour = df_above_hour[df_above_hour["AboveNow"] == True].copy()
            ...
            if df_above_hour.empty:
                ...
                vch["Close"] = pd.to_numeric(vch["Close"], errors="coerce")
                vch["Kijun"] = pd.to_numeric(vch["Kijun"], errors="coerce")
                ...
                _render_direction_grouped_scanner_tables(
                    f"Forex Hourly Price > Kijun({ichi_base}) — grouped by Trend and Slope",
                    vch,
                    ntd_col=None,
                    sort_col="Symbol",
                    display_cols=[
