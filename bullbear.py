...
# ---------------------------
# NEW (THIS REQUEST): Regression Trend scanner helper
#   Condition A (UP):
#     - Global trendline slope > 0
#     - Regression slope > 0 (regression_with_band over selected daily view)
#     - Price just reversed from LOWER 2σ band (band-bounce BUY)
#   Condition B (DOWN):
#     - Global trendline slope < 0
#     - Regression slope < 0
#     - Price just reversed from UPPER 2σ band (band-bounce SELL)
# ---------------------------
@st.cache_data(ttl=120)
def daily_regression_trend_reversal_scan(symbol: str,
                                         daily_view_label: str,
                                         slope_lb: int,
                                         max_bars_since_bounce: int = 2):
    """
    Returns dict with:
      - "Bucket": "UP" or "DOWN"
      - core slopes and bounce info
    or None if no match.
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        ...
        g_m, g_r2, ts = daily_global_slope(symbol, daily_view_label=daily_view_label)
        ...
        t_sig = sig.get("time", None)
        if t_sig is None or t_sig not in close_show.index:
            return None

        loc = int(close_show.index.get_loc(t_sig))
        bars_since = int((len(close_show) - 1) - loc)
        if bars_since > int(max_bars_since_bounce):
            return None

        side = str(sig.get("side", "")).upper()
        last_px = float(close_show.iloc[-1]) if np.isfinite(close_show.iloc[-1]) else np.nan
        sig_px = float(sig.get("price", np.nan))
        dlt = (last_px / sig_px - 1.0) if np.isfinite(last_px) and np.isfinite(sig_px) and sig_px != 0 else np.nan

        # Bucket A (UP): both slopes > 0 and bounce is BUY (from lower 2σ)
        if float(g_m) > 0.0 and float(m) > 0.0 and side == "BUY":
            return {
                "Bucket": "UP",
                "Symbol": symbol,
                "Bars Since Bounce": int(bars_since),
                "Bounce Time": t_sig,
                "Bounce Price": sig_px,
                "Current Price": last_px,
                "DeltaPct": dlt,
                "Global Slope": float(g_m),
                "Global R2": float(g_r2) if np.isfinite(g_r2) else np.nan,
                "Regression Slope": float(m),
                "Regression R2": float(r2) if np.isfinite(r2) else np.nan,
            }

        # Bucket B (DOWN): both slopes < 0 and bounce is SELL (from upper 2σ)
        if float(g_m) < 0.0 and float(m) < 0.0 and side == "SELL":
            return {
                "Bucket": "DOWN",
                "Symbol": symbol,
                "Bars Since Bounce": int(bars_since),
...
def _maybe_add_legend(ax, max_items: int = 14, loc: str = "best"):
    ...
    if len(hh) > max_items:
        hh = hh[:max_items]
        ll = ll[:max_items]
    ax.legend(hh, ll, fontsize=8, loc=loc, frameon=True)

def _plot_price_panel(symbol: str,
                      close_show: pd.Series,
                      ohlc_show: pd.DataFrame = None,
                      slope_lb: int = 80,
                      ...
                      ich_conv: int = 9, ich_base: int = 26, ich_span_b: int = 52,
                      show_supertrend: bool = True, st_atr: int = 10, st_mult: float = 3.0,
                      show_psar: bool = True, psar_step: float = 0.02, psar_max: float = 0.2,
    ...
    ax.plot(c.index, c.values, linewidth=1.7, label=f"{symbol} Close")

    # Regression channel
    yhat = up = lo = m = r2 = None
    sig = None
    if show_regression and len(c) >= max(8, int(slope_lb)):
        yhat, up, lo, m, r2 = regression_with_band(c, lookback=int(slope_lb))
        ax.plot(yhat.index, yhat.values, linewidth=1.2, label=f"Reg({slope_lb}) m={m:.3g} r2={r2:.2f}")
        ax.plot(up.index, up.values, linewidth=1.0, linestyle="--", label="+2σ")
        ax.plot(lo.index, lo.values, linewidth=1.0, linestyle="--", label="-2σ")
        try:
            sig = find_band_bounce_signal(c, up, lo, m)
            if isinstance(sig, dict) and sig.get("time", None) in c.index:
                t = sig["time"]
                px = float(sig.get("price", np.nan))
                side = str(sig.get("side", "")).upper()
                if np.isfinite(px):
                    if side == "BUY":
                        ax.scatter([t], [px], marker="^", s=120, zorder=15, label="BUY (2σ bounce)")
                    elif side == "SELL":
                        ax.scatter([t], [px], marker="v", s=120, zorder=15, label="SELL (2σ bounce)")
        except Exception:
            pass
    ...
    if show_fibs:
        try:
            fibs = fibonacci_levels(c)
            if isinstance(fibs, dict) and len(fibs):
                for k, v in fibs.items():
                    if not np.isfinite(v):
                        continue
                    ax.axhline(float(v), linewidth=0.9, linestyle=":", alpha=0.7, label=f"Fib {k}")
        except Exception:
            pass
    ...
    if show_sr:
        try:
            sup, res = detect_support_resistance(c, window=int(sr_lb))
            sup = _coerce_1d_series(sup).reindex(c.index)
            res = _coerce_1d_series(res).reindex(c.index)
            ax.plot(sup.index, sup.values, linewidth=1.0, linestyle="--", alpha=0.9, label=f"Support({sr_lb})")
            ax.plot(res.index, res.values, linewidth=1.0, linestyle="--", alpha=0.9, label=f"Resist({sr_lb})")
        except Exception:
            sup = res = None

    # Ichimoku (use OHLC)
    kijun = None
    if show_ichimoku and ohlc_show is not None and ({"High","Low","Close"}.issubset(ohlc_show.columns)):
        try:
            ...
            tenkan = _coerce_1d_series(tenkan).reindex(c.index).ffill()
            kijun  = _coerce_1d_series(kijun).reindex(c.index).ffill()
            ax.plot(tenkan.index, tenkan.values, linewidth=1.0, alpha=0.9, label="Tenkan")
            ax.plot(kijun.index,  kijun.values,  linewidth=1.1, alpha=0.9, label="Kijun")
        except Exception:
            kijun = None

    # Supertrend (use OHLC)
    if show_supertrend and ohlc_show is not None and ({"High","Low","Close"}.issubset(ohlc_show.columns)):
        try:
            st_df = compute_supertrend(ohlc_show, atr_period=int(st_atr), atr_mult=float(st_mult))
            st_line = _coerce_1d_series(st_df["ST"]).reindex(c.index).ffill()
            ax.plot(st_line.index, st_line.values, linewidth=1.1, alpha=0.9, label="Supertrend")
        except Exception:
            pass

    # PSAR (use OHLC)
    if show_psar and ohlc_show is not None and ({"High","Low"}.issubset(ohlc_show.columns)):
        try:
            ps = compute_psar_from_ohlc(ohlc_show, step=float(psar_step), max_step=float(psar_max))
            psar = _coerce_1d_series(ps["PSAR"]).reindex(c.index).ffill()
            ax.scatter(psar.index, psar.values, s=10, alpha=0.9, label="PSAR")
        except Exception:
            pass

    # Session lines (PST)
    if show_session_lines and isinstance(c.index, pd.DatetimeIndex) and c.index.tz is not None:
        try:
            lines = compute_session_lines(c.index)
            h, l = draw_session_lines(ax, lines, alpha=0.25)
            for hi, li in zip(h, l):
                ax.plot([], [], linestyle=hi.get_linestyle(), linewidth=hi.get_linewidth(),
                        color=hi.get_color(), label=li)
        except Exception:
            pass
    ...
    if show_news:
        try:
            ndf = fetch_yf_news(symbol, window_days=int(news_window_days))
            if ndf is not None and not ndf.empty and "time" in ndf.columns:
                draw_news_markers(ax, list(ndf["time"].values), label="News (YF)")
        except Exception:
            pass
    ...
    # Add slope/r2 in corner (if available)
    if show_regression and m is not None and r2 is not None and np.isfinite(m) and np.isfinite(r2):
        ax.text(0.01, 0.98, f"Reg m={m:.4g}  R²={r2:.2f}",
                transform=ax.transAxes, ha="left", va="top",
                fontsize=9, bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="black", alpha=0.75))

    _maybe_add_legend(ax, max_items=16, loc="best")
    fig.tight_layout()
    return fig, sup, res, kijun

def _plot_ntd_panel(symbol: str,
                    close_show: pd.Series,
                    ntd_win: int = 55,
                    show_npx: bool = True,
    ...
    ntd = compute_normalized_trend(close_show, window=int(ntd_win)).reindex(close_show.index)
    ax.plot(ntd.index, ntd.values, linewidth=1.5, label=f"NTD({ntd_win})")
    ax.axhline(0.0, linewidth=1.0, alpha=0.6)
    ax.axhline(0.75, linewidth=0.9, linestyle="--", alpha=0.5)
    ax.axhline(-0.75, linewidth=0.9, linestyle="--", alpha=0.5)

    # In-range shading based on S/R
    last_state = np.nan
    if show_inrange and sup is not None and res is not None:
        try:
            last_state = overlay_inrange_on_ntd(ax, close_show, sup, res)
        except Exception:
            last_state = np.nan
    ...
    if show_npx:
        try:
            npx = compute_normalized_price(close_show, window=int(ntd_win)).reindex(close_show.index)
            overlay_npx_on_ntd(ax, npx, ntd, mark_crosses=True)
        except Exception:
            pass

    # Trend slope for triangle logic (simple global slope on this view)
    trend_slope = np.nan
    try:
        if len(close_show) >= 3:
            x = np.arange(len(close_show), dtype=float)
    ...
    # Triangles at NTD 0-cross and extremes
    if show_triangles and np.isfinite(trend_slope):
        try:
    ...
    if show_stars and sup is not None and res is not None and np.isfinite(trend_slope):
        try:
    ...
    # HMA reversal markers (on NTD panel, but computed from price/HMA)
    if show_hma_rev:
        try:
            hma = compute_hma(close_show, period=int(hma_period))
            overlay_hma_reversal_on_ntd(ax,
                                       price=close_show,
                                       hma=hma,
    ...
    try:
        last_ntd = float(_coerce_1d_series(ntd).dropna().iloc[-1]) if len(_coerce_1d_series(ntd).dropna()) else np.nan
        ax.text(0.99, 0.92, f"NTD(last)={last_ntd:.2f}",
                transform=ax.transAxes, ha="right", va="top",
                fontsize=9, bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="black", alpha=0.75))
    except Exception:
        pass

    ax.set_title(f"{symbol} — NTD Panel")
    ax.set_ylim(-1.1, 1.1)
    ax.grid(True, alpha=0.25)
    _maybe_add_legend(ax, max_items=14, loc="best")
    fig.tight_layout()
    return fig

def _safe_df(d: list) -> pd.DataFrame:
    if not isinstance(d, list) or len(d) == 0:
        return pd.DataFrame()
    return pd.DataFrame([x for x in d if isinstance(x, dict)])
...
# Scanner runners
# ---------------------------
def _split_symbols(text: str) -> list:
    if not isinstance(text, str):
        return []
    raw = [x.strip().upper() for x in text.replace("\n", ",").replace(" ", ",").split(",")]
    return [x for x in raw if x]

def run_scan(symbols: list, fn, *args, **kwargs) -> pd.DataFrame:
    out = []
    for sym in symbols:
        try:
...
def show_scanner_table(df: pd.DataFrame, sort_by: str = None, ascending: bool = True, limit: int = 50):
    if df is None or df.empty:
        st.info("No matches.")
        return
    dff = df.copy()
    if "DeltaPct" in dff.columns:
        dff["DeltaPct"] = dff["DeltaPct"].apply(_format_pct)
    if sort_by and sort_by in dff.columns:
        try:
            dff = dff.sort_values(sort_by, ascending=ascending)
        except Exception:
            pass
...
def _render_news_list(symbol: str, window_days: int = 7):
    ndf = fetch_yf_news(symbol, window_days=int(window_days))
    if ndf is None or ndf.empty:
        st.caption("No recent Yahoo Finance news returned.")
        return
    ndf = ndf.sort_values("time", ascending=False)
    for _, r in ndf.head(10).iterrows():
        t = r.get("time", "")
        title = r.get("title", "")
        pub = r.get("publisher", "")
        link = r.get("link", "")
        st.write(f"- {t} — **{title}** ({pub})")
...
# ---------------------------
def app():
    st.set_page_config(page_title="BullBear — Trend & Reversal Dashboard", layout="wide")
    st.title("BullBear — Trend & Reversal Dashboard")

    with st.sidebar:
        st.header("Inputs")
        symbols_text = st.text_area("Symbols (comma separated)", value="SPY, QQQ, AAPL, MSFT, NVDA", height=90)
        symbols = _split_symbols(symbols_text)

        st.subheader("Daily View")
        daily_view_label = st.selectbox("Window", options=["3M", "6M", "1Y", "2Y", "5Y", "MAX"], index=2)

        st.subheader("Core Params")
        slope_lb = st.slider("Regression Lookback (bars)", min_value=20, max_value=220, value=80, step=5)
        ntd_win = st.slider("NTD Window", min_value=10, max_value=120, value=55, step=1)
        sr_lb = st.slider("Support/Resistance Window", min_value=10, max_value=120, value=40, step=5)

        st.subheader("Overlays")
        show_regression = st.checkbox("Regression + 2σ bands", value=True)
        show_fibs = st.checkbox("Fibonacci levels", value=True)
        show_sr = st.checkbox("Support/Resistance", value=True)
        show_ichimoku = st.checkbox("Ichimoku (Tenkan/Kijun)", value=False)
        show_supertrend = st.checkbox("Supertrend", value=False)
        show_psar = st.checkbox("PSAR", value=False)
        show_session = st.checkbox("London/NY session lines (PST)", value=False)

        st.subheader("News")
        show_news = st.checkbox("Show news markers", value=False)
        news_days = st.slider("News window (days)", 1, 30, 7, 1)

        st.subheader("NTD Panel")
        show_npx = st.checkbox("Overlay NPX", value=True)
        show_inrange = st.checkbox("Shade S↔R In-Range", value=True)
        show_triangles = st.checkbox("Triangles (trend filter)", value=True)
        show_stars = st.checkbox("Stars (S/R reversals)", value=True)
        prox = st.number_input("S/R proximity (fraction)", min_value=0.0, max_value=0.05, value=0.0025, step=0.0005, format="%.4f")
        bars_confirm = st.slider("S/R reversal confirm bars", 1, 5, 2, 1)

        show_hma_rev = st.checkbox("HMA reversal markers", value=False)
        hma_period = st.slider("HMA period", 10, 200, 55, 1)
        hma_rev_lb = st.slider("HMA reversal lookback", 2, 10, 3, 1)

        st.subheader("Ichimoku Params")
        ich_conv = st.number_input("Tenkan (conv)", 3, 30, 9, 1)
        ich_base = st.number_input("Kijun (base)", 10, 80, 26, 1)
        ich_span_b = st.number_input("Span B", 20, 120, 52, 1)

        st.subheader("Supertrend Params")
        st_atr = st.number_input("ATR period", 3, 50, 10, 1)
        st_mult = st.number_input("ATR multiplier", 0.5, 10.0, 3.0, 0.5)

        st.subheader("PSAR Params")
        psar_step = st.number_input("Step", 0.005, 0.2, 0.02, 0.005, format="%.3f")
        psar_max = st.number_input("Max step", 0.05, 0.5, 0.2, 0.05, format="%.2f")

    if not symbols:
        st.warning("Enter at least one symbol.")
        return

    tab1, tab2, tab3 = st.tabs(["Charts", "Scanners", "News"])

    with tab1:
        st.subheader("Charts")
        sym = st.selectbox("Select symbol", options=symbols, index=0)

        # Daily series in the same daily-view window as price chart
        close_full = _coerce_1d_series(fetch_hist(sym)).dropna()
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()

        # OHLC for overlays (Ichimoku / Supertrend / PSAR)
        ohlc = fetch_hist_ohlc(sym)
        ohlc_show = None
        if ohlc is not None and not ohlc.empty and isinstance(close_show.index, pd.DatetimeIndex):
            try:
                ohlc = ohlc.sort_index()
                x0, x1 = close_show.index[0], close_show.index[-1]
                ohlc_show = ohlc.loc[(ohlc.index >= x0) & (ohlc.index <= x1)]
        ...
        fig_price, sup, res, kijun = _plot_price_panel(
            sym,
            close_show=close_show,
            ohlc_show=ohlc_show,
            slope_lb=int(slope_lb),
            show_regression=bool(show_regression),
            show_fibs=bool(show_fibs),
            show_sr=bool(show_sr),
            sr_lb=int(sr_lb),
            show_ichimoku=bool(show_ichimoku),
            ich_conv=int(ich_conv), ich_base=int(ich_base), ich_span_b=int(ich_span_b),
            show_supertrend=bool(show_supertrend), st_atr=int(st_atr), st_mult=float(st_mult),
            show_psar=bool(show_psar), psar_step=float(psar_step), psar_max=float(psar_max),
            show_session_lines=bool(show_session),
            show_news=bool(show_news),
            news_window_days=int(news_days),
        )
        st.pyplot(fig_price, use_container_width=True)

        fig_ntd = _plot_ntd_panel(
            sym,
            close_show=close_show,
            ntd_win=int(ntd_win),
            ...
            sup=sup,
            res=res,
            show_triangles=bool(show_triangles),
            show_stars=bool(show_stars),
            prox=float(prox),
            bars_confirm=int(bars_confirm),
            show_hma_rev=bool(show_hma_rev),
    ...
    with tab2:
        st.subheader("Scanners (Daily)")
        colA, colB = st.columns(2)

        with colA:
            st.markdown("### Band Bounce (2σ)")
            max_bars = st.slider("Max bars since bounce", 0, 10, 2, 1)
            if st.button("Run: Band Bounce (Daily)", use_container_width=True):
                df = run_scan(symbols, last_band_bounce_signal_daily, int(slope_lb))
                if not df.empty:
                    df = df[df["Bars Since"].astype(int) <= int(max_bars)]
                show_scanner_table(df, sort_by="Bars Since", ascending=True)

            st.markdown("### Regression Trend Reversal Buckets (UP/DOWN)")
            max_bars2 = st.slider("Max bars since bounce (bucket scan)", 0, 10, 2, 1)
            if st.button("Run: Regression Trend Reversal Buckets", use_container_width=True):
                df = run_scan(symbols, daily_regression_trend_reversal_scan, daily_view_label, int(slope_lb), int(max_bars2))
                show_scanner_table(df, sort_by="Bars Since Bounce", ascending=True)

        with colB:
            st.markdown("### Ichimoku Kijun Cross-Up (Daily)")
            within_n = st.slider("Cross within last N bars", 0, 30, 5, 1)
            if st.button("Run: Kijun Cross-Up", use_container_width=True):
                df = run_scan(
                    symbols,
                ...
                show_scanner_table(df, sort_by="Bars Since Cross", ascending=True)

            st.markdown("### R² Rank (Daily)")
            if st.button("Run: R² Rank", use_container_width=True):
                rows = []
                for sym in symbols:
                    r2, m, ts = daily_regression_r2(sym, int(slope_lb))
                    rows.append({"Symbol": sym, "R2": r2, "Slope": m, "As Of": ts})
                df = pd.DataFrame(rows)
                try:
        ...
        st.divider()
        st.subheader("NPX/NTD helpers (Daily)")
        colC, colD = st.columns(2)

        with colC:
            st.markdown("### NPX↑NTD in Uptrend")
            if st.button("Run: NPX↑NTD (Uptrend)", use_container_width=True):
                df = run_scan(symbols, last_daily_npx_cross_up_in_uptrend, int(ntd_win), daily_view_label)
                show_scanner_table(df, sort_by="Bars Since", ascending=True)

        with colD:
            st.markdown("### Support reversal heading up (Daily)")
            sr_prox = st.number_input("Support proximity (fraction)", min_value=0.0, max_value=0.05, value=0.0025, step=0.0005, format="%.4f")
            hz = st.slider("Touch horizon (bars)", 1, 60, 15, 1)
            if st.button("Run: Support reversal heading up", use_container_width=True):
                df = run_scan(symbols, daily_support_reversal_heading_up, daily_view_label, int(sr_lb), float(sr_prox), int(bars_confirm), int(hz))
                show_scanner_table(df, sort_by="Bars Since Touch", ascending=True)

        st.divider()
        st.subheader("Fib + NPX Zero-Cross reversal (Daily)")
        colE, colF = st.columns(2)
        with colE:
            fib_prox = st.number_input("Fib proximity (fraction of price)", min_value=0.0, max_value=0.05, value=0.0025, step=0.0005, format="%.4f")
            fib_lb = st.slider("Fib lookback bars", 20, 400, 120, 10)
            fib_slope_lb = st.slider("Slope lookback (confirm)", 10, 200, 60, 5)
            npx_conf = st.slider("NPX confirm bars", 1, 5, 1, 1)
        with colF:
            dir_opt = st.selectbox("Direction", options=["BUY (touch low + NPX↑0 + slope flip)", "SELL (touch high + NPX↓0 + slope flip)"], index=0)
            if st.button("Run: Fib + NPX Zero signal", use_container_width=True):
                direction = "buy" if dir_opt.startswith("BUY") else "sell"
                df = run_scan(
                    symbols,
    ...
    with tab3:
        st.subheader("News")
        sym = st.selectbox("Symbol for news", options=symbols, index=0, key="news_sym")
        _render_news_list(sym, window_days=int(news_days))
