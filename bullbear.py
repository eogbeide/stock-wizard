...
with tab1:
    ...
    # 2) Choose which charts
    chart = st.radio("Chart View:", ["Daily","Hourly","Both"], key="orig_chart")

    # 3) Run forecast and fetch data
    if st.button("Run Forecast", key="orig_btn"):
        ...
        # intraday fetch
        st.session_state.intraday = yf.download(ticker, period="1d", interval="5m")
    ...
    if st.session_state.run_all:
        ...
        # Daily forecast & indicators with trendline
        if chart in ("Daily","Both"):
            ...
            ax.plot(hist.index, hist, label="History")
            ax.plot(hist.index, trend, "--", label="Trend")
        ...
        if chart in ("Hourly","Both"):
            ...
            if intraday.empty:
                ...
                ax2.plot(hc.index, hc.ewm(span=30).mean(), "--", label="Trend")
...
with tab2:
    ...
    if not st.session_state.run_all:
        ...
        # Enhanced daily view with trendline
        if view in ("Daily","Both"):
            ...
            ax.fill_between(idx, ci.iloc[:,0], ci.iloc[:,1], alpha=0.3)
            # Fibonacci levels
            high, low = hist.max(), hist.min()
            diff = high - low
            for lev in (0.236, 0.382, 0.5, 0.618):
                ax.hlines(high - diff*lev, hist.index[0], hist.index[-1], linestyles="dotted")
        ...
        if view in ("Intraday","Both"):
            ...
            if intraday.empty:
                ...
                # Trendline on intraday
                x2 = np.arange(len(ic))
                slope2, intercept2 = np.polyfit(x2, ic.values, 1)
                trend2 = slope2 * x2 + intercept2

                fig3, ax3 = plt.subplots(figsize=(14,5))
                ax3.plot(ic.index, ic, label="Intraday")
                ax3.plot(ic.index, trend2, "--", label="Trend")
...
with tab3:
    ...
    if not st.session_state.run_all:
        ...
        st.markdown("---")
        st.write(f"Data for **{st.session_state.ticker}** over the past **{bb_period}**")
