...
if st.button("Forecast"):
    ...
    # Step 5: Forecast 20-day and 200-day EMA
    # Combine historical data with forecasted values for EMA calculation
    combined_data = pd.concat([data, forecast_values.rename('Forecasted Price')])
    
    # Calculate the future EMAs based on the combined data
    future_ema_20 = combined_data.ewm(span=20, adjust=False).mean()
    future_ema_200 = combined_data.ewm(span=200, adjust=False).mean()

    # Step 6: Plot historical data, forecast, and EMA
    plt.figure(figsize=(14, 7))
    plt.plot(data[-180:], label='Last 6 Months Historical Data', color='blue')  # Last 6 months of historical data
    plt.plot(ema_20[-180:], label='20-Day EMA', color='red', linestyle='--')  # Historical 20-day EMA
    plt.plot(ema_200[-180:], label='200-Day EMA', color='green', linestyle='--')  # Historical 200-day EMA
    plt.plot(forecast_index, forecast_values, label='3 Months Forecast', color='orange')  # Forecast
    plt.fill_between(forecast_index, conf_int.iloc[:, 0], conf_int.iloc[:, 1], color='orange', alpha=0.3)  # Confidence intervals
    
    # Add forecasted EMAs to the plot
    plt.plot(forecast_index, future_ema_20[-forecast_steps:], label='Forecasted 20-Day EMA', color='red', linestyle='-.')
    plt.plot(forecast_index, future_ema_200[-forecast_steps:], label='Forecasted 200-Day EMA', color='green', linestyle='-.')
