...
if st.button("Forecast"):
    ...
    if len(prices) < 30:  # Ensure there are enough data points
        st.error("Not enough data points to perform the forecast. Please select a different stock or check the date range.")
    else:
        # Calculate 200-day EMA
        ema_200 = prices.ewm(span=200, adjust=False).mean()

        # Calculate daily moving average (e.g., 30-day) only if there are enough data points
        moving_average = prices.rolling(window=30).mean() if len(prices) >= 30 else pd.Series(np.nan, index=prices.index)
        ...
        order = (0, 1, 0)  # Updated order
        seasonal_order = (1, 0, 0, 12)  # Updated seasonal order
