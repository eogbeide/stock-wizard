...
def compute_supertrend(df: pd.DataFrame, atr_period: int = 10, atr_mult: float = 3.0) -> pd.DataFrame:
    ...
    NOTE (THIS REQUEST): used on BOTH Daily + Hourly price charts; default params are length=10, factor=3.
    """
    if df is None or df.empty or not {"High","Low","Close"}.issubset(df.columns):
    ...
    for i in range(len(ohlc)):
        if i == 0:
            ...
            st_line.iloc[i] = float(lowerband.iloc[i]) if np.isfinite(lowerband.iloc[i]) else np.nan
            continue
...
# ---------------------------
# NEW (THIS REQUEST): Regression Trend helpers (Daily)
#   (a) slope > 0 and price just reversed from LOWER +2σ/-2σ band going UP
#   (b) slope > 0 and price just reversed from UPPER +2σ/-2σ band going DOWN
#   NOTE: tab UI added later; helper here keeps existing UI untouched.
# ---------------------------
@st.cache_data(ttl=120)
def last_regression_trend_reversal_daily(symbol: str,
    ...
    """
    which:
      - "lower_up": (a) slope>0 and prev close <= lower band, now back inside & moving up
      - "upper_down": (b) slope>0 and prev close >= upper band, now back inside & moving down
    """
    try:
        ...
        if close_show.empty or len(close_show) < max(5, int(slope_lb)):
            return None

        yhat, upper, lower, m, r2 = regression_with_band(close_show, lookback=int(slope_lb), z=2.0)
        if yhat.empty or upper.empty or lower.empty:
            return None
        if not (np.isfinite(m) and float(m) > 0.0):
            return None

        c = close_show.reindex(yhat.index)
        u = upper.reindex(yhat.index)
        l = lower.reindex(yhat.index)
        ...
        if which == "lower_up":
            # prev touched/breached lower; now inside (>= lower) and moving up
            rev = (c >= l) & (c.shift(1) <= l.shift(1)) & (c > c.shift(1))
            label = "(a) Lower -2σ reversal UP"
        else:
            # prev touched/breached upper; now inside (<= upper) and moving down
            rev = (c <= u) & (c.shift(1) >= u.shift(1)) & (c < c.shift(1))
            label = "(b) Upper +2σ reversal DOWN"
        ...
        px = float(c.loc[t]) if np.isfinite(c.loc[t]) else np.nan
        prev_px = float(c.shift(1).loc[t]) if (t in c.index and np.isfinite(c.shift(1).loc[t])) else np.nan
        band_val = float(l.loc[t]) if which == "lower_up" else float(u.loc[t])
...
def _maybe_add_legend(ax, loc="best"):
    try:
        ax.legend(loc=loc, fontsize=8, frameon=True, framealpha=0.85)
    except Exception:
        pass

def plot_price_with_overlays(ax,
                             df_ohlc: pd.DataFrame,
                             close: pd.Series,
                             daily_view_label: str,
                             slope_lb: int,
                             show_regression_band: bool = True,
                             show_fibs: bool = True,
    ...
    # Regression band (+/-2σ)
    band_info = {}
    if show_regression_band and len(c) >= max(10, int(slope_lb)):
        yhat, upper, lower, m, r2 = regression_with_band(c, lookback=int(slope_lb), z=2.0)
        ax.plot(c.index, c.values, linewidth=1.3, label="Close")
        if not yhat.empty:
            ax.plot(yhat.index, yhat.values, linewidth=1.2, linestyle="--", label="Regress")
        if not upper.empty and not lower.empty:
            ax.plot(upper.index, upper.values, linewidth=1.0, linestyle=":", label="+2σ")
            ax.plot(lower.index, lower.values, linewidth=1.0, linestyle=":", label="-2σ")
    ...
    # Fibonacci levels (computed from visible series)
    if show_fibs:
        fibs = fibonacci_levels(c)
        if fibs:
            for k, v in fibs.items():
                if np.isfinite(v):
                    ax.axhline(v, linewidth=0.8, alpha=0.35, linestyle="--")
            # lightweight label
            hi = fibs.get("0%", np.nan)
            lo = fibs.get("100%", np.nan)
            if np.isfinite(hi) and np.isfinite(lo):
                ax.text(0.01, 0.98, f"Fib Hi={_fmt_float(hi)}  Lo={_fmt_float(lo)}",
                        transform=ax.transAxes, va="top", ha="left",
                        fontsize=9, bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="black", alpha=0.75))

    # Ichimoku (optional; uses OHLC)
    if show_ichimoku and df_ohlc is not None and {"High","Low","Close"}.issubset(df_ohlc.columns):
        tenkan, kijun, senkou_a, senkou_b, chikou = ichimoku_lines(df_ohlc["High"], df_ohlc["Low"], df_ohlc["Close"])
        if not tenkan.empty:  ax.plot(tenkan.index, tenkan.values, linewidth=0.9, label="Tenkan")
        if not kijun.empty:   ax.plot(kijun.index, kijun.values, linewidth=0.9, label="Kijun")
        if not senkou_a.empty: ax.plot(senkou_a.index, senkou_a.values, linewidth=0.8, label="Senkou A")
        if not senkou_b.empty: ax.plot(senkou_b.index, senkou_b.values, linewidth=0.8, label="Senkou B")

    # Supertrend (requested: used for BOTH Daily + Hourly)
    st_df = None
    if show_supertrend and df_ohlc is not None and {"High","Low","Close"}.issubset(df_ohlc.columns):
        st_df = compute_supertrend(df_ohlc[["High","Low","Close"]], atr_period=int(supertrend_len), atr_mult=float(supertrend_factor))
        if not st_df.empty and "ST" in st_df:
            ax.plot(st_df.index, st_df["ST"].values, linewidth=1.1, label=f"Supertrend({supertrend_len},{supertrend_factor:g})")

    # PSAR
    ps_df = None
    if show_psar and df_ohlc is not None and {"High","Low"}.issubset(df_ohlc.columns):
        ps_df = compute_psar_from_ohlc(df_ohlc[["High","Low"]], step=float(psar_step), max_step=float(psar_max))
        if not ps_df.empty and "PSAR" in ps_df:
            ax.scatter(ps_df.index, ps_df["PSAR"].values, s=10, alpha=0.75, label="PSAR")

    ax.set_title(title)
    ax.grid(True, alpha=0.25)
    ax.set_ylabel("Price")
    return {"band": band_info, "supertrend": st_df, "psar": ps_df}

def plot_ntd_panel(ax,
                   close: pd.Series,
                   ntd_win: int,
                   slope_lb: int,
                   show_npx: bool = True,
    ...
    c = _coerce_1d_series(close).ffill().dropna()
    if c.empty:
        return {}
    ...
    ntd = _coerce_1d_series(ntd).reindex(c.index)
    npx = _coerce_1d_series(npx).reindex(c.index)

    ax.plot(ntd.index, ntd.values, linewidth=1.4, label=f"NTD({ntd_win})")
    ax.axhline(0.0, linewidth=1.0, alpha=0.35, linestyle="--")
    ax.axhline(0.75, linewidth=0.8, alpha=0.25, linestyle=":")
    ax.axhline(-0.75, linewidth=0.8, alpha=0.25, linestyle=":")

    # Trend slope from regression of price
    _, _, _, m, r2 = regression_with_band(c, lookback=int(slope_lb))
    m = float(m) if np.isfinite(m) else np.nan

    # Support/Resistance from rolling midline + range (simple but stable)
    # (Assumes helpers exist elsewhere; fall back to rolling midlines)
    sup = rolling_midline(c, window=max(10, int(ntd_win))) - c.rolling(max(10, int(ntd_win))).std(ddof=0)
    res = rolling_midline(c, window=max(10, int(ntd_win))) + c.rolling(max(10, int(ntd_win))).std(ddof=0)
    ...
    if show_triangles and np.isfinite(m):
        overlay_ntd_triangles_by_trend(ax, ntd, trend_slope=m, upper=0.75, lower=-0.75)

    if show_sr_stars and np.isfinite(m):
        overlay_ntd_sr_reversal_stars(ax, c, sup, res, trend_slope=m, ntd=ntd, prox=0.0025, bars_confirm=2)

    # HMA reversal markers (if HMA helper exists; otherwise skip)
    if show_hma_rev:
        try:
            hma = hull_moving_average(c, period=55)
            overlay_hma_reversal_on_ntd(ax, c, hma, lookback=3, period=55, ntd=ntd)
        except Exception:
            pass
    ...
    ax.set_ylabel("NTD")
    ax.set_ylim(-1.2, 1.2)
    return {"ntd": ntd, "npx": npx, "slope": m, "r2": float(r2) if np.isfinite(r2) else np.nan}

def plot_volume_panel(ax, df_ohlc: pd.DataFrame):
    if df_ohlc is None or df_ohlc.empty or "Volume" not in df_ohlc.columns:
        ax.set_axis_off()
        return
    v = _coerce_1d_series(df_ohlc["Volume"])
    if not _has_volume_to_plot(v):
        ax.set_axis_off()
        return
    ax.bar(v.index, v.values, width=0.9)
    ax.set_title("Volume")
    ax.grid(True, alpha=0.20)

def make_daily_figure(symbol: str,
                      daily_view_label: str,
                      ntd_win: int,
                      slope_lb: int,
                      show_news: bool = True,
    ...
    # Build a minimal OHLC proxy if fetch_hist returns only Close; else prefer actual OHLC if present
    try:
        df = yf.download(symbol, period="2y", interval="1d", progress=False, auto_adjust=False)
        df = df.tz_localize("UTC") if isinstance(df.index, pd.DatetimeIndex) and df.index.tz is None else df
        df = df.tz_convert(PACIFIC) if isinstance(df.index, pd.DatetimeIndex) else df
        df = df.rename(columns=str.title)
        df_show = subset_by_daily_view(df, daily_view_label) if not df.empty else pd.DataFrame()
    except Exception:
        df_show = pd.DataFrame()

    fig = plt.figure(figsize=(14, 8))
    gs = fig.add_gridspec(3, 1, height_ratios=[3.0, 1.6, 0.9], hspace=0.12)
    ax1 = fig.add_subplot(gs[0, 0])
    ax2 = fig.add_subplot(gs[1, 0], sharex=ax1)
    ax3 = fig.add_subplot(gs[2, 0], sharex=ax1)

    # Price panel
    overlays = plot_price_with_overlays(
        ax1,
        df_show if (df_show is not None and not df_show.empty) else None,
        close,
        daily_view_label=daily_view_label,
        slope_lb=int(slope_lb),
        show_regression_band=True,
        show_fibs=True,
        show_ichimoku=bool(show_ichimoku),
        show_supertrend=bool(show_supertrend),
        show_psar=bool(show_psar),
        title=f"{symbol} — Daily ({daily_view_label})"
    )

    # News vertical markers
    if show_news:
        try:
            news = fetch_yf_news(symbol, window_days=7)
            if news is not None and not news.empty:
                draw_news_markers(ax1, list(news["time"].values), label="News (7d)")
        except Exception:
            pass

    # Sessions (mostly useful on intraday; kept optional)
    if show_sessions and isinstance(close.index, pd.DatetimeIndex) and close.index.tz is not None:
        ...
        h, l = draw_session_lines(ax1, lines, alpha=0.25)
        # legend will pick them up via handles
        try:
            ax1.legend(handles=h, labels=l, fontsize=8, frameon=True, framealpha=0.85, loc="upper left")
        except Exception:
            pass

    # NTD panel
    ntd_info = plot_ntd_panel(ax2, close, ntd_win=int(ntd_win), slope_lb=int(slope_lb))

    # Annotate “Reverse Possible” using fib-touch + slope flip logic
    try:
        rev = regression_slope_reversal_at_fib_extremes(close, slope_lb=int(slope_lb),
                                                        proximity_pct_of_range=0.02, confirm_bars=2, lookback_bars=120)
        if isinstance(rev, dict):
            annotate_reverse_possible(ax1, rev, text="Reverse Possible")
    except Exception:
        pass

    # Volume panel
    plot_volume_panel(ax3, df_show if (df_show is not None and not df_show.empty) else None)

    for ax in (ax1, ax2):
        _maybe_add_legend(ax, loc="best")
    ...
    # Quick stats header
    m = ntd_info.get("slope", np.nan)
    r2 = ntd_info.get("r2", np.nan)
    ax1.text(0.99, 0.02, f"Slope={_fmt_float(m,4)}  R²={_fmt_float(r2,3)}",
             transform=ax1.transAxes, ha="right", va="bottom", fontsize=9,
             bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="black", alpha=0.75))

    return fig

def make_hourly_figure(symbol: str,
                       intraday_period: str,
                       ntd_win: int,
                       slope_lb: int,
                       show_news: bool = True,
                       ...
                       show_psar: bool = False,
                       show_ichimoku: bool = False):
    df = fetch_intraday(symbol, period=intraday_period)
    if df is None or df.empty or "Close" not in df.columns:
        fig = plt.figure(figsize=(12, 6))
        ax = fig.add_subplot(111)
        ax.set_title(f"{symbol} — hourly ({intraday_period}) no data")
        return fig
    ...
    if c.empty:
        fig = plt.figure(figsize=(12, 6))
        ax = fig.add_subplot(111)
        ax.set_title(f"{symbol} — hourly ({intraday_period}) no close")
        return fig
    ...
    # Price panel (Supertrend requested on hourly too)
    plot_price_with_overlays(
        ax1,
        df[["High","Low","Close","Volume"]].copy() if {"High","Low","Close"}.issubset(df.columns) else None,
        c,
        daily_view_label="(intraday)",
        slope_lb=int(slope_lb),
        show_regression_band=True,
        show_fibs=True,
        show_ichimoku=bool(show_ichimoku),
        show_supertrend=bool(show_supertrend),
        show_psar=bool(show_psar),
        title=f"{symbol} — Hourly ({intraday_period})"
    )

    if show_sessions and isinstance(c.index, pd.DatetimeIndex) and c.index.tz is not None:
        lines = compute_session_lines(c.index)
        h, l = draw_session_lines(ax1, lines, alpha=0.22)
        try:
            ax1.legend(handles=h, labels=l, fontsize=8, frameon=True, framealpha=0.85, loc="upper left")
        except Exception:
            pass
    ...
    if show_news:
        try:
            news = fetch_yf_news(symbol, window_days=3)
            if news is not None and not news.empty:
                # Only draw markers if timestamps fall in chart range
                times = [t for t in news["time"].tolist() if (c.index.min() <= t <= c.index.max())]
                if times:
                    draw_news_markers(ax1, times, label="News (3d)")
        except Exception:
            pass

    plot_ntd_panel(ax2, c, ntd_win=int(ntd_win), slope_lb=int(slope_lb))
    plot_volume_panel(ax3, df)
...
def scan_recent_band_bounce(symbols, slope_lb: int, intraday_period: str):
    rows = []
    for sym in symbols:
        d = last_band_bounce_signal_daily(sym, slope_lb=int(slope_lb))
        if isinstance(d, dict):
            rows.append(d)
        h = last_band_bounce_signal_hourly(sym, period=intraday_period, slope_lb=int(slope_lb))
        if isinstance(h, dict):
            rows.append(h)
    df = pd.DataFrame(rows)
    if df.empty:
        return df
    # prefer newest first; then by Bars Since
    if "Bars Since" in df.columns:
        df = df.sort_values(["Bars Since"], ascending=[True])
    return df.reset_index(drop=True)

def scan_regression_trend_reversals(symbols,
                                    daily_view_label: str,
                                    slope_lb: int,
    ...
    rows = []
    for sym in symbols:
        r = last_regression_trend_reversal_daily(sym,
                                                 daily_view_label=daily_view_label,
                                                 slope_lb=int(slope_lb),
                                                 within_last_n_bars=int(within_last_n_bars),
                                                 which=str(which))
...
def scan_npx_crosses(symbols,
    ...
    rows = []
    for sym in symbols:
        a = last_daily_npx_cross_up_in_uptrend(sym, ntd_win=int(ntd_win), daily_view_label=daily_view_label)
        if isinstance(a, dict):
            rows.append(a)
        b = last_daily_npx_zero_cross_with_local_slope(sym,
                                                       ntd_win=int(ntd_win),
                                                       daily_view_label=daily_view_label,
                                                       local_slope_lb=int(local_slope_lb),
                                                       max_abs_npx_at_cross=float(max_abs_npx_at_cross),
                                                       direction="up")
        if isinstance(b, dict):
            rows.append(b)
        c = last_daily_npx_zero_cross_with_local_slope(sym,
                                                       ntd_win=int(ntd_win),
                                                       daily_view_label=daily_view_label,
                                                       local_slope_lb=int(local_slope_lb),
                                                       max_abs_npx_at_cross=float(max_abs_npx_at_cross),
...
def _download_df_button(df: pd.DataFrame, label: str, filename: str):
    if df is None or df.empty:
        return
    try:
...
def app():
    ...
    st.title("BullBear — Regression/NTD dashboard")

    with st.sidebar:
        st.header("Universe")
        default_watch = "SPY, QQQ, IWM, DIA, AAPL, MSFT, NVDA, AMZN, GOOGL, META, TSLA"
        watch_text = st.text_area("Symbols (comma or newline)", value=default_watch, height=110)
        symbols = _parse_symbol_list(watch_text)
        if not symbols:
        ...
        daily_view_label = st.selectbox("Daily View", options=["3mo", "6mo", "1y", "2y"], index=2)
        intraday_period = st.selectbox("Hourly Period", options=["1d", "5d", "1mo", "3mo"], index=1)

        ntd_win = st.slider("NTD / NPX Window", min_value=10, max_value=200, value=55, step=1)
        slope_lb = st.slider("Regression Lookback", min_value=10, max_value=250, value=90, step=1)

        st.header("Overlays")
        show_news = st.checkbox("News markers", value=True)
        show_sessions = st.checkbox("London/NY session lines", value=True)
        ...
        within_last_n_bars = st.slider("Trend reversal: within last N bars", 0, 10, 2, 1)
        local_slope_lb = st.slider("NPX local slope lookback", 5, 120, 25, 1)
        max_abs_npx_at_cross = st.slider("NPX cross proximity (eps)", 0.01, 0.50, 0.10, 0.01)

    tab_chart, tab_scans, tab_about = st.tabs(["Charts", "Scanners", "About"])
    ...
    with tab_chart:
        ...
        with col1:
            st.subheader("Daily")
            fig_d = make_daily_figure(
                symbol=symbol,
                daily_view_label=daily_view_label,
                ntd_win=int(ntd_win),
                slope_lb=int(slope_lb),
                show_news=bool(show_news),
                show_sessions=False,  # daily: usually not needed
                show_supertrend=bool(show_supertrend),
                show_psar=bool(show_psar),
                show_ichimoku=bool(show_ichimoku),
            )
            st.pyplot(fig_d, clear_figure=True, use_container_width=True)

        with col2:
            st.subheader("Hourly")
            fig_h = make_hourly_figure(
                symbol=symbol,
    ...
    with tab_scans:
        ...
        s1, s2, s3 = st.tabs([
            "Recent BUY/SELL (Band Bounce)",
            "Regression Trend Reversals",
            "NPX Signals"
        ])

        with s1:
            df = scan_recent_band_bounce(symbols, slope_lb=int(slope_lb), intraday_period=intraday_period)
            st.dataframe(df, use_container_width=True, hide_index=True)
            _download_df_button(df, "Download CSV", "band_bounce_scanner.csv")

        with s2:
            st.markdown("**Daily Trend Reversals (slope > 0)** — detects 2σ band touches then reversal back inside the band.")

            t_a, t_b = st.tabs([
                "(a) Lower -2σ reversal UP",
                "(b) Upper +2σ reversal DOWN"
            ])

            with t_a:
                df_a = scan_regression_trend_reversals(
                    symbols,
                    daily_view_label=daily_view_label,
                    slope_lb=int(slope_lb),
                    within_last_n_bars=int(within_last_n_bars),
                ...
                _download_df_button(df_a, "Download CSV", "regression_trend_reversal_a_lower_up.csv")

            with t_b:
                # (b) YES — included per your confirmation
                df_b = scan_regression_trend_reversals(
                    symbols,
                    daily_view_label=daily_view_label,
                    slope_lb=int(slope_lb),
                    within_last_n_bars=int(within_last_n_bars),
                ...
                _download_df_button(df_b, "Download CSV", "regression_trend_reversal_b_upper_down.csv")

        with s3:
            df_npx = scan_npx_crosses(
                symbols,
                ntd_win=int(ntd_win),
                daily_view_label=daily_view_label,
                local_slope_lb=int(local_slope_lb),
            ...
            st.dataframe(df_npx, use_container_width=True, hide_index=True)
            _download_df_button(df_npx, "Download CSV", "npx_signals.csv")

    with tab_about:
        st.markdown(
            """
**What this app shows**
- **Price panel** with regression line + ±2σ bands, Fib levels, optional Ichimoku/PSAR, and **Supertrend** (daily + hourly).
- **NTD panel** (normalized trend deviation) with NPX overlay and signal markers.
- **Scanners** for band-bounce, **(a)/(b) regression trend reversals**, and NPX events.

**Notes**
- Timestamps are displayed in **Pacific time** where possible.
- Yahoo Finance news markers are from the symbol’s feed and filtered to recent days.
            """.strip()
        )
