...
# S/R Reversal line and cross scanner
st.sidebar.subheader("S/R Reversal Line (NTD)")
show_sr_reversal_line = st.sidebar.checkbox(
    "Show S/R Reversal line on NTD panels",
    value=True,
    key="sb_show_sr_reversal_line"
)
sr_reversal_smooth = st.sidebar.slider(
    "S/R Reversal line smoothing",
    1, 50, 8, 1,
    key="sb_sr_reversal_smooth"
)
sr_cross_recent_daily = st.sidebar.slider(
    "Recent daily S/R zero-cross window (bars)",
    1, 60, 10, 1,
    key="sb_sr_cross_recent_daily"
)
sr_cross_recent_hourly = st.sidebar.slider(
    "Recent hourly S/R zero-cross window (bars)",
    1, 120, 24, 1,
    key="sb_sr_cross_recent_hourly"
)
...
def find_band_bounce_signal(price: pd.Series,
    ...
    """
    Detect the most recent BUY/SELL signal from a 'bounce' off the ±2σ band,
    optionally requiring local-vs-global slope alignment.
    """
...
def overlay_hma_reversal_on_ntd(ax, price: pd.Series, hma: pd.Series,
                                ...
                                upper_thr: float = 0.75, lower_thr: float = -0.75):
    ...
    if len(idx_up):
        ax.scatter(idx_up, [y_up]*len(idx_up), marker="s", s=70, color="tab:green", zorder=8,
                   label=f"HMA({period}) REV")
    if len(idx_dn):
        ax.scatter(idx_dn, [y_dn]*len(idx_dn), marker="D", s=70, color="tab:red",   zorder=8,
                   label=f"HMA({period}) REV")
...
def overlay_ntd_triangles_by_trend(ax, ntd: pd.Series, trend_slope: float,
    ...
    if s.empty or not np.isfinite(trend_slope):
        return
    uptrend = trend_slope > 0; downtrend = trend_slope < 0
...
def overlay_ntd_sr_reversal_stars(ax, price: pd.Series, sup: pd.Series, res: pd.Series,
                                  trend_slope: float, ntd: pd.Series, prox: float = 0.0025, bars_confirm: int = 2):
    ...
    if not (t in s_sup.index and t in s_res.index and t in s_ntd.index):
        return
    c0 = float(p.iloc[-1]); c1 = float(p.iloc[-2]) if len(p) >= 2 else np.nan
...
def last_green_dot_daily(symbol: str, ntd_win: int, level: float = None):
    try:
        ...
        if df.shape[0] < 2:
            return False, np.nan, None, np.nan, np.nan
        above = df["npx"] > df["ntd"]
        above_now = bool(above.iloc[-1]); above_prev = bool(above.iloc[-2])
        is_signal = (not above_prev) and above_now
        ntd_last = float(df["ntd"].iloc[-1]); npx_last = float(df["npx"].iloc[-1]); idx_last = df.index[-1]
...
def last_green_dot_hourly(symbol: str, ntd_win: int, period: str = "1d", level: float = None):
    try:
        ...
        above_now = bool(above.iloc[-1]); above_prev = bool(above.iloc[-2])
        is_signal = (not above_prev) and above_now
        ntd_last = float(df2["ntd"].iloc[-1]); npx_last = float(df2["npx"].iloc[-1]); idx_last = df2.index[-1]
...
# ========= S/R Reversal line + zero-cross scanner =========
def compute_sr_reversal_line(price: pd.Series,
                             support: pd.Series,
                             resistance: pd.Series,
                             smooth_span: int = 8) -> pd.Series:
    """
    Normalized S/R Reversal line for NTD panels and scanner.

    The line measures where price sits inside the rolling support/resistance
    channel:
      - near +1: closer to resistance
      - near -1: closer to support
      - 0.0: channel midpoint

    A 0.0 cross is useful for scanning symbols where price has recently
    reclaimed or lost the midpoint of the S/R channel.
    """
    p = _coerce_1d_series(price).astype(float)
    ...
    mid = (sup + res) / 2.0
    half_width = ((res - sup) / 2.0).replace(0, np.nan)
    line = ((p - mid) / half_width).replace([np.inf, -np.inf], np.nan).clip(-1.0, 1.0)

    try:
        span = max(1, int(smooth_span))
    except Exception:
        span = 8
    if span > 1:
        line = line.ewm(span=span, adjust=False, min_periods=1).mean()

    return line.reindex(p.index)


def sr_reversal_zero_cross_info(sr_line: pd.Series) -> dict:
    """
    Return information for the most recent 0.0 cross of the S/R Reversal line.
    """
    s = _coerce_1d_series(sr_line).dropna()
    if s.shape[0] < 2:
        return {
            "has_cross": False,
            "direction": None,
            ...
            "cross_time": None,
            "cross_value": np.nan,
            "current_value": _safe_last_float(s),
        }

    cross_up = (s.shift(1) <= 0.0) & (s > 0.0)
    cross_dn = (s.shift(1) >= 0.0) & (s < 0.0)

    events = []
    for pos, ts in enumerate(s.index):
        if bool(cross_up.iloc[pos]):
            events.append((pos, ts, "Upward", float(s.iloc[pos])))
        elif bool(cross_dn.iloc[pos]):
            events.append((pos, ts, "Downward", float(s.iloc[pos])))
    ...
    pos, ts, direction, val = events[-1]
    return {
        "has_cross": True,
        "direction": direction,
        "bars_since": int((len(s) - 1) - pos),
        "cross_time": ts,
        "cross_value": val,
...
@st.cache_data(ttl=120, show_spinner=False)
def sr_reversal_cross_scan_symbol(symbol: str,
                                  timeframe: str,
                                  hourly_period: str = "4d",
                                  daily_sr_lookback: int = 60,
                                  hourly_sr_lookback: int = 60,
                                  smooth_span: int = 8) -> dict:
    """
    Scan one symbol for the latest zero-line cross of the S/R Reversal line.
    timeframe must be 'Daily' or 'Hourly'.
    """
    try:
        if timeframe == "Daily":
            close = fetch_hist(symbol)
            if close is None or close.empty:
                return {"Symbol": symbol, "Chart": timeframe, "Status": "No data"}
            lb = max(2, int(daily_sr_lookback))
        else:
            df = fetch_intraday(symbol, period=hourly_period)
            if df is None or df.empty or "Close" not in df.columns:
                return {"Symbol": symbol, "Chart": timeframe, "Status": "No data"}
            close = _coerce_1d_series(df["Close"]).ffill().dropna()
            if close.empty:
                return {"Symbol": symbol, "Chart": timeframe, "Status": "No data"}
            lb = max(2, int(hourly_sr_lookback))

        support = close.rolling(lb, min_periods=1).min()
        resistance = close.rolling(lb, min_periods=1).max()
        sr_line = compute_sr_reversal_line(close, support, resistance, smooth_span=smooth_span)
        info = sr_reversal_zero_cross_info(sr_line)

        return {
            "Symbol": symbol,
            "Chart": timeframe,
            "Direction": info.get("direction"),
            "Bars Since": info.get("bars_since"),
            "Cross Time": info.get("cross_time"),
            "Cross Value": info.get("cross_value"),
            "Current Line": info.get("current_value"),
            "Last Close": _safe_last_float(close),
            "Status": "OK" if info.get("has_cross") else "No cross",
        }
    except Exception as e:
        return {"Symbol": symbol, "Chart": timeframe, "Status": f"Error: {type(e).__name__}"}


def format_scan_results_table(rows, recent_daily: int, recent_hourly: int) -> pd.DataFrame:
    df = pd.DataFrame(rows)
    if df.empty or "Bars Since" not in df.columns:
        return pd.DataFrame()

    def _is_recent(row):
        try:
            bars = int(row.get("Bars Since"))
        except Exception:
            return False
        chart = row.get("Chart")
        limit = int(recent_daily) if chart == "Daily" else int(recent_hourly)
        return bars <= limit

    df = df[df.apply(_is_recent, axis=1)].copy()
    if df.empty:
        return pd.DataFrame()

    df = df.sort_values(["Chart", "Bars Since", "Symbol"]).reset_index(drop=True)
    ...
    if "Cross Time" in df.columns:
        def _fmt_ts(x):
            try:
                ...
                return str(x)
        df["Cross Time"] = df["Cross Time"].map(_fmt_ts)

    for col in ["Cross Value", "Current Line"]:
        if col in df.columns:
            df[col] = df[col].map(lambda x: f"{float(x):+.3f}" if pd.notna(x) and np.isfinite(float(x)) else "n/a")
    if "Last Close" in df.columns:
        df["Last Close"] = df["Last Close"].map(lambda x: fmt_price_val(x) if pd.notna(x) and np.isfinite(float(x)) else "n/a")

    keep_cols = ["Symbol", "Chart", "Direction", "Bars Since", "Cross Time", "Current Line", "Last Close"]
...
def render_hourly_views(sel: str,
    ...
    # Local slope (lookback) & regression band
    ...
    # alignment test (local vs global)
    ...
    if align_ok_h:
        instr_txt = format_trade_instruction(
            trend_slope=m_global_h, buy_val=sup_val, sell_val=res_val, close_val=px_val, symbol=sel, use_take_profit=True
        )
    else:
        instr_txt = "MIXED — wait for alignment"

    # Reversal probability (uses local slope)
    ...
    ax2.set_title(
        f"{sel} Intraday ({hour_range_label})  ↑{fmt_pct(p_up)}  ↓{fmt_pct(p_dn)} — {instr_txt}  "
        f"[P(slope rev≤{rev_horizon} bars)={rev_txt_h}]"
    )

    # Top-of-chart big badge (only when aligned)
    if align_ok_h:
    ...
    # HMA + S/R marker (price area), alignment-gated
    hma_sr_sig_h = find_hma_sr_signal(hc, hma_h, sup_h, res_h, local_slope_h, sr_prox_pct, global_slope=m_global_h, require_align=True)
    if hma_sr_sig_h is not None:
        annotate_crossover(ax2, hma_sr_sig_h["time"], hma_sr_sig_h["price"], hma_sr_sig_h["side"], note="(HMA+S/R)")

    # Bottom info bars
    ax2.text(0.01, 0.02,
             f"Local slope: {fmt_slope(local_slope_h)}/bar  |  P(rev≤{rev_horizon}): {fmt_pct(rev_prob_h)}",
    ...
    if show_nrsi:
        ...
        overlay_ntd_sr_reversal_stars(ax2r, price=hc, sup=sup_h, res=res_h, trend_slope=m_local_h, ntd=ntd_h,
                                      prox=sr_prox_pct, bars_confirm=rev_bars_confirm)

        if show_sr_reversal_line:
            sr_line_h = compute_sr_reversal_line(
                hc,
                sup_h,
                res_h,
                smooth_span=sr_reversal_smooth
            )
            if not sr_line_h.dropna().empty:
                ax2r.plot(sr_line_h.index, sr_line_h.values, "-", linewidth=1.5,
                          color="tab:purple", alpha=0.9, label="S/R Reversal Line")
...
with tab1:
    ...
    if st.session_state.run_all and st.session_state.ticker == sel:
        ...
        if chart in ("Daily","Both"):
            ...
            if align_ok_d:
                instr_txt_d = format_trade_instruction(m_global_d, sup_val_d, res_val_d, px_val_d, symbol=sel, use_take_profit=True)
            else:
                instr_txt_d = "MIXED — wait for alignment"
            ax.set_title(
            ...
            if show_ntd and not ntd_d_show.dropna().empty:
                ...
                overlay_ntd_sr_reversal_stars(axdw, price=df_show, sup=sup_d_show, res=res_d_show,
                                              trend_slope=m_local_d, ntd=ntd_d_show, prox=sr_prox_pct,
                                              bars_confirm=rev_bars_confirm)
            if show_sr_reversal_line and len(df_show.index) > 0:
                sr_line_d_show = compute_sr_reversal_line(
                    df_show,
                    sup_d_show,
                    res_d_show,
                    smooth_span=sr_reversal_smooth
                )
                if not sr_line_d_show.dropna().empty:
                    axdw.plot(sr_line_d_show.index, sr_line_d_show.values, "-", linewidth=1.5,
                              color="tab:purple", alpha=0.9, label="S/R Reversal Line")
    ...
    if not st.session_state.run_all:
        ...
        if view in ("Daily","Both"):
            ...
            if not upper_d_show.empty and not lower_d_show.empty:
                ax.plot(upper_d_show.index, upper_d_show.values, "--", linewidth=2.2, color="black", alpha=0.85, label="Daily Trend +2σ")
                ax.plot(lower_d_show.index, lower_d_show.values, "--", linewidth=2.2, color="black", alpha=0.85, label="Daily Trend -2σ")
                bounce_sig_d2 = find_band_bounce_signal(df_show, upper_d_show, lower_d_show, m_local_d, global_slope=m_global_d2, require_align=True)
                if bounce_sig_d2 is not None:
                    annotate_crossover(ax, bounce_sig_d2["time"], bounce_sig_d2["price"], bounce_sig_d2["side"], note="(±2σ bounce)")
            hma_sr_sig_d2 = find_hma_sr_signal(df_show, hma_d2_show, sup_d2_show, res_d2_show, m_local_d, sr_prox_pct, global_slope=m_global_d2, require_align=True)
            if hma_sr_sig_d2 is not None:
                annotate_crossover(ax, hma_sr_sig_d2["time"], hma_sr_sig_d2["price"], hma_sr_sig_d2["side"], note="(HMA+S/R)")

            rev_txt_d2 = fmt_pct(rev_prob_d2) if np.isfinite(rev_prob_d2) else "n/a"
            if align_ok_d2:
                instr_txt_d2 = format_trade_instruction(m_global_d2, sup_val_d2, res_val_d2, px_val_d2, symbol=st.session_state.ticker, use_take_profit=True)
            else:
                instr_txt_d2 = "MIXED — wait for alignment"
            ax.set_title(
                f"{st.session_state.ticker} Daily — {daily_view} — ↑{fmt_pct(p_up)}  ↓{fmt_pct(p_dn)} — {instr_txt_d2}  "
                f"[P(slope rev≤{rev_horizon} bars)={rev_txt_d2}]  History, 30 EMA, S/R (w={sr_lb_daily}), Slope")
            if align_ok_d2:
                badge = "▲ BUY & TAKE PROFIT" if m_global_d2 > 0 else "▼ SELL & TAKE PROFIT"
                ax.text(0.01, 0.98, badge, transform=ax.transAxes, ha="left", va="top",
                        fontsize=11, fontweight="bold",
                        bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="grey", alpha=0.8))
            ...
            axdw2.axhline(-0.75, linestyle="-",  linewidth=1.0, color="black", label="-0.75")
            axdw2.set_ylim(-1.1, 1.1); axdw2.set_xlabel("Date (PST)")
...
# --- Tab 7: S/R Reversal Cross Scanner ---
with tab7:
    st.header("S/R Reversal Line Zero-Cross Scanner")
    st.caption(
        "This scans the S/R Reversal line used on the NTD chart and lists symbols where that line "
        "recently crossed the 0.0 midpoint. Upward means the line crossed from below/at 0 to above 0; "
        "downward means it crossed from above/at 0 to below 0."
    )

    c1, c2, c3, c4 = st.columns(4)
    scan_daily = c1.checkbox("Scan Daily", value=True, key="sr_scan_daily")
    scan_hourly = c2.checkbox("Scan Hourly", value=True, key="sr_scan_hourly")
    scan_hour_range = c3.selectbox("Hourly scan lookback", ["24h", "48h", "96h"], index=2, key="sr_scan_hour_range")
    scan_scope = c4.selectbox("Symbol scope", ["Current universe", "Selected ticker only"], index=0, key="sr_scan_scope")

    scan_period_map = {"24h": "1d", "48h": "2d", "96h": "4d"}
    scan_period = scan_period_map.get(scan_hour_range, "4d")

    if scan_scope == "Selected ticker only":
        selected_symbol = st.session_state.get("ticker") or (universe[0] if len(universe) else "")
        symbols_to_scan = [selected_symbol]
    else:
        symbols_to_scan = list(universe)

    st.write(
        f"Recent-cross filter: Daily ≤ **{sr_cross_recent_daily}** bars, "
        f"Hourly ≤ **{sr_cross_recent_hourly}** bars. "
        f"S/R line smoothing = **{sr_reversal_smooth}**, daily S/R lookback = **{sr_lb_daily}**, "
        f"hourly S/R lookback = **{sr_lb_hourly}**."
    )

    if st.button("Run S/R Reversal Cross Scan", key="btn_sr_reversal_cross_scan"):
        rows = []
        total_jobs = len(symbols_to_scan) * int(bool(scan_daily) + bool(scan_hourly))
        if total_jobs == 0:
            st.warning("Select Daily and/or Hourly to scan.")
        else:
            ...
            for sym in symbols_to_scan:
                if scan_daily:
                    rows.append(sr_reversal_cross_scan_symbol(
                        sym,
                        "Daily",
                        hourly_period=scan_period,
                        daily_sr_lookback=sr_lb_daily,
                        hourly_sr_lookback=sr_lb_hourly,
                        smooth_span=sr_reversal_smooth,
                    ))
                    done += 1
            ...
            progress.empty()
            result = format_scan_results_table(
                rows,
                recent_daily=sr_cross_recent_daily,
                recent_hourly=sr_cross_recent_hourly
            )

            c1m, c2m, c3m = st.columns(3)
            c1m.metric("Symbols scanned", len(symbols_to_scan))
            c2m.metric("Chart scans", total_jobs)
            c3m.metric("Recent crosses", int(result.shape[0]) if result is not None else 0)

            if result is None or result.empty:
                st.info("No recent S/R Reversal zero-line crosses found for the selected scan settings.")
                raw_df = pd.DataFrame(rows)
                if not raw_df.empty:
                    with st.expander("Show scan status"):
                        st.dataframe(raw_df.reset_index(drop=True), use_container_width=True)
            else:
                st.dataframe(result, use_container_width=True)
                up_count = int((result["Direction"] == "Upward").sum()) if "Direction" in result.columns else 0
                down_count = int((result["Direction"] == "Downward").sum()) if "Direction" in result.columns else 0
                c4m, c5m = st.columns(2)
                c4m.metric("Upward crosses", up_count)
                c5m.metric("Downward crosses", down_count)
    else:
        st.info("Click **Run S/R Reversal Cross Scan** to scan the selected symbols.")
