...
# UPDATED (this fix):
#   • Daily chart now also removes gaps so price is continuous (gapless daily prices)
#     - Applies the SAME “gapless” offset logic used for intraday, but tuned for DAILY session breaks
#     - Works even when you later .asfreq("D") + ffill, because the gapless adjustment is applied
#       on the raw trading-day series BEFORE resampling.
#
# Keeps ALL prior features as-is:
...
#   • Keeps shared hourly renderer + all overlays, scanners, etc.
...
# ---------- NEW: Asset-class mode buttons (Forex / Stocks) ----------
...
def make_gapless_ohlc(df: pd.DataFrame,
    ...
    Detection:
      - expected step = median index delta (seconds)
      - break if delta >= max(expected*gap_mult, min_gap_seconds)
    """
...
# NEW (this fix): tune gap detection for DAILY session breaks (weekends/holidays)
_DAILY_GAP_MULT = 1.5
_DAILY_MIN_GAP_SECONDS = 36 * 3600  # 36h catches 2–3 day calendar breaks reliably

def make_gapless_close_daily(raw_close: pd.Series) -> pd.Series:
    """
    Apply the same gapless-offset logic to a DAILY trading-day close series.
    Run this BEFORE .asfreq("D")+ffill so weekends/holidays still appear as time gaps.
    """
    s = _coerce_1d_series(raw_close).dropna()
    if s.empty:
        return s
    tmp = pd.DataFrame({"Open": s, "High": s, "Low": s, "Close": s})
    tmp2 = make_gapless_ohlc(tmp, gap_mult=_DAILY_GAP_MULT, min_gap_seconds=_DAILY_MIN_GAP_SECONDS)
    return _coerce_1d_series(tmp2["Close"]).reindex(s.index)

def _tz_to_pacific(idx: pd.DatetimeIndex) -> pd.DatetimeIndex:
...
if st.sidebar.button("🧹 Clear cache (data + run state)", use_container_width=True, key="btn_clear_cache"):
    try:
        ...
        pass
    _reset_run_state_for_mode_switch()
    # reset defaults requested (so they take effect immediately after rerun)
...
# UPDATED: Fibonacci ON by default
...
# UPDATED: Momentum OFF by default
...
def fetch_hist(ticker: str) -> pd.Series:
    """
    DAILY close used for charts/indicators.
    FIX: apply gapless adjustment on trading-day data first, then asfreq("D")+ffill.
    """
    ...
    raw = make_gapless_close_daily(raw)  # <-- FIX (daily gapless)

    s = raw.asfreq("D").ffill()
    s.index = _tz_to_pacific(s.index)
    return s

@st.cache_data(ttl=120)
def fetch_hist_max(ticker: str) -> pd.Series:
    """
    Max daily close history.
    FIX: apply daily gapless before resampling.
    """
...
def fetch_hist_ohlc(ticker: str) -> pd.DataFrame:
    """
    Daily OHLC (trading days).
    FIX: apply daily gapless to OHLC too, so overlays (Kijun/PSAR/etc.) match the gapless daily close.
    """
    ...
    df = make_gapless_ohlc(df, gap_mult=_DAILY_GAP_MULT, min_gap_seconds=_DAILY_MIN_GAP_SECONDS)  # <-- FIX

    df.index = _tz_to_pacific(df.index)
    return df
...
def fetch_intraday(ticker: str, period: str = "1d") -> pd.DataFrame:
    ...
    # remove price gaps so intraday price is continuous (gapless)
    if {'Open','High','Low','Close'}.issubset(df.columns):
        df = make_gapless_ohlc(df)  # intraday defaults
...
def compute_sarimax_forecast(series_like):
    ...
    if isinstance(series.index, pd.DatetimeIndex):
        series.index = _tz_to_pacific(series.index)
    try:
...
# =========================
# ---------- Ichimoku, Supertrend, PSAR, Sessions, News, Channel shading ----------
...
def render_hourly_views(sel: str,
    ...
    slope_sig_h = m_h if np.isfinite(m_h) else float("nan")

    rev_prob_h = slope_reversal_probability(
        hc, slope_sig_h, hist_window=rev_hist_lb, slope_window=slope_lb_hourly, horizon=rev_horizon
    )
...
with tab1:
    ...
    if st.button("Run Forecast", key="btn_run_forecast") or auto_run:
        df_hist = fetch_hist(sel)          # <-- daily gapless already applied here
        df_ohlc = fetch_hist_ohlc(sel)     # <-- daily gapless applied here too
    ...
    if st.session_state.run_all and st.session_state.ticker == sel:
        ...
        if chart in ("Daily","Both"):
            ...
            ax.set_title(f"{sel} Daily — {daily_view} — Gapless Price, 30 EMA, S/R, Slope, Pivots [P(slope rev≤{rev_horizon} bars)={rev_txt_d}]")
            ax.plot(df_show, label="History (Gapless)")
            ...
            if np.isfinite(res_val_d) and np.isfinite(sup_val_d):
                ax.hlines(res_val_d, xmin=df_show.index[0], xmax=df_show.index[-1], colors="tab:red", linewidth=1.6, label=f"Resistance (w={sr_lb_daily})")
                ax.hlines(sup_val_d, xmin=df_show.index[0], xmax=df_show.index[-1], colors="tab:green", linewidth=1.6, label=f"Support (w={sr_lb_daily})")
...
with tab2:
    ...
    if not st.session_state.run_all:
        ...
        if view in ("Daily","Both"):
            st.info("Daily chart uses the same (gapless) daily series as Tab 1.")
            # Reuse Tab 1 daily rendering by telling user to view there (keeps behavior stable)
            st.caption("Tip: Daily layout is identical to Tab 1; use Tab 1 for full overlays.")
            # Minimal duplicate daily plot:
            df_show = subset_by_daily_view(df, daily_view)
            fig, ax = plt.subplots(figsize=(14, 4))
            ax.set_title(f"{st.session_state.ticker} Daily (Gapless) — {daily_view}")
            ax.plot(df_show, label="History (Gapless)")
            draw_trend_direction_line(ax, df_show, label_prefix="Trend (global)")
            ax.legend(loc="lower left", framealpha=0.5)
            st.pyplot(fig)
...
with tab3:
    ...
    sym = st.selectbox("Symbol:", universe, key=f"bb_symbol_{mode}")
    s = fetch_hist(sym)
    if s is None or s.dropna().empty:
        ...
        if bb_period == "1mo":
            ...
            start = end - pd.Timedelta(days=93)
        elif bb_period == "6mo":
        ...
        bull = float((ret > 0).mean()) if len(ret) else np.nan
        bear = float((ret < 0).mean()) if len(ret) else np.nan

        fig, ax = plt.subplots(figsize=(14, 3.5))
        ax.set_title(f"{sym} — {bb_period} — Bull vs Bear (Gapless Daily)")
        ax.plot(w.index, w.values, label="Price")
        ax.legend(loc="lower left", framealpha=0.5)
        st.pyplot(fig)

        st.metric("Bull (Up days)", fmt_pct(bull, digits=0))
        st.metric("Bear (Down days)", fmt_pct(bear, digits=0))

# ==================== TAB 4: METRICS ====================
with tab4:
    st.header("Metrics")
    sym = st.session_state.ticker if st.session_state.get("ticker") else st.selectbox("Symbol:", universe, key=f"m_symbol_{mode}")
    s = fetch_hist(sym)
    if s is None or s.dropna().empty:
        st.warning("No data.")
    else:
        yhat, up, lo, m, r2 = regression_with_band(s, slope_lb_daily)
        last = _safe_last_float(s)
        st.write(pd.DataFrame([{
            "Symbol": sym,
            "Last": last,
            "Slope (daily)": m,
            "R²": r2,
            "NTD (daily)": _coerce_1d_series(compute_normalized_trend(s, ntd_window)).dropna().iloc[-1] if compute_normalized_trend(s, ntd_window).dropna().shape[0] else np.nan
        }]))
...
with tab5:
    ...
    thr = -0.75
    st.caption(f"Shows symbols with latest NTD below **{thr}** (daily).")
    rows = []
    for sym in universe:
        v, ts = last_daily_ntd_value(sym, ntd_window)
        if np.isfinite(v) and v < thr:
            rows.append({"Symbol": sym, "NTD": v, "Time": ts})
    out = pd.DataFrame(rows).sort_values("NTD") if rows else pd.DataFrame(columns=["Symbol","NTD","Time"])
    st.dataframe(out, use_container_width=True)

# ==================== TAB 6: LONG-TERM HISTORY ====================
with tab6:
    st.header("Long-Term History")
    sym = st.selectbox("Symbol:", universe, key=f"lt_symbol_{mode}")
    s = fetch_hist_max(sym)
    if s is None or s.dropna().empty:
        st.warning("No data.")
    else:
        yrs = st.slider("Years to display", 1, 25, min(10, int(st.session_state.get("hist_years", 10))), 1, key="lt_years")
        st.session_state.hist_years = yrs
        end = s.index.max()
        start = end - pd.Timedelta(days=int(365.25 * yrs))
        w = s.loc[(s.index >= start) & (s.index <= end)].dropna()

        fig, ax = plt.subplots(figsize=(14, 4))
        ax.set_title(f"{sym} — Last {yrs} years (Gapless Daily)")
        ax.plot(w.index, w.values, label="Price")
        draw_trend_direction_line(ax, w, label_prefix="Trend (global)")
        ax.legend(loc="lower left", framealpha=0.5)
        st.pyplot(fig)
