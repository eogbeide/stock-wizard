...
def _safe_current_yahoo_price(symbol: str, fallback: float = np.nan) -> float:
    ...
    label reflects the latest close rather than the potentially gap-adjusted intraday series.
    """
    try:
        tk = yf.Ticker(symbol)
...
# =========================
# Additional scanners used by remaining tabs
# =========================
@st.cache_data(ttl=120)
def green_cross_row_daily(symbol: str,
                          daily_view_label: str,
                          ntd_win: int = 60,
                          max_bars_since: int = 5):
    try:
...
@st.cache_data(ttl=120)
def green_cross_row_hourly(symbol: str,
                           period: str,
                           ntd_win: int = 60,
                           max_bars_since: int = 60):
    try:
...
def hma_signal_row_daily(symbol: str,
    ...
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        close = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        if len(close) < max(20, hma_period + 5):
            return None

        hma = _coerce_1d_series(compute_hma(close, period=hma_period))
        cross_up, cross_dn = _strict_cross_series(close, hma)
        cross_up = cross_up.reindex(close.index, fill_value=False)
        cross_dn = cross_dn.reindex(close.index, fill_value=False)
        ...
        bars_since = _bars_since_event(close.index, t)
        if bars_since > int(max_bars_since):
            return None
        ...
        return {
            ...
            "Bars Since Cross": int(bars_since),
            "Cross Time (PST)": t,
            "Price@Cross": float(close.loc[t]) if np.isfinite(close.loc[t]) else np.nan,
            "HMA@Cross": float(hma.loc[t]) if np.isfinite(hma.loc[t]) else np.nan,
            "Last Price": float(close.iloc[-1]) if np.isfinite(close.iloc[-1]) else np.nan,
        }
    except Exception:
        return None
...
def hma_signal_row_hourly(symbol: str,
    ...
    try:
        ...
        if df is None or df.empty or "Close" not in df.columns:
            return None
        close = _coerce_1d_series(df["Close"]).ffill().dropna()
        if len(close) < max(30, hma_period + 5):
            return None
...
@st.cache_data(ttl=120)
def price_reg_cross_row_daily(symbol: str,
                              daily_view_label: str,
                              slope_lb: int,
                              max_bars_since: int = 5):
    try:
        ...
        yhat, _, _, m, r2 = regression_with_band(close, lookback=min(len(close), int(slope_lb)))
        yhat = _coerce_1d_series(yhat).reindex(close.index)
        up_mask, dn_mask = _strict_cross_series(close, yhat)
        up_mask = up_mask.reindex(close.index, fill_value=False)
        dn_mask = dn_mask.reindex(close.index, fill_value=False)
        ...
        return {
            ...
            "Cross Time (PST)": t,
            "Price@Cross": float(close.loc[t]) if np.isfinite(close.loc[t]) else np.nan,
            "Regression@Cross": float(yhat.loc[t]) if np.isfinite(yhat.loc[t]) else np.nan,
...
@st.cache_data(ttl=120)
def price_reg_cross_row_hourly(symbol: str,
                               period: str,
                               slope_lb: int,
                               max_bars_since: int = 10):
    try:
...
@st.cache_data(ttl=120)
def bull_bear_row(symbol: str, bb_period_sel: str):
    try:
        period_map_bb = {"1mo": 21, "3mo": 63, "6mo": 126, "1y": 252}
        lb = int(period_map_bb.get(bb_period_sel, 126))
        close = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if len(close) < max(10, lb // 2):
            return None

        tail = close.iloc[-lb:]
        ret = float(tail.iloc[-1] / tail.iloc[0] - 1.0) if len(tail) >= 2 and tail.iloc[0] != 0 else np.nan
        g = _global_slope_1d(tail)
        return {
            "Symbol": symbol,
            "Lookback": bb_period_sel,
            "Return": ret,
            "Trendline Slope": float(g) if np.isfinite(g) else np.nan,
            "State": "Bull" if np.isfinite(ret) and ret >= 0 else "Bear",
            "Last Price": float(close.iloc[-1]) if np.isfinite(close.iloc[-1]) else np.nan,
        }
    except Exception:
        return None
...
def long_term_history_row(symbol: str):
    try:
        ...
        if len(close) < 30:
            return None
        hi = float(close.max()) if np.isfinite(close.max()) else np.nan
        lo = float(close.min()) if np.isfinite(close.min()) else np.nan
        last_px = float(close.iloc[-1]) if np.isfinite(close.iloc[-1]) else np.nan
        cagr = np.nan
        if len(close) >= 2 and close.iloc[0] > 0:
            years = max((close.index[-1] - close.index[0]).days / 365.25, 1e-9)
            cagr = float((close.iloc[-1] / close.iloc[0]) ** (1 / years) - 1.0)
        return {
            "Symbol": symbol,
            "Start": close.index[0],
            "End": close.index[-1],
...
def ntd_cross_row_daily(symbol: str,
    ...
    Requested new tab:
      ...
      Return rows for both:
        (a) Regression slope > 0
        (b) Regression slope < 0
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        close = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        ...
        regime = "Regression > 0" if float(rm) > 0.0 else "Regression < 0"
...
with tab8:
    ...
    st.caption("NPX recently crossed up through NTD while NTD is above 0.")
    ...
    if st.button("Scan Green Cross", key=f"btn_gc_{mode}", use_container_width=True):
        ...
        with st.spinner("Scanning..."):
            for symbol in universe:
                row = (
                    green_cross_row_daily(symbol, daily_view, ntd_win=ntd_window, max_bars_since=5)
                    if frame_choice == "Daily"
                    else green_cross_row_hourly(symbol, period_map[hour_choice], ntd_win=ntd_window, max_bars_since=60)
                )
                if row is not None:
                    rows.append(row)
...
with tab9:
    ...
    frame_choice = c1.selectbox("Frame", ["Daily", "Hourly"], key=f"hma_sig_frame_{mode}")
    hour_choice = c2.selectbox("Hourly lookback", ["24h", "48h", "96h"], key=f"hma_sig_hour_{mode}")

    if st.button("Scan HMA Signal", key=f"btn_hma_sig_{mode}", use_container_width=True):
        rows = []
        with st.spinner("Scanning..."):
            for symbol in universe:
                row = (
        ...
        if out.empty:
            ...
            out = out.sort_values(["Bars Since Cross", "Symbol"], ascending=[True, True])
            st.dataframe(out, use_container_width=True)
...
with tab10:
    ...
    c1, c2 = st.columns([1, 1])
    frame_choice = c1.selectbox("Frame", ["Daily", "Hourly"], key=f"new_hma_frame_{mode}")
    hour_choice = c2.selectbox("Hourly lookback", ["24h", "48h", "96h"], key=f"new_hma_hour_{mode}")

    if st.button("Scan New HMA Cross", key=f"btn_new_hma_{mode}", use_container_width=True):
        rows = []
        with st.spinner("Scanning..."):
            for symbol in universe:
                row = (
                    new_hma_cross_row_daily(symbol, daily_view, max_bars_since=3)
                    if frame_choice == "Daily"
...
with tab11:
    ...
    c1, c2, c3 = st.columns([1, 1, 1])
    frame_choice = c1.selectbox("Frame", ["Daily", "Hourly"], key=f"prc_frame_{mode}")
    hour_choice = c2.selectbox("Hourly lookback", ["24h", "48h", "96h"], key=f"prc_hour_{mode}")
    max_rows = int(c3.number_input("Max rows", min_value=5, max_value=500, value=100, step=5, key=f"prc_rows_{mode}"))

    if st.button("Scan Price↔Regression Cross", key=f"btn_prc_{mode}", use_container_width=True):
        rows = []
        with st.spinner("Scanning..."):
            for symbol in universe:
                row = (
                    price_reg_cross_row_daily(symbol, daily_view, slope_lb_daily, max_bars_since=5)
                    if frame_choice == "Daily"
                    else price_reg_cross_row_hourly(symbol, period_map[hour_choice], slope_lb_hourly, max_bars_since=10)
                )
                if row is not None:
                    rows.append(row)
        ...
        out = pd.DataFrame(rows)
        if out.empty:
            st.info("No Price↔Regression crosses found.")
        else:
            out = out.sort_values(["Bars Since Cross", "R2", "Symbol"], ascending=[True, False, True]).head(max_rows)
            st.dataframe(out, use_container_width=True)
...
with tab12:
    ...
    st.caption("Simple return and trendline ranking over the selected lookback.")

    if st.button("Scan Bull vs Bear", key=f"btn_bb_{mode}", use_container_width=True):
        rows = []
        with st.spinner("Scanning..."):
            for symbol in universe:
        ...
        out = pd.DataFrame(rows)
        if out.empty:
            st.info("No Bull/Bear data found.")
        else:
            bull_df = out[out["State"] == "Bull"].sort_values(["Return", "Trendline Slope"], ascending=[False, False])
            bear_df = out[out["State"] == "Bear"].sort_values(["Return", "Trendline Slope"], ascending=[True, True])

            st.subheader("Bull")
            st.dataframe(bull_df, use_container_width=True)
            st.subheader("Bear")
...
with tab13:
    ...
    st.caption("Summary stats using max available history.")
    ...
    if st.button("Scan Long-Term History", key=f"btn_lth_{mode}", use_container_width=True):
        ...
        out = pd.DataFrame(rows)
        if out.empty:
            st.info("No long-term history found.")
        else:
...
with tab14:
    ...
    st.caption("NPX recently crossed up through NTD and the regression slope is > 0.")

    c1, c2, c3 = st.columns([1, 1, 1])
    frame_choice = c1.selectbox("Frame", ["Daily", "Hourly"], key=f"ntd_buy_frame_{mode}")
    hour_choice = c2.selectbox("Hourly lookback", ["24h", "48h", "96h"], key=f"ntd_buy_hour_{mode}")
    max_rows = int(c3.number_input("Max rows", min_value=5, max_value=500, value=100, step=5, key=f"ntd_buy_rows_{mode}"))

    if st.button("Scan NTD Buy Signal", key=f"btn_ntd_buy_{mode}", use_container_width=True):
        rows = []
        with st.spinner("Scanning..."):
            for symbol in universe:
                row = (
                    ...
                    if frame_choice == "Daily"
                    else ntd_buy_signal_row_hourly(symbol, period_map[hour_choice], slope_lb_hourly, ntd_win=ntd_window, max_bars_since=60)
                )
                if row is not None:
                    rows.append(row)
...
with tab15:
    st.header("NTD Cross")
    st.caption("Symbols where NPX recently crossed up through the NTD line, split by regression slope > 0 and < 0.")

    c1, c2, c3 = st.columns([1, 1, 1])
    frame_choice = c1.selectbox("Frame", ["Daily", "Hourly"], key=f"ntd_cross_frame_{mode}")
    hour_choice = c2.selectbox("Hourly lookback", ["24h", "48h", "96h"], key=f"ntd_cross_hour_{mode}")
    max_rows = int(c3.number_input("Max rows", min_value=5, max_value=500, value=100, step=5, key=f"ntd_cross_rows_{mode}"))

    if st.button("Scan NTD Cross", key=f"btn_ntd_cross_{mode}", use_container_width=True):
        rows = []
        with st.spinner("Scanning..."):
            for symbol in universe:
                row = (
                    ...
                    if frame_choice == "Daily"
                    else ntd_cross_row_hourly(symbol, period_map[hour_choice], slope_lb_hourly, ntd_win=ntd_window, max_bars_since=60)
                )
                if row is not None:
                    rows.append(row)
        ...
        if out.empty:
            ...
            pos_df = out[out["Group"] == "Regression > 0"].sort_values(
                ["Bars Since Cross", "R2", "Symbol"], ascending=[True, False, True]
            ).head(max_rows)
            neg_df = out[out["Group"] == "Regression < 0"].sort_values(
                ["Bars Since Cross", "R2", "Symbol"], ascending=[True, False, True]
            ).head(max_rows)

            st.subheader("NPX↑NTD with Regression > 0")
            if pos_df.empty:
                st.info("No symbols found in this group.")
            else:
