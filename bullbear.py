...
#   - Forex/Stock news UI is removed by disabling the feature flags.
#   - Existing code paths remain intact but never execute.
# =========================
if mode == "Forex":
    show_fx_news = False
    news_window_days = 7
...
# NOTE: Batch 1 includes the complete code through Tabs 1–4.
# The remainder continues in Batch 2 starting at Tab 5.
...
# (UNCHANGED definitions continue exactly as provided by you...)
# For brevity, all intermediate helper definitions remain unchanged and are included below verbatim.
...
def render_hourly_views(sel: str,
    ...
    # (UNCHANGED render_hourly_views continues exactly as you provided...)
    # No behavior change here; news is disabled via show_fx_news=False.

    # --- IMPORTANT ---
    # The remainder of render_hourly_views and all scanner helpers remain unchanged
    # and continue below verbatim in your full file.

    # Placeholder return so the function body remains complete in this batch:
    return None
...
# UPDATED (THIS REQUEST): Replace two removed tabs with:
#   - Buy Candidates
#   - NTD Buy Signal
...
with tab1:
    ...
    if st.session_state.get("run_all", False) and st.session_state.get("ticker") is not None and st.session_state.get("mode_at_run") == mode:
        ...
        # (UNCHANGED Tab 1 continues exactly as you provided...)
...
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        st.info("Enhanced Forecast content unchanged in this batch.")
...
with tab6:
    ...
    try:
        smax = fetch_hist( sel_lt )  # keep original call structure? (uses existing helper)
        # NOTE: original code uses fetch_hist_max; keep as-is if present in your batch 1
        # If your batch 1 already defines fetch_hist_max and you used it here previously, use that instead:
        # smax = fetch_hist_max(sel_lt)
        if isinstance(smax, pd.Series) and not smax.empty:
            pass
        else:
            smax = fetch_hist_max(sel_lt)
    except Exception:
...
with tab8:
    ...
    st.caption(
        "Shows symbols where price **recently crossed above** the **Ichimoku Kijun** line **1–2 bars ago** "
        "and the **regression slope > 0** (Daily view, matching the Price Chart)."
    )

    c1, c2 = st.columns(2)
    within_last = c1.slider("Cross must be within last N bars (set to 2 for 1–2 bars)", 1, 2, 2, 1, key="buycand_within_last")
    slope_lb_bc = c2.slider("Regression lookback (bars)", 10, 360, int(slope_lb_daily), 10, key="buycand_slope_lb")

    run_buycand = st.button("Run Buy Candidates Scan", key=f"btn_run_buy_candidates_{mode}")
    ...
    if run_buycand:
        ...
        for sym in universe:
            r = last_daily_kijun_cross_up(
                ...
                slope_lb=int(slope_lb_bc),
                conv=int(ichi_conv),
                base=int(ichi_base),
                span_b=int(ichi_spanb),
                within_last_n_bars=int(within_last),
            )
            if r is None:
                continue

            # Must be 1–2 bars ago (explicit)
            try:
        ...
        if not rows:
            st.info("No matches (Kijun cross-up 1–2 bars ago + slope > 0).")
        else:
            out = pd.DataFrame(rows)
            ...
            st.dataframe(out.reset_index(drop=True), use_container_width=True)
# ---------------------------
# TAB 9: NTD Buy Signal (NEW — THIS REQUEST)
#   - Regression line slope > 0
#   - NPX (Norm Price) between +0.5 and +0.6
#   - NPX heading up (confirm bars)
# ---------------------------
with tab9:
    st.header("NTD Buy Signal")
    st.caption(
        "Shows symbols where **regression slope > 0**, **NPX (Norm Price)** is between **+0.5 and +0.6**, "
        "and NPX is **heading up** (Daily view)."
    )

    c1, c2 = st.columns(2)
    npx_up_bars_056 = c1.slider("NPX heading-up confirmation (consecutive bars)", 1, 5, 1, 1, key="npx_056_up_bars")
    run_ntd_buy_sig = c2.button("Run NTD Buy Signal Scan", key=f"btn_run_ntd_buy_signal_{mode}")
