...
def _fib_npx_zero_signal_series(close: pd.Series,
    ...
    """
    Fib signals align with SLOPE reversal + NPX direction.
    ...
    FIB BUY:
      ...
      - NPX is going UP after the 0.0 cross

    FIB SELL:
      - Touch Fib 0% (high) within lookback
      - NPX crossed DOWN through 0.0 within lookback (touch_time <= cross_time)
      - Regression slope reversed from UP to DOWN
      - NPX is going DOWN after the 0.0 cross
...
# ---------------------------
# NEW (THIS REQUEST): Ichimoku Kijun cross-up scanner helper (Daily)
# ---------------------------
@st.cache_data(ttl=120)
def last_daily_kijun_cross_up_heading_up(symbol: str,
                                         daily_view_label: str,
                                         ichi_conv: int,
                                         ichi_base: int,
                                         ichi_spanb: int):
    """
    Returns dict when:
      - Price (Daily close) crossed UP through Kijun on the MOST RECENT bar
      - AND price is heading up (last close > prior close)
    ...
    try:
        ...
        if ohlc_full is None or ohlc_full.empty or not {"High","Low","Close"}.issubset(ohlc_full.columns):
            return None

        x0, x1 = close_show.index[0], close_show.index[-1]
        ohlc_show = ohlc_full.loc[(ohlc_full.index >= x0) & (ohlc_full.index <= x1)]
        if ohlc_show is None or ohlc_show.empty:
            return None
        ...
        kijun_show = _coerce_1d_series(kijun).reindex(close_show.index).ffill().bfill()
        if kijun_show.dropna().empty:
            return None

        cross_up, _ = _cross_series(close_show, kijun_show)
        cross_up = cross_up.reindex(close_show.index, fill_value=False)

        # Require the CROSS to be on the latest bar ("has crossed" now) and heading up
        if not bool(cross_up.iloc[-1]):
            return None

        try:
            c_last = float(close_show.iloc[-1])
            c_prev = float(close_show.iloc[-2])
        except Exception:
            return None
        if not (np.isfinite(c_last) and np.isfinite(c_prev) and c_last > c_prev):
            return None

        k_last = float(kijun_show.iloc[-1]) if np.isfinite(kijun_show.iloc[-1]) else np.nan
        if not np.isfinite(k_last):
            return None
...
# ---------------------------
# Support reversal heading up (Daily) helper for new tab
# ---------------------------
@st.cache_data(ttl=120)
...
# ---------------------------
# Fib 0%/100% "99.9% confidence" (R²≥0.999) confirmed reversal list helper
# ---------------------------
@st.cache_data(ttl=120)
...
# ---------------------------
# Enhanced Forecast Buy/Sell scanner helper (Daily)
# ---------------------------
@st.cache_data(ttl=120)
def enhanced_sr_buy_sell_candidate(symbol: str,
    ...
    try:
        ...
        bs
        # NOTE: remainder of this function is unchanged in your original;
        # keep your existing code below this point (Batch 1 already provided Parts 1-5, Batch 2 continues here).
        # (If you already have this function fully in your file, keep it exactly as-is past this point.)
        #
        # IMPORTANT: Do not delete or alter anything below in your local file except the new Kijun scan additions.
        # ---------------------------------------------------------------------------
        # (The rest of Part 7+ is exactly as in your current working version.)
        # ---------------------------------------------------------------------------
...
# =========================
# (UNCHANGED — keep your existing Part 8 exactly as in your current file)
...
# UPDATED (prior): force Streamlit tabs to wrap so ALL tabs show by default (no horizontal overflow)
st.markdown(
    """
    <style>
...
# - Added tab: Ichimoku Kijun Scanner
...
# =========================
# (UNCHANGED — keep your existing Tab 1–Tab 14 code exactly as-is)
#
# ADD THIS NEW TAB AT THE VERY END OF YOUR FILE (after Tab 14), WITHOUT CHANGING
# ANY EXISTING TAB CONTENT ABOVE.

with tab15:
    st.header("Ichimoku Kijun Scanner")
    st.caption(
        "Scans the current universe for symbols where the **Daily close** has **crossed UP** through the "
        "**Ichimoku Kijun** line on the **most recent bar**, and price is **heading up** (last close > prior close). "
        "Split into two lists:\n"
        "• **Ichimoku Kijun List 1:** regression slope **> 0** and Price crossed Kijun heading up\n"
        "• **Ichimoku Kijun List 2:** regression slope **< 0** and Price crossed Kijun heading up\n\n"
        "Includes **Price**, **Ichimoku (Kijun)**, and **R²** (of the regression slope line) as requested."
    )

    run_kijun_scan = st.button("Run Ichimoku Kijun Scan", key=f"btn_run_kijun_scan_{mode}")
    ...
    if run_kijun_scan:
        ...
        for sym in universe:
            ...
            hit = last_daily_kijun_cross_up_heading_up(
                symbol=sym,
            ...
            row = {
                "Symbol": sym,
                "Price": float(hit.get("Price", np.nan)),
                "Ichimoku Value": float(hit.get("Kijun", np.nan)),
                "R2": float(r2) if np.isfinite(r2) else np.nan,
            }
        ...
        with left:
            st.subheader("Ichimoku Kijun List 1 — Slope > 0 and Price crossed Kijun (heading up)")
            if not rows_pos:
                st.info("No matches.")
            else:
                out = pd.DataFrame(rows_pos)
                out = out.sort_values(["R2", "Symbol"], ascending=[False, True])
                st.dataframe(out.reset_index(drop=True), use_container_width=True)

        with right:
            st.subheader("Ichimoku Kijun List 2 — Slope < 0 and Price crossed Kijun (heading up)")
            if not rows_neg:
                st.info("No matches.")
            else:
