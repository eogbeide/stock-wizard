# bullbear.py
# =========================
# Batch 1/3 — Beginning through Tabs 1–4 (UPDATED per request)
# =========================
...
def format_trade_instruction(trend_slope: float,
    ...
    SELL only when both agree (both DOWN). Otherwise show an alert message.
    """
    def _finite(x):
        try:
...
def compute_npx_zero_cross_masks(npx: pd.Series, level: float = 0.0):
...
def _fib_npx_zero_signal_series(close: pd.Series,
    ...
    cross_up0, cross_dn0 = compute_npx_zero_cross_masks(npx_lb, level=0.0)
...
def render_hourly_views(sel: str,
    ...
    if ax2w is not None:
        ...
        if show_ntd_channel:
            # channel overlays are handled in later batches (unchanged in your original)
            pass
...
#   - Added (placeholders for Batch 2/3): Buy Candidates, NBB -0.8..-1.0 Lists, Hourly NPX 0.0-0.2 (NTD)
# =========================
try:
    plt.rcParams.update({
        "legend.frameon": True,
...
# ---------------------------
# TAB 2: ENHANCED FORECAST (UPDATED: add regression line + ±2σ bands on Daily)
# ---------------------------
with tab2:
    ...
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        if view in ("Daily", "Both"):
            ...
            # NEW (per request): regression line + ±2σ lines on enhanced daily chart
            ...
            if not yhat_d.dropna().empty:
                ax.plot(yhat_d.index, yhat_d.values, "-", linewidth=2.0,
                        label=f"Regression ({slope_lb_daily} bars, {fmt_slope(m_d)}/bar)")
...
# ---------------------------
# TAB 8: BUY CANDIDATES (NEW — replaces removed NPX 0.5-Cross Scanner tab)
#   Criteria:
#     • Price crossed ABOVE Ichimoku Kijun within the last 1–2 bars
#     • Regression line slope > 0
# ---------------------------
with tab8:
    st.header("Buy Candidates")
    st.caption(
        "Shows symbols where **price crossed above Ichimoku Kijun** within the last **1–2 bars** "
        "and the **regression slope > 0** (Daily view range matches the Price Chart)."
    )

    c1, c2, c3 = st.columns(3)
    min_bars = c1.slider("Min bars since Kijun cross", 0, 10, 1, 1, key="buycand_min_bars")
    max_bars = c2.slider("Max bars since Kijun cross", 0, 10, 2, 1, key="buycand_max_bars")
    run_buycand = c3.button("Run Buy Candidates Scan", key=f"btn_run_buy_candidates_{mode}")

    if run_buycand:
        rows = []
        lo = int(min(min_bars, max_bars))
        hi = int(max(min_bars, max_bars))
        ...
        for sym in universe:
            ...
            # Must be within 1–2 bars (or user-selected bounds) AND slope > 0
            if (lo <= bars_since <= hi) and np.isfinite(m) and float(m) > 0.0:
                rows.append(r)

        if not rows:
            st.info("No buy candidates found for the selected bar window.")
        else:
            out = pd.DataFrame(rows)
            ...
            st.dataframe(out.reset_index(drop=True), use_container_width=True)
# ============================================================
# IMPORTANT (THIS REQUEST): Remove News everywhere (disable)
#   - Keeps the original UI controls intact
#   - Prevents news markers + tables from rendering anywhere
# ============================================================
show_fx_news = False


# ============================================================
# IMPORTANT (THIS REQUEST): UPDATED TABS BLOCK (REPLACE YOUR CURRENT ONE)
#   - Removes: "NPX 0.5-Cross Scanner", "Fib NPX 0.0 Signal Scanner"
#   - Adds: "Buy Candidates"
#   - Adds: "NBB -0.8 to -1.0 Lists"
#   - Keeps everything else in the same order/behavior
# ============================================================
...
# ============================================================
# TAB 2: ENHANCED FORECAST (REPLACE YOUR CURRENT TAB 2 BLOCK)
#   (3) Add regression line + ±2σ bands on DAILY enhanced chart
# ============================================================
with tab2:
    st.header("Enhanced Forecast")
    if not st.session_state.get("run_all", False) or st.session_state.get("ticker") is None or st.session_state.get("mode_at_run") != mode:
        ...
        if view in ("Daily", "Both"):
            ...
            # NEW (THIS REQUEST): regression + ±2σ on enhanced DAILY chart
            ...
            # NEW (THIS REQUEST): plot regression + ±2σ
            ...
            if not up_e.dropna().empty and not lo_e.dropna().empty:
                ax.plot(up_e.index, up_e.values, "--", linewidth=2.2, color="black", alpha=0.85, label="+2σ")
                ax.plot(lo_e.index, lo_e.values, "--", linewidth=2.2, color="black", alpha=0.85, label="-2σ")
            ...
            ax.text(
                0.50, 0.02, f"R² ({slope_lb_daily} bars): {fmt_r2(r2_e)}",
                transform=ax.transAxes, ha="center", va="bottom",
                fontsize=9, color="black",
                bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="grey", alpha=0.7)
            )
...
# ============================================================
# TAB 15: NBB -0.8 to -1.0 Lists (NEW — THIS REQUEST)
#   (5) Two categories:
...
with tab15:
    ...
    st.caption(
        "Scans the current universe and creates two lists:\n"
        "• **(a)** Regression slope **> 0** AND **NBB** between **-1.0** and **-0.8**\n"
        "• **(b)** Regression slope **< 0** AND **NBB** between **-1.0** and **-0.8**\n\n"
        "NBB is derived from Bollinger %B: **NBB = 2·%B − 1** (same as your chart logic)."
    )

    c1, c2, c3 = st.columns(3)
    nbb_lo = c1.slider("NBB lower bound", -1.0, 0.0, -1.0, 0.01, key="nbb_bound_lo")
    nbb_hi = c2.slider("NBB upper bound", -1.0, 0.0, -0.8, 0.01, key="nbb_bound_hi")
    run_nbb = c3.button("Run NBB Scan", key=f"btn_run_nbb_scan_{mode}")
    ...
    if run_nbb:
        ...
        for sym in universe:
            ...
            # regression sign (same as other scanners)
            m, r2, ts = daily_global_slope(sym, daily_view_label=daily_view)
            if not np.isfinite(m):
                continue

            # NBB from Bollinger Bands (same compute_bbands used in charts)
            _, _, _, pctb, nbb = compute_bbands(close_show, window=bb_win, mult=bb_mult, use_ema=bb_use_ema)
            nbb_last = float(_coerce_1d_series(nbb).dropna().iloc[-1]) if len(_coerce_1d_series(nbb).dropna()) else np.nan
            pct_last = float(_coerce_1d_series(pctb).dropna().iloc[-1]) if len(_coerce_1d_series(pctb).dropna()) else np.nan
            if not np.isfinite(nbb_last):
                continue

            if not (lo_b <= nbb_last <= hi_b):
                continue
            ...
            row = {
                ...
                "R2": float(r2) if np.isfinite(r2) else np.nan,
                "NBB": float(nbb_last),
                "%B": float(pct_last) if np.isfinite(pct_last) else np.nan,
                "AsOf": ts
            }
