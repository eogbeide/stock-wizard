...
#   • Scanner tab: symbols where NTD just crossed up through -0.75 and NTD slope > 0
...
def _coerce_1d_series(obj) -> pd.Series:
    ...
    if isinstance(obj, pd.Series):
        ...
        num_cols = [
            c for c in obj.columns if pd.api.types.is_numeric_dtype(obj[c])
        ]
        if not num_cols:
            return pd.Series(dtype=float)
        s = obj[num_cols[0]]
...
def format_trade_instruction(
    ...
    if uptrend:
        ...
        text = (
            f"▲ BUY @{fmt_price_val(leg_a_val)} → "
            f"▼ SELL @{fmt_price_val(leg_b_val)}"
        )
    else:
        leg_a_val, leg_b_val = exit_sell, entry_buy
        text = (
            f"▼ SELL @{fmt_price_val(leg_a_val)} → "
            f"▲ BUY @{fmt_price_val(leg_b_val)}"
        )
...
def label_on_left(
    ax, y_val: float, text: str, color: str = "black", fontsize: int = 9
):
...
bb_period = st.sidebar.selectbox(
    "Bull/Bear Lookback:",
    ["1mo", "3mo", "6mo", "1y"],
    index=2,
    key="sb_bb_period",
...
show_fibs = st.sidebar.checkbox(
    "Show Fibonacci (hourly only)", value=False, key="sb_show_fibs"
)

slope_lb_daily = st.sidebar.slider(
    "Daily slope lookback (bars)", 10, 360, 90, 10, key="sb_slope_lb_daily"
)
slope_lb_hourly = st.sidebar.slider(
    "Hourly slope lookback (bars)", 12, 480, 120, 6, key="sb_slope_lb_hourly"
)

st.sidebar.subheader("Hourly Support/Resistance Window")
sr_lb_hourly = st.sidebar.slider(
    "Hourly S/R lookback (bars)", 20, 240, 60, 5, key="sb_sr_lb_hourly"
)

st.sidebar.subheader("Hourly Momentum")
show_mom_hourly = st.sidebar.checkbox(
    "Show hourly momentum (ROC%)", value=True, key="sb_show_mom_hourly"
)
mom_lb_hourly = st.sidebar.slider(
    "Momentum lookback (bars)", 3, 120, 12, 1, key="sb_mom_lb_hourly"
)

st.sidebar.subheader("Hourly Indicator Panel")
show_nrsi = st.sidebar.checkbox(
    "Show Hourly NTD panel", value=True, key="sb_show_nrsi"
)
nrsi_period = st.sidebar.slider(
    "RSI period (unused)", 5, 60, 14, 1, key="sb_nrsi_period"
)
...
atr_period = st.sidebar.slider(
    "ATR period", 5, 50, 10, 1, key="sb_atr_period"
)
atr_mult = st.sidebar.slider(
    "ATR multiplier", 1.0, 5.0, 3.0, 0.5, key="sb_atr_mult"
)

st.sidebar.subheader("Parabolic SAR")
show_psar = st.sidebar.checkbox(
    "Show Parabolic SAR", value=True, key="sb_psar_show"
)
psar_step = st.sidebar.slider(
    "PSAR acceleration step", 0.01, 0.20, 0.02, 0.01, key="sb_psar_step"
)
psar_max = st.sidebar.slider(
    "PSAR max acceleration", 0.10, 1.00, 0.20, 0.10, key="sb_psar_max"
)

st.sidebar.subheader("Signal Logic")
signal_threshold = st.sidebar.slider(
    "S/R proximity signal threshold", 0.50, 0.99, 0.90, 0.01, key="sb_sig_thr"
)
sr_prox_pct = (
    st.sidebar.slider(
        "S/R proximity (%)", 0.05, 1.00, 0.25, 0.05, key="sb_sr_prox"
    )
    / 100.0
)

st.sidebar.subheader("NTD (Daily/Hourly)")
show_ntd = st.sidebar.checkbox(
    "Show NTD overlay", value=True, key="sb_show_ntd"
)
ntd_window = st.sidebar.slider(
    "NTD slope window", 10, 300, 60, 5, key="sb_ntd_win"
)
shade_ntd = st.sidebar.checkbox(
    "Shade NTD (green=up, red=down)", value=True, key="sb_ntd_shade"
)
show_npx_ntd = st.sidebar.checkbox(
    "Overlay normalized price (NPX) on NTD",
    value=True,
    key="sb_show_npx_ntd",
)
mark_npx_cross = st.sidebar.checkbox(
    "Mark NPX↔NTD crosses (dots)", value=True, key="sb_mark_npx_cross"
)

st.sidebar.subheader("Normalized Ichimoku (Kijun on price)")
show_ichi = st.sidebar.checkbox(
    "Show Ichimoku Kijun on price", value=True, key="sb_show_ichi"
)
ichi_conv = st.sidebar.slider(
    "Conversion (Tenkan)", 5, 20, 9, 1, key="sb_ichi_conv"
)
ichi_base = st.sidebar.slider(
    "Base (Kijun)", 20, 40, 26, 1, key="sb_ichi_base"
)
ichi_spanb = st.sidebar.slider(
    "Span B", 40, 80, 52, 1, key="sb_ichi_spanb"
)

st.sidebar.subheader("Bollinger Bands (Price Charts)")
show_bbands = st.sidebar.checkbox(
    "Show Bollinger Bands", value=True, key="sb_show_bbands"
)
bb_win = st.sidebar.slider(
    "BB window", 5, 120, 20, 1, key="sb_bb_win"
)
bb_mult = st.sidebar.slider(
    "BB multiplier (σ)", 1.0, 4.0, 2.0, 0.1, key="sb_bb_mult"
)
bb_use_ema = st.sidebar.checkbox(
    "Use EMA midline (vs SMA)", value=False, key="sb_bb_ema"
)

st.sidebar.subheader("Probabilistic HMA Crossover (Price Charts)")
show_hma = st.sidebar.checkbox(
    "Show HMA crossover signal", value=True, key="sb_hma_show"
)
hma_period = st.sidebar.slider(
    "HMA period", 5, 120, 55, 1, key="sb_hma_period"
)
hma_conf = st.sidebar.slider(
    "Crossover confidence", 0.50, 0.99, 0.95, 0.01, key="sb_hma_conf"
)

st.sidebar.subheader("HMA(55) Reversal on NTD")
show_hma_rev_ntd = st.sidebar.checkbox(
    "Mark HMA cross + slope reversal on NTD",
    value=True,
    key="sb_hma_rev_ntd",
)
hma_rev_lb = st.sidebar.slider(
    "HMA reversal slope lookback (bars)", 2, 10, 3, 1, key="sb_hma_rev_lb"
)

st.sidebar.subheader("Reversal Stars (on NTD panel)")
rev_bars_confirm = st.sidebar.slider(
    "Consecutive bars to confirm reversal", 1, 4, 2, 1, key="sb_rev_bars"
)

# Forex-only controls
if mode == "Forex":
    show_fx_news = st.sidebar.checkbox(
        "Show Forex news markers (intraday)",
        value=True,
        key="sb_show_fx_news",
    )
    news_window_days = st.sidebar.slider(
        "Forex news window (days)", 1, 14, 7, key="sb_news_window_days"
    )
    st.sidebar.subheader("Sessions (PST)")
    show_sessions_pst = st.sidebar.checkbox(
        "Show London/NY session times (PST)",
        value=True,
        key="sb_show_sessions_pst",
    )
else:
    show_fx_news = False
    news_window_days = 7
...
def fetch_hist(ticker: str) -> pd.Series:
    s = (
        yf.download(
            ticker, start="2018-01-01", end=pd.to_datetime("today")
        )["Close"]
        .asfreq("D")
        .fillna(method="ffill")
    )
...
def current_daily_pivots(ohlc: pd.DataFrame) -> dict:
    if (
        ohlc is None
        or ohlc.empty
        or not {"High", "Low", "Close"}.issubset(ohlc.columns)
    ):
        return {}
...
def regression_with_band(
    series_like, lookback: int = 0, z: float = 2.0
):
    """
...
def compute_npo(
    close: pd.Series, fast: int = 12, slow: int = 26, norm_win: int = 240
) -> pd.Series:
    ...
    ema_fast = s.ewm(span=int(fast), adjust=False).mean()
    ema_slow = (
        s.ewm(span=int(slow), adjust=False).mean().replace(0, np.nan)
    )
    ppo = (ema_fast - ema_slow) / ema_slow * 100.0
    minp = max(10, int(norm_win) // 10)
    mean = ppo.rolling(int(norm_win), min_periods=minp).mean()
    std = ppo.rolling(int(norm_win), min_periods=minp).std().replace(
        0, np.nan
    )
    z = (ppo - mean) / std
...
def compute_normalized_trend(close: pd.Series, window: int = 60) -> pd.Series:
    ...
    slope_roll = s.rolling(window, min_periods=minp).apply(
        _slope, raw=False
    )
...
def draw_trend_direction_line(
    ax, series_like: pd.Series, label_prefix: str = "Trend"
):
    s = _coerce_1d_series(series_like).dropna()
    if s.shape[0] < 2:
        return np.nan
...
def compute_supertrend(
    df: pd.DataFrame, atr_period: int = 10, atr_mult: float = 3.0
):
    if (
        df is None
        or df.empty
        ...
        return pd.DataFrame(
            index=idx, columns=["ST", "in_uptrend", "upperband", "lowerband"]
        )
...
def compute_parabolic_sar(
    ...
    if df.empty:
        ...
        return (
            ...
            pd.Series(index=idx, dtype=bool),
        )
    ...
    return (
        ...
        pd.Series(up, index=df.index, name="in_uptrend"),
    )


def compute_psar_from_ohlc(
    df: pd.DataFrame, step: float = 0.02, max_step: float = 0.2
) -> pd.DataFrame:
    if df is None or df.empty or not {"High", "Low"}.issubset(df.columns):
        idx = df.index if df is not None else pd.Index([])
        return pd.DataFrame(index=idx, columns=["PSAR", "in_uptrend"])
    ps, up = compute_parabolic_sar(
        df["High"], df["Low"], step=step, max_step=max_step
    )
...
def ichimoku_lines(
    ...
    if H.empty or L.empty or C.empty:
        idx = (
            C.index
            if not C.empty
            else (H.index if not H.empty else L.index)
        )
    ...
    span_b_raw = (
        H.rolling(span_b).max() + L.rolling(span_b).min()
    ) / 2.0
...
def compute_bbands(
    close: pd.Series, window: int = 20, mult: float = 2.0, use_ema: bool = False
):
    s = _coerce_1d_series(close).astype(float)
    idx = s.index
    ...
    minp = max(2, window // 2)
    mid = (
        s.ewm(span=window, adjust=False).mean()
        if use_ema
        else s.rolling(window, min_periods=minp).mean()
    )
...
def _cross_series(price: pd.Series, line: pd.Series):
    ...
    return (
        cross_up.reindex(p.index, fill_value=False),
        cross_dn.reindex(p.index, fill_value=False),
    )


def detect_hma_reversal_masks(
    price: pd.Series, hma: pd.Series, lookback: int = 3
):
...
def overlay_hma_reversal_on_ntd(
    ax,
    price: pd.Series,
    hma: pd.Series,
    lookback: int = 3,
    ...
    buy_rev, sell_rev = detect_hma_reversal_masks(
        price, hma, lookback=lookback
    )
...
def overlay_npx_on_ntd(
    ax, npx: pd.Series, ntd: pd.Series, mark_crosses: bool = True
):
...
def overlay_ntd_triangles_by_trend(
    ax,
    ntd: pd.Series,
    trend_slope: float,
    upper: float = 0.75,
    ...
    cross_out_hi = (s >= upper) & (s.shift(1) < upper)  # > +0.75
    cross_out_lo = (s <= lower) & (s.shift(1) > lower)  # < -0.75
...
def overlay_ntd_sr_reversal_stars(
    ax,
    price: pd.Series,
    sup: pd.Series,
    res: pd.Series,
    ...
    buy_cond = (
        (trend_slope > 0)
        and near_support
        and _n_consecutive_increasing(p, bars_confirm)
        and toward_res
    )
    sell_cond = (
        (trend_slope < 0)
        and near_resist
        and _n_consecutive_decreasing(p, bars_confirm)
        and toward_sup
    )
...
def session_markers_for_index(
    idx: pd.DatetimeIndex, session_tz, open_hr: int, close_hr: int
):
    opens, closes = [], []
    if (
    ...
    for d in rng:
        try:
            dt_open_local = session_tz.localize(
                datetime(d.year, d.month, d.day, open_hr, 0, 0), is_dst=None
            )
...
def draw_session_lines(ax, lines: dict):
    ...
    for t in lines.get("ldn_close", []):
        ax.axvline(
            t, linestyle="--", linewidth=1.0, color="tab:blue", alpha=0.35
        )
    for t in lines.get("ny_open", []):
        ax.axvline(
            t, linestyle="-", linewidth=1.0, color="tab:orange", alpha=0.35
        )
...
def channel_state_series(
    price: pd.Series, sup: pd.Series, res: pd.Series, eps: float = 0.0
) -> pd.Series:
    p = _coerce_1d_series(price)
...
@st.cache_data(ttl=120)
def last_hourly_ntd_value(
    symbol: str, ntd_win: int, period: str = "1d"
):
    try:
...
# --- NEW: NTD -0.75 up-cross helpers (Daily & Hourly) ---
@st.cache_data(ttl=120)
def last_daily_ntd_upcross(
    symbol: str,
    ntd_win: int,
    level: float = -0.75,
    slope_lb: int = 60,
    ...
    """
    Daily NTD up-cross detector.

    Returns:
      (trigger: bool, last_ntd: float, timestamp, slope: float)

    trigger = True only if:
      • latest NTD bar crossed up through `level` from below, AND
      • local NTD slope over `slope_lb` bars is > 0.
    """
    try:
        s = fetch_hist(symbol)
        ntd = compute_normalized_trend(s, window=ntd_win).dropna()
        if ntd.shape[0] < 3:
            return False, np.nan, None, np.nan

        last2 = ntd.iloc[-2:]
        prev_val = float(last2.iloc[0])
        last_val = float(last2.iloc[1])
        crossed_up = (prev_val < level) and (last_val >= level)

        lookback = min(max(int(slope_lb), 2), len(ntd))
        _, m = slope_line(ntd, lookback)
        rising = np.isfinite(m) and (m > 0)

        triggered = bool(crossed_up and rising)
        last_ts = ntd.index[-1]
        return (
            triggered,
            last_val,
...
def last_hourly_ntd_upcross(
    ...
    """
    Hourly NTD up-cross detector (used for Forex).
...
def _price_above_kijun_from_df(
    df: pd.DataFrame, base: int = 26
):
    if (
        df is None
        or df.empty
...
@st.cache_data(ttl=120)
def price_above_kijun_info_hourly(
    symbol: str, period: str = "1d", base: int = 26
):
    try:
        df = fetch_intraday(symbol, period=period)
        return _price_above_kijun_from_df(df, base=base)
...
with tab1:
    ...
    chart = st.radio(
        "Chart View:", ["Daily", "Hourly", "Both"], key="orig_chart"
    )
    ...
    if st.session_state.run_all and st.session_state.ticker == sel:
        ...
        p_up = (
            np.mean(st.session_state.fc_vals.to_numpy() > last_price)
        ...
        if chart in ("Daily", "Both"):
            ...
            ntd_d = (
                compute_normalized_trend(df, window=ntd_window)
                if show_ntd
                else pd.Series(index=df.index, dtype=float)
            )
            npx_d_full = (
                compute_normalized_price(df, window=ntd_window)
                if show_npx_ntd
                else pd.Series(index=df.index, dtype=float)
            )
            ...
            yhat_d_show = (
                yhat_d.reindex(df_show.index) if not yhat_d.empty else yhat_d
            )
            upper_d_show = (
                upper_d.reindex(df_show.index)
                if not upper_d.empty
                else upper_d
            )
            lower_d_show = (
                lower_d.reindex(df_show.index)
                if not lower_d.empty
                else lower_d
            )
            yhat_ema_show = (
                yhat_ema30.reindex(df_show.index)
            ...
            if (
                show_bbands
                and not bb_up_d_show.dropna().empty
            ...
            if piv and len(df_show) > 0:
                ...
                for lbl, y in piv.items():
                    ax.hlines(
                        y, xmin=x0, xmax=x1, linestyles="dashed", linewidth=1.0
                    )
                for lbl, y in piv.items():
                    ax.text(
                        x1,
                        y,
                        f" {lbl} = {fmt_price_val(y)}",
                        va="center",
                    )
            ...
            if show_ntd and not ntd_d_show.dropna().empty:
                ...
                # Stars for S/R reversals (unchanged)
            ...
            if (
                show_npx_ntd
                and not npx_d_show.dropna().empty
            ...
            if (
                show_hma_rev_ntd
                and not hma_d_show.dropna().empty
            ...
            axdw.axhline(
                0.0, linestyle="--", linewidth=1.0, color="black", label="0.00"
            )
            axdw.axhline(
                0.75, linestyle="-", linewidth=1.0, color="black", label="+0.75"
            )
            axdw.axhline(
                -0.75, linestyle="-", linewidth=1.0, color="black", label="-0.75"
            )
        ...
        if chart in ("Hourly", "Both"):
            ...
            if intraday is None or intraday.empty or "Close" not in intraday:
                ...
                st_line_intr = (
                    st_intraday["ST"].reindex(hc.index)
                ...
                if (
                    {"High", "Low", "Close"}.issubset(intraday.columns)
                    and show_ichi
                ):
                ...
                ax2.plot(he.index, he, "--", label="20 EMA")
                ax2.plot(
                    hc.index,
                    trend_h,
                    "--",
                    label=f"Trend (m={fmt_slope(slope_h)}/bar)",
                    linewidth=2,
                )
                ...
                if np.isfinite(px_val):
                    ...
                    try:
                        last_pct = (
                            float(bb_pctb_h.dropna().iloc[-1])
                            if show_bbands
                            else np.nan
                        )
                        last_nbb = (
                            float(bb_nbb_h.dropna().iloc[-1])
                            if show_bbands
                            else np.nan
                        )
                ...
                vol = (
                    _coerce_1d_series(
                        intraday.get(
                            "Volume", pd.Series(index=hc.index)
                        )
                    )
                    .reindex(hc.index)
                    .astype(float)
                ...
                if _has_volume_to_plot(vol):
                    ...
                    ax2v.plot(
                        vol.index, vol, linewidth=1.0, color="tab:blue"
                    )
                    ax2v.plot(
                        v_mid.index,
                        v_mid,
                ...
                if show_nrsi:
                    ...
                    ntd_trend_h, ntd_m_h = slope_line(
                        ntd_h, slope_lb_hourly
                    )
                    npx_h = (
                        compute_normalized_price(hc, window=ntd_window)
                        if show_npx_ntd
                        else pd.Series(index=hc.index, dtype=float)
                    )
                    ...
                    ax2r.set_title(
                        "Hourly Indicator Panel — NTD + NPX + Trend "
                        f"(win={ntd_window})"
                    )
                    ...
                    if (
                        show_ntd_channel
                        and np.isfinite(res_val)
                        and np.isfinite(sup_val)
                    ):
...
with tab2:
    ...
    if not st.session_state.run_all:
        ...
        last_price = _safe_last_float(df)
        p_up = (
            np.mean(vals.to_numpy() > last_price)
            if np.isfinite(last_price)
            else np.nan
        )
        p_dn = 1 - p_up if np.isfinite(p_up) else np.nan
        ...
        view = st.radio(
            "View:", ["Daily", "Intraday", "Both"], key="enh_view"
        )
        ...
        if view in ("Daily", "Both"):
            ...
            # NEW: regression-based daily trend with band + R²
            yhat_d, upper_d, lower_d, m_d, r2_d = regression_with_band(
                df, slope_lb_daily
            )
            ...
            if (
                show_bbands
                and not bb_up_d2_show.dropna().empty
                ...
                ax.plot(
                    bb_mid_d2_show.index,
                    bb_mid_d2_show.values,
                    "-",
                    linewidth=1.1,
                    label=f"BB mid ({'EMA' if bb_use_ema else 'SMA'}, w={bb_win})",
                )
                ax.plot(
                    bb_up_d2_show.index,
                    bb_up_d2_show.values,
                    ":",
                    linewidth=1.0,
                )
                ax.plot(
                    bb_lo_d2_show.index,
                    bb_lo_d2_show.values,
                    ":",
                    linewidth=1.0,
                )
            ...
            if (
                show_npx_ntd
                and not npx_d2_show.dropna().empty
                and not ntd_d_show.dropna().empty
            ):
                overlay_npx_on_ntd(
                    axdw2,
            ...
            if (
                show_hma_rev_ntd
                and not hma_d2_show.dropna().empty
                and not df_show.dropna().empty
            ):
                overlay_hma_reversal_on_ntd(
                    axdw2,
        ...
        if view in ("Intraday", "Both"):
            ...
            if intr is None or intr.empty or "Close" not in intr:
                ...
                st.info(
                    "Intraday view is rendered in Tab 1. (Logic unchanged here.)"
                )
...
# --- Tab 5: NTD -0.75 Up-Cross Scanner ---
with tab5:
    st.header("NTD -0.75 Up-Cross Scanner")
    st.caption(
        "Shows symbols where the NTD line has just crossed **-0.75** upward "
        "and the NTD slope is > 0. Daily for all symbols; Hourly for Forex only. "
        "Ichimoku Kijun scanners are kept unchanged."
    )
    ...
    with c2:
        run = st.button("Scan Universe", key="btn_ntd_cross_scan")

    if run:
        # -------- Daily NTD up-cross scan --------
        daily_rows = []
        for sym in universe:
            trigger, last_ntd, ts, slope_val = last_daily_ntd_upcross(
                sym,
                ntd_window,
                level=cross_level,
        ...
        hits_daily = df_daily[df_daily["Trigger"] == True].sort_values(
            ["Timestamp", "NTD_Last"], ascending=[False, False]
        )

        c3, c4 = st.columns(2)
        c3.metric("Universe Size", len(universe))
        c4.metric(
            f"Daily NTD up-cross @ {cross_level:+.2f}",
            int(hits_daily.shape[0]),
        )

        st.subheader(
            f"Daily — NTD crossed up through {cross_level:+.2f} "
            "and NTD slope > 0"
        )
        if hits_daily.empty:
            st.info(
                f"No symbols where the latest Daily NTD bar crossed up through "
                f"{cross_level:+.2f} with a positive NTD slope."
            )
        else:
            ...
            view["Slope"] = view["Slope"].map(
                lambda x: f"{x:+.4f}" if np.isfinite(x) else "n/a"
            )
            st.dataframe(
                view[
                    ["Symbol", "Timestamp", "NTD_Last", "Slope"]
                ].reset_index(drop=True),
                use_container_width=True,
            )

        # -------- Daily Price > Kijun scanner (unchanged) --------
        st.markdown("---")
        st.subheader(
        ...
        if df_above_daily.empty:
            ...
            st.dataframe(
                view_above[
                    ["Symbol", "Timestamp", "Close", "Kijun"]
                ].reset_index(drop=True),
                use_container_width=True,
            )

        # -------- Hourly scans for Forex only --------
        if mode == "Forex":
            st.markdown("---")
            st.subheader(
                f"Forex Hourly — NTD up-cross @ {cross_level:+.2f} "
            ...
            hits_hour = df_hour[df_hour["Trigger"] == True].sort_values(
                ["Timestamp", "NTD_Last"], ascending=[False, False]
            )

            if hits_hour.empty:
                st.info(
                    f"No Forex pairs where the latest Hourly NTD bar crossed up "
                    f"through {cross_level:+.2f} with a positive NTD slope."
                )
            else:
            ...
            if df_above_hour.empty:
                st.info(
                    "No Forex pairs with Price > Kijun on the latest Hourly bar."
                )
            else:
...
with tab6:
    ...
    st.caption(
        "Showing last **{years} years**. Support/Resistance = rolling "
        "**252-day** extremes; trendline fits the shown window."
    )
    ...
    if s_full is None or s_full.empty:
        ...
        if s.empty:
            ...
            res_last = (
                float(res_roll.iloc[-1]) if len(res_roll) else np.nan
            )
            sup_last = (
                float(sup_roll.iloc[-1]) if len(sup_roll) else np.nan
            )
            ...
            if np.isfinite(res_last) and np.isfinite(sup_last):
                ...
                label_on_left(
                    ax, res_last, f"R {fmt_price_val(res_last)}", "tab:red"
                )
                label_on_left(
                    ax, sup_last, f"S {fmt_price_val(sup_last)}", "tab:green"
                )
