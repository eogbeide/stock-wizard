...
def _flatten_yf_columns(df: pd.DataFrame, ticker: str = None) -> pd.DataFrame:
    """
    Normalize yfinance output so the app always sees flat OHLCV columns.
    ...
    if isinstance(out.columns, pd.MultiIndex):
        ...
        # If the selected ticker is present on any column level, slice it out first.
        ...
        # If it is still MultiIndex, keep the level that contains OHLCV names.
        if isinstance(out.columns, pd.MultiIndex):
            for level in range(out.columns.nlevels):
                vals = set(map(str, out.columns.get_level_values(level)))
                if any(col in vals for col in wanted):
                    try:
...
def _ensure_datetime_pacific_index(obj):
    """Return a copy with a clean DatetimeIndex converted/localized to US/Pacific."""
    if obj is None:
        return obj
    try:
        if len(obj) == 0:
...
def _close_series_from_yf(df: pd.DataFrame, ticker: str = None) -> pd.Series:
    ...
    s = _ensure_datetime_pacific_index(s)
    return s.sort_index()
...
def _ohlc_from_yf(df: pd.DataFrame, ticker: str = None) -> pd.DataFrame:
    ...
    out = _ensure_datetime_pacific_index(out)
