...
# --- Helpers to ensure we always pass a string symbol to yf.download() ---
def _symbol(t):
    return t.ticker if isinstance(t, YFTicker) else t

# --- Data fetching & caching ---
@st.cache_data(ttl=900)
def fetch_hist(ticker):
    sym = _symbol(ticker)
    df  = yf.download(sym, start="2018-01-01", end=pd.to_datetime("today"))['Close']
...
with tab2:
    ...
    if not st.session_state.run_all:
        ...
        if view in ("Intraday","Both"):
            ...
            xi,y  = np.arange(len(ic)), ic.values.flatten()
            slope_i, intercept_i = np.polyfit(xi,y,1)
...
with tab3:
    ...
    if not st.session_state.run_all:
        ...
        df3 = yf.download(_symbol(st.session_state.ticker), period=bb_period)[['Close']].dropna()
...
with tab4:
    ...
    if not st.session_state.run_all:
        ...
        st.markdown("---")
        df0 = yf.download(_symbol(st.session_state.ticker), period=bb_period)[['Close']].dropna()
