...
def hma_cross_up_row_daily(symbol: str,
    ...
    try:
        ...
        px = close_show[ok]
        hh = _coerce_1d_series(hma).reindex(px.index)

        cross_up, _ = _cross_series(px, hh)
        cross_up = cross_up.reindex(px.index, fill_value=False)
        if not cross_up.any():
            return None

        t_cross = cross_up[cross_up].index[-1]
        bars_since = int((len(px) - 1) - int(px.index.get_loc(t_cross)))
        if int(bars_since) > int(max_bars_since):
            return None
        ...
        last_px = float(px.iloc[-1]) if np.isfinite(px.iloc[-1]) else np.nan
        last_hma = float(hh.iloc[-1]) if np.isfinite(hh.iloc[-1]) else np.nan
...
# =========================
# NEW: Price↔Regression Cross helper (Daily)
# =========================
@st.cache_data(ttl=120)
def price_reg_cross_row_daily(symbol: str,
                              ...
                              max_bars_since: int = 5):
    """
    Daily Price↔Regression Cross:
      - BUY if price crossed UP through regression line recently
      - SELL if price crossed DOWN through regression line recently
    Returns row with slopes for downstream filtering.
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        ...
        tm = _global_slope_1d(close_show)
        reg, _, _, rm, r2 = regression_with_band(close_show, lookback=min(len(close_show), int(slope_lb)))
        reg = _coerce_1d_series(reg).reindex(close_show.index)
        ok = close_show.notna() & reg.notna()
        if ok.sum() < 3:
            return None
        ...
        cross_up, cross_dn = _cross_series(px, rr)
        cross_up = cross_up.reindex(px.index, fill_value=False)
        cross_dn = cross_dn.reindex(px.index, fill_value=False)
        ...
        return {
            ...
            "Regression Slope": float(rm) if np.isfinite(rm) else np.nan,
            "R2": float(r2) if np.isfinite(r2) else np.nan,
            "Last Price": float(px.iloc[-1]) if np.isfinite(px.iloc[-1]) else np.nan,
            "Regression Line (last)": float(rr.iloc[-1]) if np.isfinite(rr.iloc[-1]) else np.nan,
        }
    except Exception:
        return None

# =========================
# NEW: NTD Buy Signal helpers (Daily + Hourly)
# =========================
...
def ntd_minus05_cross_row_daily(symbol: str,
    ...
    try:
        ...
        t = mask[mask].index[-1]
        bars_since = int((len(ntd_show) - 1) - int(ntd_show.index.get_loc(t)))
        if int(bars_since) > int(max_bars_since):
            return None
        ...
        ntd_last = float(ntd_show.iloc[-1]) if np.isfinite(ntd_show.iloc[-1]) else np.nan
        last_px = float(close_show.iloc[-1]) if np.isfinite(close_show.iloc[-1]) else np.nan
...
def ntd_minus05_cross_row_hourly(symbol: str,
    ...
    try:
        ...
        ntd = _coerce_1d_series(compute_normalized_trend(hc, window=int(ntd_win))).dropna()
        if len(ntd) < 2:
            return None
...
# =========================
def _npx_minus075_cross_up_mask(npx: pd.Series) -> pd.Series:
    """
    Cross up through -0.75, upward.
    This represents the NPX line crossing the green-circle level upward.
    """
    s = _coerce_1d_series(npx)
    prev = s.shift(1)
    return ((s >= -0.75) & (prev < -0.75) & (s > prev)).fillna(False)
...
def star_buy_alert_row_daily(symbol: str,
    ...
    Star Buy Alert (Daily):
      ...
      - NPX recently crossed UP through -0.75
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        ...
        mask = _npx_minus075_cross_up_mask(npx_show)
        if not mask.any():
            return None
...
def star_buy_alert_row_hourly(symbol: str,
    ...
    try:
        ...
        mask = _npx_minus075_cross_up_mask(npx)
        if not mask.any():
            return None
...
with tab7:
    ...
    st.caption(
        "Shows symbols where:\n"
        "• **BUY:** Trendline slope > 0 AND Regression slope > 0, and **Price crossed UP through the Regression line** recently.\n"
        "• **SELL:** Trendline slope < 0 AND Regression slope < 0, and **Price crossed DOWN through the Regression line** recently.\n\n"
        "Includes **Daily** results."
    )

    c1, c2, c3 = st.columns(3)
    max_rows = c1.slider("Max rows per list", 10, 300, 60, 10, key=f"prc_rows_{mode}")
    within_bars = c2.slider("Daily: Max bars since Price↔Regression cross", 0, 60, 5, 1, key=f"prc_within_d_{mode}")
    ...
    if run_prc:
        ...
        for sym in universe:
            ...
            if side == "BUY" and (tm > 0.0) and (rm > 0.0):
                buy_rows.append(r)
            elif side == "SELL" and (tm < 0.0) and (rm < 0.0):
                sell_rows.append(r)
        ...
        show_cols = [
            ...
            "Bars Since Cross", "Cross Time",
            "Trendline Slope", "Regression Slope", "R2",
            "Regression Line (last)", "Last Price"
        ]

        def _fmt_prc_time(df: pd.DataFrame, col: str = "Cross Time") -> pd.DataFrame:
            if df is None or df.empty or col not in df.columns:
                return df
            try:
                df["_ct"] = pd.to_datetime(df[col], errors="coerce")
        ...
        with cL:
            st.subheader("BUY — Price crossed UP through Regression")
            if not buy_rows:
                st.write("No matches.")
            else:
                df = pd.DataFrame(buy_rows)
                df = _fmt_prc_time(df, "Cross Time")
                df["_score"] = df["Trendline Slope"].astype(float) + df["Regression Slope"].astype(float)
                df = df.sort_values(["Bars Since Cross", "_score", "R2"], ascending=[True, False, False])
                st.dataframe(df[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)

        with cR:
            st.subheader("SELL — Price crossed DOWN through Regression")
            if not sell_rows:
                st.write("No matches.")
            else:
                df = pd.DataFrame(sell_rows)
                df = _fmt_prc_time(df, "Cross Time")
                df["_score"] = df["Trendline Slope"].astype(float) + df["Regression Slope"].astype(float)
                df = df.sort_values(["Bars Since Cross", "_score", "R2"], ascending=[True, True, False])
                st.dataframe(df[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)
...
with tab11:
    ...
    st.caption(
        "Shows symbols where:\n"
        "• **Global trend** is **UP**\n"
        "• **NPX (Norm Price)** has recently crossed **UP through -0.75**\n"
        "• This represents the NPX line crossing the **green-circle level** upward on the Hourly Indicator Panel — NTD/NPX Chart.\n\n"
        "Displays **Daily** and **Hourly (5m)** results."
    )

    c1, c2, c3, c4 = st.columns(4)
    max_rows = c1.slider("Max rows per list", 10, 300, 60, 10, key=f"star_rows_{mode}")
    within_daily = c2.slider("Daily: Max bars since NPX crossed -0.75", 0, 60, 5, 1, key=f"star_within_d_{mode}")
    hours = c3.selectbox("Hourly scan window", ["24h", "48h", "96h"], index=0, key=f"star_hr_win_{mode}")
    within_hourly = c4.slider("Hourly: Max bars since NPX crossed -0.75 (5m bars)", 0, 480, 60, 5, key=f"star_within_h_{mode}")
    ...
    if run_star:
        ...
        def _fmt_star_time(df: pd.DataFrame, col: str = "Cross Time (PST)") -> pd.DataFrame:
            if df is None or df.empty or col not in df.columns:
                return df
            try:
                df["_ct"] = pd.to_datetime(df[col], errors="coerce")
        ...
        with cL:
            ...
            if not d_rows:
                ...
                df = pd.DataFrame(d_rows)
                df = _fmt_star_time(df, "Cross Time (PST)")
                df["_score"] = df["Trendline Slope"].astype(float)
                df = df.sort_values(["Bars Since Cross", "_score", "NPX(last)"], ascending=[True, False, True])
                st.dataframe(df[show_cols].head(max_rows).reset_index(drop=True), use_container_width=True)
