...
# =========================
# Part 8/10 — Session state + sidebar toggle (unique key)
# =========================
...
DEFAULT_STOCKS = ["AAPL","SPY","AMZN","DIA","TSLA","SPGI","JPM","VTWG","PLTR","NVDA",
        "META","SITM","MARA","GOOG","HOOD","BABA","IBM","AVGO","GUSH","VOO",
        "MSFT","TSM","NFLX","MP","AAL","URI","DAL","BBAI","QUBT","AMD","SMCI",
        "ORCL","TLT"]
DEFAULT_FX = ["EURUSD=X","EURJPY=X","GBPUSD=X","USDJPY=X","AUDUSD=X","NZDUSD=X","CADJPY=X","USDCHF=X",
...
def render_hourly_views(sel: str,
    ...
    # -------- FIX (minimal): normalize MultiIndex columns + force Close to 1D Series --------
    # This prevents res_h.iloc[-1] from being a Series (ambiguous truth value) and keeps UI unchanged.
    if isinstance(intraday.columns, pd.MultiIndex):
        intraday = intraday.copy()
        intraday.columns = [c[0] for c in intraday.columns]
        intraday = intraday.loc[:, ~pd.Index(intraday.columns).duplicated(keep="first")]
    # --------------------------------------------------------------------------------------
    ...
    # -------- FIX (minimal): make sure hc is a Series (not a DataFrame) --------
    hc = _coerce_1d_series(intraday["Close"]).ffill()
    # --------------------------------------------------------------------------
