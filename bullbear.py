...
# ---------------------------
# NEW (THIS REQUEST): regression upper-band reversal DOWN while slope>0 (for Regression Trend tab)
# ---------------------------
def find_upper_band_reversal_down(price: pd.Series,
                                  upper_band: pd.Series,
                                  lower_band: pd.Series,
                                  slope_val: float):
    """
    For slope > 0:
      - previous bar was ABOVE upper 2σ band
      - current bar is back INSIDE the band
      - current close is DOWN vs previous close
    Returns dict or None.
    """
    p = _coerce_1d_series(price)
    u = _coerce_1d_series(upper_band).reindex(p.index)
    ...
    inside_now = (p <= u) & (p >= l)
    prev_above = p.shift(1) > u.shift(1)
    down_now = p < p.shift(1)

    candidates = inside_now & prev_above & down_now
    if not candidates.any():
        return None
    t = candidates[candidates].index[-1]
    return {"time": t, "price": float(p.loc[t]), "side": "DOWN"}
...
# ---------------------------
# NEW (THIS REQUEST): Regression Trend bounce helpers (daily view) for the new "Regression Trend" tab
#   - (a) slope > 0 AND last bar reversed from LOWER -2σ band (inside after below) and is going UP
#   - (b) slope > 0 AND last bar reversed from UPPER +2σ band (inside after above) and is going DOWN
#   NOTE: UI decides whether to display (b). Logic provided for both.
# ---------------------------
def _reg_band_reversal_lastbar(price: pd.Series, upper: pd.Series, lower: pd.Series):
    p = _coerce_1d_series(price)
    ...
    # A: was below lower, now inside, and last close is up
    a = bool(inside.iloc[-1] and (prev_p.iloc[-1] < prev_l.iloc[-1]) and (p.iloc[-1] > prev_p.iloc[-1]))

    # B: was above upper, now inside, and last close is down
    b = bool(inside.iloc[-1] and (prev_p.iloc[-1] > prev_u.iloc[-1]) and (p.iloc[-1] < prev_p.iloc[-1]))

    return a, b

@st.cache_data(ttl=120)
def last_daily_regression_trend_reversal(symbol: str, daily_view_label: str, slope_lb: int):
    """
    Returns dict with keys:
      - "A": row dict for (a) condition, else None
      - "B": row dict for (b) condition, else None
    """
    try:
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if close_full.empty:
        ...
        if not (np.isfinite(m) and np.isfinite(r2)) or float(m) <= 0.0:
            return {"A": None, "B": None}

        a_ok, b_ok = _reg_band_reversal_lastbar(close_show, up, lo)
        t = close_show.index[-1]
        ...
        row_base = {
            ...
            "Slope": float(m),
            "R2": float(r2),
            "Upper(+2σ)": float(_safe_last_float(_coerce_1d_series(up).reindex(close_show.index))),
            "Lower(-2σ)": float(_safe_last_float(_coerce_1d_series(lo).reindex(close_show.index))),
        }
        ...
        return {
            "A": row_a if a_ok else None,
            "B": row_b if b_ok else None
        }
    except Exception:
        return {"A": None, "B": None}
...
# ---------------------------
def _apply_ax_common(ax):
    ax.grid(True, alpha=0.25)
    for spine in ax.spines.values():
        spine.set_alpha(0.25)

def _nice_legend(ax, max_items: int = 14):
    h, l = ax.get_legend_handles_labels()
    if not h:
        return
    # de-dup by label (keep first)
    seen = set()
    hh, ll = [], []
    for hi, li in zip(h, l):
        if li in seen or li.strip() == "":
            continue
        seen.add(li)
        hh.append(hi); ll.append(li)
        if len(hh) >= max_items:
            break
    ax.legend(hh, ll, loc="best", fontsize=9, framealpha=0.85)

def _plot_regression_bands_on_price(ax, price: pd.Series, slope_lb: int, band_sigma: float = 2.0):
    p = _coerce_1d_series(price).dropna()
    if p.empty or len(p) < 3:
        return None
    yhat, up, lo, m, r2 = regression_with_band(p, lookback=int(slope_lb), band_sigma=float(band_sigma))
    ax.plot(p.index, yhat.values, "-", linewidth=1.6, alpha=0.95, label=f"Reg({slope_lb})")
    ax.plot(p.index, up.values, "--", linewidth=1.2, alpha=0.9, label=f"+{band_sigma}σ")
    ax.plot(p.index, lo.values, "--", linewidth=1.2, alpha=0.9, label=f"-{band_sigma}σ")
    return {"yhat": yhat, "up": up, "lo": lo, "slope": m, "r2": r2}

def _plot_fibs(ax, price: pd.Series):
    p = _coerce_1d_series(price).dropna()
    fibs = fibonacci_levels(p)
    if not fibs:
        return
    # only the main ones for clarity
    keep = ["0%", "23.6%", "38.2%", "50%", "61.8%", "78.6%", "100%"]
    for k in keep:
        if k in fibs and np.isfinite(fibs[k]):
            ax.axhline(float(fibs[k]), linewidth=0.9, alpha=0.22)
    ax.plot([], [], linewidth=2, alpha=0.22, label="Fibs")

def _plot_sr(ax, price: pd.Series, sr_lb: int):
    p = _coerce_1d_series(price).dropna()
    if p.empty:
        return None
    sup = p.rolling(int(sr_lb), min_periods=1).min()
    res = p.rolling(int(sr_lb), min_periods=1).max()
    ax.plot(p.index, sup.values, "-", linewidth=1.0, alpha=0.55, label=f"Support({sr_lb})")
    ax.plot(p.index, res.values, "-", linewidth=1.0, alpha=0.55, label=f"Resist({sr_lb})")
    return sup, res

def _plot_ichimoku(ax, ohlc: pd.DataFrame, conv: int, base: int, span_b: int):
    if ohlc is None or ohlc.empty:
        return None
    tenkan, kijun, senkou_a, senkou_b, chikou = ichimoku_lines(
        ohlc["High"], ohlc["Low"], ohlc["Close"],
        conv=int(conv), base=int(base), span_b=int(span_b),
        shift_cloud=False  # match scanner + simpler
    )
    if kijun.dropna().empty:
        return None
    ax.plot(kijun.index, kijun.values, "-", linewidth=1.2, alpha=0.95, label=f"Kijun({base})")
    ax.plot(tenkan.index, tenkan.values, "-", linewidth=1.0, alpha=0.70, label=f"Tenkan({conv})")
    return {"tenkan": tenkan, "kijun": kijun}

def _plot_supertrend_psar(ax, ohlc: pd.DataFrame, atr_period: int, atr_mult: float, psar_step: float, psar_max: float):
    if ohlc is None or ohlc.empty:
        return
    st_df = compute_supertrend(ohlc, atr_period=int(atr_period), atr_mult=float(atr_mult))
    if not st_df.empty and "ST" in st_df:
        ax.plot(st_df.index, st_df["ST"].values, "-", linewidth=1.2, alpha=0.9, label=f"Supertrend({atr_period},{atr_mult:g})")
    psar_df = compute_psar_from_ohlc(ohlc, step=float(psar_step), max_step=float(psar_max))
    if not psar_df.empty and "PSAR" in psar_df:
        ax.scatter(psar_df.index, psar_df["PSAR"].values, s=14, alpha=0.7, label="PSAR")

def _plot_hma(ax, price: pd.Series, period: int = 55):
    p = _coerce_1d_series(price).dropna()
    if p.empty:
        return None
    h = hma(p, int(period))
    if h.dropna().empty:
        return None
    ax.plot(h.index, h.values, "-", linewidth=1.5, alpha=0.9, label=f"HMA({period})")
    return h

def make_price_figure(symbol: str,
                      frame: str,
                      daily_view_label: str,
                      ntd_win: int,
                      slope_lb: int,
    ...
    frame:
      - "Daily"
      - "Hourly (1d|5d)" via sidebar period param (handled by caller) => pass as "Hourly"
    """
    if str(frame).lower().startswith("d"):
        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        if close_full.empty:
            return None
        close = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna()
        ...
        # Hourly is built by caller with real-times; this helper expects "Close" series already aligned
        return None
    ...
    ax.plot(close.index, close.values, "-", linewidth=1.6, alpha=0.95, label=f"{symbol} Close")

    reg = _plot_regression_bands_on_price(ax, close, slope_lb=int(slope_lb), band_sigma=2.0)
    sr = _plot_sr(ax, close, sr_lb=int(sr_lb))

    if show_fibs:
        _plot_fibs(ax, close)

    if ohlc is not None and not ohlc.empty:
        _plot_ichimoku(ax, ohlc, conv=int(ich_conv), base=int(ich_base), span_b=int(ich_spanb))
        _plot_supertrend_psar(ax, ohlc, atr_period=int(atr_period), atr_mult=float(atr_mult),
                              psar_step=float(psar_step), psar_max=float(psar_max))

    hma_line = _plot_hma(ax, close, period=55)

    # band bounce markers (same as scanner)
    if reg is not None:
        sig = find_band_bounce_signal(close, reg["up"], reg["lo"], reg["slope"])
        if isinstance(sig, dict) and sig.get("time", None) is not None:
            t = sig["time"]
            side = str(sig.get("side", "")).upper()
            px = float(sig.get("price", np.nan))
            if t in close.index and np.isfinite(px):
                col = "tab:green" if side.startswith("B") else "tab:red"
                mark = "^" if side.startswith("B") else "v"
                ax.scatter([t], [px], s=140, marker=mark, color=col, zorder=20)
                ax.text(t, px, f"  {side}", color=col, fontsize=9, fontweight="bold",
                        va="bottom" if side.startswith("B") else "top", zorder=21)

    # optional HMA reversal markers
    if hma_line is not None:
        buy_rev, sell_rev = detect_hma_reversal_masks(close, hma_line, lookback=3)
        if buy_rev.any():
            ax.scatter(close.index[buy_rev], close.loc[buy_rev], marker="s", s=70, alpha=0.9, label="HMA REV (buy)")
        if sell_rev.any():
            ax.scatter(close.index[sell_rev], close.loc[sell_rev], marker="D", s=70, alpha=0.9, label="HMA REV (sell)")
    ...
    # session lines (only meaningful for intraday; leave hook for future)
    if show_sessions and isinstance(close.index, pd.DatetimeIndex) and close.index.tz is not None:
        ...
        h2, l2 = draw_session_lines(ax, lines)
        # add handles to legend
        base_h, base_l = ax.get_legend_handles_labels()
        ax.legend(base_h + h2, base_l + l2, loc="best", fontsize=9, framealpha=0.85)

    # header text
    if reg is not None:
        m = float(reg["slope"]) if np.isfinite(reg["slope"]) else np.nan
        r2 = float(reg["r2"]) if np.isfinite(reg["r2"]) else np.nan
        ax.set_title(f"{symbol} — {daily_view_label}   |   slope={m:+.4g}   R²={r2:.3f}")
    else:
        ax.set_title(f"{symbol} — {daily_view_label}")
...
def make_hourly_price_figure(symbol: str,
                             period: str,
                             ntd_win: int,
                             slope_lb: int,
                             atr_period: int,
    ...
    ax.plot(close.index, close.values, "-", linewidth=1.6, alpha=0.95, label=f"{symbol} Close (Hourly)")

    # regression bands: use numeric X but keep plotting on real time
    x = np.arange(len(close), dtype=float)
    y = close.to_numpy(dtype=float)
    m, b = np.polyfit(x, y, 1)
    yhat = pd.Series(m * x + b, index=close.index)
    resid = y - yhat.to_numpy(dtype=float)
    sigma = float(np.nanstd(resid)) if len(resid) else np.nan
    up = yhat + (2.0 * sigma if np.isfinite(sigma) else 0.0)
    lo = yhat - (2.0 * sigma if np.isfinite(sigma) else 0.0)

    ax.plot(yhat.index, yhat.values, "-", linewidth=1.6, alpha=0.95, label=f"Reg({slope_lb})")
    ax.plot(up.index, up.values, "--", linewidth=1.2, alpha=0.9, label="+2σ")
    ax.plot(lo.index, lo.values, "--", linewidth=1.2, alpha=0.9, label="-2σ")

    # OHLC indicators on hourly (if High/Low exist)
    if {"High","Low","Close"}.issubset(df.columns):
        _plot_supertrend_psar(ax, df[["High","Low","Close"]], atr_period=atr_period, atr_mult=atr_mult,
                              psar_step=psar_step, psar_max=psar_max)
    ...
    ax.set_title(f"{symbol} — Hourly ({period})")
    _apply_ax_common(ax)
    fig.tight_layout()
    return fig

def make_ntd_figure(symbol: str,
                    frame: str,
                    daily_view_label: str,
                    ntd_win: int,
                    sr_lb: int,
    ...
    if str(frame).lower().startswith("d"):
        ...
        npx_full = compute_normalized_price(close_full, window=int(ntd_win))
        ntd = _coerce_1d_series(ntd_full).reindex(close.index)
        npx = _coerce_1d_series(npx_full).reindex(close.index)
    else:
        df = fetch_intraday(symbol, period="5d")
        if df is None or df.empty or "Close" not in df.columns:
            return None
        close = _coerce_1d_series(df["Close"]).ffill().dropna()
        if close.empty:
    ...
    ax.axhline(0.0, linewidth=1.1, alpha=0.35)
    ax.axhline(+0.75, linewidth=0.9, alpha=0.22)
    ax.axhline(-0.75, linewidth=0.9, alpha=0.22)

    ax.plot(ntd.index, ntd.values, "-", linewidth=1.8, alpha=0.95, label=f"NTD({ntd_win})")

    # trend slope on close
    x = np.arange(len(close), dtype=float)
    m, b = np.polyfit(x, close.to_numpy(dtype=float), 1) if len(close) >= 2 else (np.nan, np.nan)

    sup = res = None
    if str(frame).lower().startswith("d"):
        sup_res = _plot_sr(plt.gca(), close, sr_lb=int(sr_lb))  # uses current axes; not desired
        # compute SR without drawing on NTD axes
        p = _coerce_1d_series(close).dropna()
        sup = p.rolling(int(sr_lb), min_periods=1).min()
        res = p.rolling(int(sr_lb), min_periods=1).max()

    if show_npx:
        overlay_npx_on_ntd(ax, npx=npx, ntd=ntd, mark_crosses=True)

    if sup is not None and res is not None:
        overlay_inrange_on_ntd(ax, price=close, sup=sup, res=res)
        overlay_ntd_sr_reversal_stars(ax, price=close, sup=sup, res=res,
                                      trend_slope=float(m) if np.isfinite(m) else np.nan,
                                      ntd=ntd, prox=0.0025, bars_confirm=2)

    overlay_ntd_triangles_by_trend(ax, ntd=ntd, trend_slope=float(m) if np.isfinite(m) else np.nan)

    ax.set_ylim(-1.05, 1.05)
    ax.set_title(f"{symbol} — NTD({ntd_win})   |   trend slope={float(m):+.4g}" if np.isfinite(m) else f"{symbol} — NTD({ntd_win})")
    _apply_ax_common(ax)
    _nice_legend(ax)
    fig.tight_layout()
    return fig
...
# Scanner runners
# ---------------------------
def _parse_symbol_list(text: str):
...
def _df_from_rows(rows: list, sort_cols=None, ascending=False):
    if not rows:
        return pd.DataFrame()
    df = pd.DataFrame(rows)
    if sort_cols:
...
def run_scanners(symbols: list,
    ...
    for sym in symbols:
        ...
        if isinstance(r, dict):
            rows.append(r)
    out["Band Bounce (Daily)"] = _df_from_rows(rows, sort_cols=["Bars Since"], ascending=True)

    # 2) Band-bounce (Hourly)
    rows = []
    for sym in symbols:
        r = last_band_bounce_signal_hourly(sym, period=str(hourly_period), slope_lb=int(slope_lb))
        if isinstance(r, dict):
            rows.append(r)
    out["Band Bounce (Hourly)"] = _df_from_rows(rows, sort_cols=["Bars Since"], ascending=True)

    # 3) NPX↑NTD in Uptrend (Daily)
    rows = []
    for sym in symbols:
        r = last_daily_npx_cross_up_in_uptrend(sym, ntd_win=int(ntd_win), daily_view_label=str(daily_view_label))
        if isinstance(r, dict):
            rows.append(r)
    out["NPX↑NTD (Daily Uptrend)"] = _df_from_rows(rows, sort_cols=["Bars Since"], ascending=True)

    # 4) NPX 0.5 cross with local slope (Daily) - both directions
    rows_up = []
    rows_dn = []
    for sym in symbols:
        ru = last_daily_npx_zero_cross_with_local_slope(
            sym, ntd_win=int(ntd_win), daily_view_label=str(daily_view_label),
            local_slope_lb=max(10, int(slope_lb)), max_abs_npx_at_cross=0.12, direction="up"
        )
        if isinstance(ru, dict):
            rows_up.append(ru)
        rd = last_daily_npx_zero_cross_with_local_slope(
            sym, ntd_win=int(ntd_win), daily_view_label=str(daily_view_label),
            local_slope_lb=max(10, int(slope_lb)), max_abs_npx_at_cross=0.12, direction="down"
        )
        if isinstance(rd, dict):
            rows_dn.append(rd)
    out["NPX 0.5↑ (Local slope OK)"] = _df_from_rows(rows_up, sort_cols=["Bars Since"], ascending=True)
    out["NPX 0.5↓ (Local slope OK)"] = _df_from_rows(rows_dn, sort_cols=["Bars Since"], ascending=True)

    # 5) Fib touch + NPX 0-cross (Daily) BUY/SELL
    rows_buy = []
    rows_sell = []
    for sym in symbols:
        rb = last_daily_fib_npx_zero_signal(
            sym, daily_view_label=str(daily_view_label), ntd_win=int(ntd_win),
            direction="buy", prox=0.01, lookback_bars=120, slope_lb=max(10, int(slope_lb)), npx_confirm_bars=1
        )
        if isinstance(rb, dict):
            rows_buy.append(rb)
        rs = last_daily_fib_npx_zero_signal(
            sym, daily_view_label=str(daily_view_label), ntd_win=int(ntd_win),
            direction="sell", prox=0.01, lookback_bars=120, slope_lb=max(10, int(slope_lb)), npx_confirm_bars=1
        )
        if isinstance(rs, dict):
            rows_sell.append(rs)
    out["Fib+NPX0 (BUY)"] = _df_from_rows(rows_buy, sort_cols=["Bars Since Cross"], ascending=True)
    out["Fib+NPX0 (SELL)"] = _df_from_rows(rows_sell, sort_cols=["Bars Since Cross"], ascending=True)

    # 6) Regression Trend reversals (Daily view) — A always, B optional
    rows_a = []
    rows_b = []
    for sym in symbols:
        res = last_daily_regression_trend_reversal(sym, daily_view_label=str(daily_view_label), slope_lb=int(slope_lb))
        if isinstance(res, dict):
            if isinstance(res.get("A", None), dict):
                rows_a.append(res["A"])
            if isinstance(res.get("B", None), dict):
                rows_b.append(res["B"])
    out["RegTrend Reversal (A)"] = _df_from_rows(rows_a, sort_cols=["Time"], ascending=False)
    if show_reg_b:
        out["RegTrend Reversal (B)"] = _df_from_rows(rows_b, sort_cols=["Time"], ascending=False)

    # 7) Kijun Cross-Up (Daily) — within last N bars
    rows = []
    for sym in symbols:
        r = last_daily_kijun_cross_up(
            sym, daily_view_label=str(daily_view_label), slope_lb=int(slope_lb),
            conv=9, base=26, span_b=52, within_last_n_bars=int(kijun_within_last_n)
        )
        if isinstance(r, dict):
            rows.append(r)
    out["Kijun Cross-Up (Daily)"] = _df_from_rows(rows, sort_cols=["Bars Since Cross"], ascending=True)

    # 8) R² tables (Daily + Hourly)
    rows_d = []
    rows_h = []
    for sym in symbols:
        r2, m, ts = daily_regression_r2(sym, slope_lb=int(slope_lb))
        if np.isfinite(r2):
            rows_d.append({"Symbol": sym, "R2": float(r2), "Slope": float(m) if np.isfinite(m) else np.nan, "Time": ts})
        r2h, mh, tsh = hourly_regression_r2(sym, period=str(hourly_period), slope_lb=int(slope_lb))
        if np.isfinite(r2h):
            rows_h.append({"Symbol": sym, "R2": float(r2h), "Slope": float(mh) if np.isfinite(mh) else np.nan, "Time": tsh})
    out["R² (Daily)"] = _df_from_rows(rows_d, sort_cols=["R2"], ascending=False)
    out["R² (Hourly)"] = _df_from_rows(rows_h, sort_cols=["R2"], ascending=False)
...
# ---------------------------
def main():
    st.set_page_config(page_title="BullBear — NTD + Regression Bands", layout="wide")

    st.title("BullBear — NTD + Regression Bands")

    with st.sidebar:
        st.header("Controls")

        symbol = st.text_input("Symbol", value="SPY").strip().upper()
        chart_mode = st.selectbox("Frame", ["Daily", "Hourly"], index=0)

        daily_view_label = st.selectbox("Daily View", ["6M", "1Y", "2Y", "5Y", "MAX"], index=1)

        hourly_period = st.selectbox("Hourly Period", ["1d", "5d"], index=1)

        ntd_win = st.slider("NTD window", min_value=10, max_value=200, value=55, step=1)
        slope_lb = st.slider("Regression lookback", min_value=10, max_value=300, value=120, step=5)
        sr_lb = st.slider("Support/Resistance lookback", min_value=10, max_value=300, value=80, step=5)

        st.subheader("Ichimoku")
        ich_conv = st.number_input("Tenkan (conv)", min_value=3, max_value=30, value=9, step=1)
        ich_base = st.number_input("Kijun (base)", min_value=10, max_value=100, value=26, step=1)
        ich_spanb = st.number_input("Span B", min_value=20, max_value=200, value=52, step=1)

        st.subheader("Supertrend / PSAR")
        atr_period = st.number_input("ATR period", min_value=3, max_value=50, value=10, step=1)
        atr_mult = st.number_input("ATR mult", min_value=1.0, max_value=10.0, value=3.0, step=0.1, format="%.1f")
        psar_step = st.number_input("PSAR step", min_value=0.01, max_value=0.10, value=0.02, step=0.01, format="%.2f")
        psar_max = st.number_input("PSAR max", min_value=0.10, max_value=0.50, value=0.20, step=0.05, format="%.2f")

        st.subheader("Extras")
        show_news = st.checkbox("Show Yahoo news markers (Daily)", value=True)
        show_fibs = st.checkbox("Show Fibonacci levels (Daily)", value=True)
        show_sessions = st.checkbox("Show London/NY session lines (Hourly)", value=True)

        # User said: "(b) No" → default False
        show_reg_b = st.checkbox("Regression Trend: show (b) upper-band reversal", value=False)

        kijun_within_last_n = st.slider("Kijun cross-up: within last N bars", min_value=0, max_value=30, value=5, step=1)

        st.subheader("Scanner List")
        default_list = "SPY, QQQ, IWM, DIA, AAPL, MSFT, NVDA, AMZN, GOOGL, META, TSLA"
        symbols_text = st.text_area("Symbols (comma/newline)", value=default_list, height=120)

    tabs = st.tabs(["Price Chart", "NTD", "Regression Trend", "Scanner"])

    # --- Price Chart ---
    with tabs[0]:
        if chart_mode == "Daily":
            fig = make_price_figure(
                symbol=symbol, frame="Daily",
                daily_view_label=daily_view_label,
                ntd_win=int(ntd_win),
                slope_lb=int(slope_lb),
                sr_lb=int(sr_lb),
                ich_conv=int(ich_conv), ich_base=int(ich_base), ich_spanb=int(ich_spanb),
            ...
            fig = make_hourly_price_figure(
                symbol=symbol, period=str(hourly_period),
                ntd_win=int(ntd_win),
                slope_lb=int(slope_lb),
                atr_period=int(atr_period), atr_mult=float(atr_mult),
                psar_step=float(psar_step), psar_max=float(psar_max),
    ...
    with tabs[1]:
        ...
        with col1:
            if chart_mode == "Daily":
                v, t = last_daily_ntd_value(symbol, int(ntd_win))
                st.metric("NTD (Daily) last", value=("—" if not np.isfinite(v) else f"{v:+.3f}"), delta=None)
            else:
                v, t = last_hourly_ntd_value(symbol, int(ntd_win), period=str(hourly_period))
                st.metric("NTD (Hourly) last", value=("—" if not np.isfinite(v) else f"{v:+.3f}"), delta=None)
        with col2:
            if chart_mode == "Daily":
                v, t = last_daily_npx_value(symbol, int(ntd_win))
                st.metric("NPX (Daily) last", value=("—" if not np.isfinite(v) else f"{v:+.3f}"), delta=None)
            else:
                v, t = last_hourly_npx_value(symbol, int(ntd_win), period=str(hourly_period))
                st.metric("NPX (Hourly) last", value=("—" if not np.isfinite(v) else f"{v:+.3f}"), delta=None)

    # --- Regression Trend tab (new) ---
    with tabs[2]:
        st.subheader("Regression Trend — last-bar band reversal (Daily view)")

        close_full = _coerce_1d_series(fetch_hist(symbol)).dropna()
        close_show = _coerce_1d_series(subset_by_daily_view(close_full, daily_view_label)).dropna() if not close_full.empty else pd.Series(dtype=float)

        if close_show.empty or len(close_show) < 3:
            st.warning("No daily data returned for the selected view.")
        else:
            yhat, up, lo, m, r2 = regression_with_band(close_show, lookback=int(slope_lb), band_sigma=2.0)

            fig = plt.figure(figsize=(12, 6.6))
            ax = fig.add_subplot(111)
            ax.plot(close_show.index, close_show.values, "-", linewidth=1.6, alpha=0.95, label="Close")
            ax.plot(yhat.index, yhat.values, "-", linewidth=1.6, alpha=0.95, label=f"Reg({slope_lb})")
            ax.plot(up.index, up.values, "--", linewidth=1.2, alpha=0.9, label="+2σ")
            ax.plot(lo.index, lo.values, "--", linewidth=1.2, alpha=0.9, label="-2σ")
            ...
            if a_ok:
                t = close_show.index[-1]
                px = float(close_show.iloc[-1])
                ax.scatter([t], [px], s=160, marker="^", color="tab:green", zorder=10)
                ax.text(t, px, "  (a) lower-band reversal", color="tab:green", fontsize=10, fontweight="bold",
                        va="bottom", zorder=11)

            # annotate (b) only if enabled
            if show_reg_b and b_ok:
                t = close_show.index[-1]
                px = float(close_show.iloc[-1])
                ax.scatter([t], [px], s=160, marker="v", color="tab:red", zorder=10)
                ax.text(t, px, "  (b) upper-band reversal", color="tab:red", fontsize=10, fontweight="bold",
                        va="top", zorder=11)

            ax.set_title(f"{symbol} — Regression Trend ({daily_view_label})   |   slope={float(m):+.4g}   R²={float(r2):.3f}")
            _apply_ax_common(ax)
            _nice_legend(ax)
            fig.tight_layout()
            st.pyplot(fig, clear_figure=True)

            st.write("Signal logic (last bar):")
            st.markdown(
                "- **(a)** Previous close was **below -2σ**, current close is **back inside** and **up** vs prior bar, with **slope > 0**.\n"
                "- **(b)** Previous close was **above +2σ**, current close is **back inside** and **down** vs prior bar, with **slope > 0** (optional display)."
            )
    ...
    with tabs[3]:
        ...
        syms = _parse_symbol_list(symbols_text)
        if not syms:
            st.warning("Enter at least one symbol.")
        else:
            st.caption(f"Scanning {len(syms)} symbols…")
            res = run_scanners(
                symbols=syms,
                ...
                show_reg_b=bool(show_reg_b),
                kijun_within_last_n=int(kijun_within_last_n),
            )
            ...
            st.markdown("---")
            st.caption("Tip: Keep your list small for faster scans.")
