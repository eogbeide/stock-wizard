...
# ... remaining helper/scanner functions continue unchanged in this same file ...
# ... including trend_buy_row_*, hot_buy_row_*, green_zone_buy_alert_row_*,
# ... zero_cross_row_*, green_cross_row_*, hma_cross_up_row_*,
# ... bb_mid_cross_up_row_*, npx_ntd_cross_up_row_*,
# ... price_regression_cross_row_daily, ntd_minus05_cross_row_* ...
...
def render_daily_chart(ticker: str, daily_view_label: str):
    ...
    fig = plt.figure(figsize=(14, 7.4 if show_macd else 6.2))
    if show_macd:
        gs = fig.add_gridspec(2, 1, height_ratios=[4.5, 1.4], hspace=0.06)
    ...
    if not global_line.empty:
        ax.plot(global_line.index, global_line.values, linestyle="--", linewidth=2.0,
                color="tab:green" if global_slope >= 0 else "tab:red",
                label=f"Global Trend ({fmt_slope(global_slope)}/bar)")
    if not _coerce_1d_series(reg).dropna().empty:
        ax.plot(reg.index, reg.values, linestyle="-.", linewidth=1.6, label=f"Regression ({fmt_slope(reg_slope)}/bar)")
    ...
    if show_bbands and not _coerce_1d_series(bb_mid).dropna().empty:
        ax.plot(bb_mid.index, bb_mid.values, linewidth=1.2, label="BB Mid")
    ...
    if show_hma and not _coerce_1d_series(hma).dropna().empty:
        ax.plot(hma.index, hma.values, linewidth=1.2, label=f"HMA({hma_period})")

    if not _coerce_1d_series(support).dropna().empty:
        ax.plot(support.index, support.values, linestyle="--", linewidth=1.1, label=f"Support ({sr_lb_daily})")
    if not _coerce_1d_series(resistance).dropna().empty:
        ax.plot(resistance.index, resistance.values, linestyle="--", linewidth=1.1, label=f"Resistance ({sr_lb_daily})")

    if show_fibs:
        fibs = fibonacci_levels(close)
        for lbl, y in fibs.items():
            ax.axhline(y, linestyle=":", linewidth=0.8, alpha=0.45)
            label_on_left(ax, y, f"Fib {lbl}", fontsize=8)
    ...
    if show_macd and axm is not None:
        axm.plot(macd.index, macd.values, label="MACD", linewidth=1.0)
        ...
        axm.axhline(0, color="0.4", linewidth=0.8)
        style_axes(axm)
    ...
    if show_ntd and not _coerce_1d_series(ntd).dropna().empty:
        fig2, ax2 = plt.subplots(figsize=(14, 3.0))
        if shade_ntd:
            shade_ntd_regions(ax2, ntd)
        ax2.plot(ntd.index, ntd.values, linewidth=1.4, label="NTD")
        ...
        ax2.axhline(-0.5, color="tab:green", linestyle=":", linewidth=0.9)
        ax2.axhline(0.5, color="tab:red", linestyle=":", linewidth=0.9)
    ...
    buy_val = float(support.iloc[-1]) if _coerce_1d_series(support).dropna().shape[0] else float(close.iloc[-1])
    sell_val = float(resistance.iloc[-1]) if _coerce_1d_series(resistance).dropna().shape[0] else float(close.iloc[-1])
    ...
    return {
        ...
        "trade_instruction": format_trade_instruction(reg_slope, buy_val, sell_val, float(close.iloc[-1]), ticker, global_trend_slope=global_slope),
        "last_price": float(close.iloc[-1]) if np.isfinite(close.iloc[-1]) else np.nan,
    }

def render_hourly_chart(ticker: str, period: str = "1d"):
    ...
    global_line_h, global_m_h = slope_line(hc, lookback=0)
    ...
    bb_mid_h, bb_up_h, bb_lo_h, _, _ = compute_bbands(hc, window=int(bb_win), mult=float(bb_mult), use_ema=bool(bb_use_ema))
    ntd_h = compute_normalized_trend(hc, window=int(ntd_window))
    npx_h = compute_normalized_price(hc, window=int(ntd_window))
    roc_h = compute_roc(hc, n=int(mom_lb_hourly))
    support_h = hc.rolling(int(sr_lb_hourly)).min()
    resistance_h = hc.rolling(int(sr_lb_hourly)).max()
    real_times = df.index if isinstance(df.index, pd.DatetimeIndex) else hc.index

    n_panels = 1 + int(show_macd) + int(show_mom_hourly) + int(show_nrsi)
    fig = plt.figure(figsize=(14, 4.8 + 1.6 * (n_panels - 1)))
    gs = fig.add_gridspec(n_panels, 1, height_ratios=[4] + [1.2] * (n_panels - 1), hspace=0.08)
    ...
    if show_ichi:
        ...
        if not _coerce_1d_series(kijun_h).dropna().empty:
    ...
    if not _coerce_1d_series(support_h).dropna().empty:
        ax.plot(support_h.index, support_h.values, linestyle="--", linewidth=1.1, label=f"Support ({sr_lb_hourly})")
    if not _coerce_1d_series(resistance_h).dropna().empty:
        ax.plot(resistance_h.index, resistance_h.values, linestyle="--", linewidth=1.1, label=f"Resistance ({sr_lb_hourly})")

    if mode == "Forex" and show_sessions_pst:
        sess_lines = compute_session_lines(real_times)
        sess_handles, sess_labels = draw_session_lines(ax, sess_lines, alpha=0.25)
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles + sess_handles, labels + sess_labels, loc="best", ncol=2)
    else:
        ax.legend(loc="best", ncol=2)

    ax.set_title(f"{ticker} Hourly ({period})")
    style_axes(ax)
    _apply_compact_time_ticks(ax, pd.DatetimeIndex(real_times), n_ticks=8)

    if show_macd and ax_macd is not None:
        ax_macd.plot(macd_h.index, macd_h.values, label="MACD", linewidth=1.0)
        ax_macd.plot(macd_sig_h.index, macd_sig_h.values, label="Signal", linewidth=1.0)
        ax_macd.bar(macd_hist_h.index, macd_hist_h.values, alpha=0.35, label="Hist")
        ax_macd.axhline(0, color="0.4", linewidth=0.8)
        style_axes(ax_macd)
        ax_macd.legend(loc="best", ncol=3)

    if show_mom_hourly and ax_roc is not None:
        ax_roc.plot(roc_h.index, roc_h.values, label=f"ROC({mom_lb_hourly})", linewidth=1.0)
    ...
    if show_nrsi and ax_ntd is not None:
        if shade_ntd:
            shade_ntd_regions(ax_ntd, ntd_h)
        ax_ntd.plot(ntd_h.index, ntd_h.values, label="NTD", linewidth=1.2)
        if show_npx_ntd and not _coerce_1d_series(npx_h).dropna().empty:
            overlay_npx_on_ntd(ax_ntd, npx_h, ntd_h, mark_crosses=mark_npx_cross)
        if show_ntd_channel:
            overlay_inrange_on_ntd(ax_ntd, hc, support_h, resistance_h)
        ax_ntd.axhline(0, color="black", linewidth=0.8)
        ax_ntd.axhline(-0.5, color="tab:green", linestyle=":", linewidth=0.9)
        ax_ntd.axhline(0.5, color="tab:red", linestyle=":", linewidth=0.9)
        ax_ntd.set_ylim(-1.05, 1.05)
        style_axes(ax_ntd)
        ax_ntd.legend(loc="best", ncol=3)
    ...
    buy_val = float(support_h.iloc[-1]) if _coerce_1d_series(support_h).dropna().shape[0] else float(hc.iloc[-1])
    sell_val = float(resistance_h.iloc[-1]) if _coerce_1d_series(resistance_h).dropna().shape[0] else float(hc.iloc[-1])

    return {
        "global_slope": float(global_m_h) if np.isfinite(global_m_h) else np.nan,
        "reg_slope": float(m_h) if np.isfinite(m_h) else np.nan,
        "rev_prob": float(rev_prob_h) if np.isfinite(rev_prob_h) else np.nan,
        "trade_instruction": format_trade_instruction(m_h, buy_val, sell_val, float(hc.iloc[-1]), ticker, global_trend_slope=global_m_h),
